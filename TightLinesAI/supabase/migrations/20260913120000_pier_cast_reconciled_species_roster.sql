-- Reconcile recurring-catch admission: Manistee North Pier smallmouth. Preserve v1/v2 historical rosters.
...
begin
  ...
  if p_version is null or p_version not in ('piercast-private-roster-v2-2026-09-12','piercast-private-roster-v3-2026-09-13') then raise exception 'unknown PierCast roster'; end if;
  ...
  if p_city = 'manistee_mi' and p_version = 'piercast-private-roster-v3-2026-09-13' then result := result || array['smallmouth_bass']; end if;
  return result;
end; $$;
...
alter table public.pier_cast_shadow_forecasts add constraint pier_cast_shadow_forecasts_species_id_check check (species_id = any(public.pier_cast_private_roster(city_id, 'piercast-private-roster-v3-2026-09-13')));
alter table public.pier_cast_shadow_outcomes drop constraint pier_cast_shadow_outcomes_species_id_check;
alter table public.pier_cast_shadow_outcomes add constraint pier_cast_shadow_outcomes_species_id_check check (species_id = any(public.pier_cast_private_roster(city_id, 'piercast-private-roster-v3-2026-09-13')));
alter table public.pier_cast_shadow_forecast_runs drop constraint pier_cast_shadow_forecast_runs_forecast_count_check;
alter table public.pier_cast_shadow_forecast_runs add constraint pier_cast_shadow_forecast_runs_forecast_count_check check (forecast_count in (100,135,140));
...
begin
  ...
  expected_count := case when roster_version = 'piercast-five-city-four-species-v1' then 100 when roster_version = 'piercast-private-roster-v2-2026-09-12' then 135 else 140 end;

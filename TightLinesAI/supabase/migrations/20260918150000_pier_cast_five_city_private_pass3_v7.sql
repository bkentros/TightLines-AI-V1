-- Add Two Rivers, Kewaunee, Algoma, Manitowoc, and Waukegan to the
...
begin
  ...
  if p_run->>'configVersion'<>'piercast-v3-seventeen-city-five-city-pass3-v7'
     or p_run->>'engineVersion'<>'pier-cast-opportunity-modes-v3-shadow-v1.5.0'
     ...
     or jsonb_array_length(p_forecasts)<>590
     or (select count(distinct item->>'cityId') from jsonb_array_elements(p_forecasts)item)<>17
     or (select count(distinct (item->>'cityId',item->>'speciesId')) from jsonb_array_elements(p_forecasts)item)<>118
     or exists(
  ...
  if inserted_run is null then
    ...
    if target_run is null or existing_count<>590 then
      raise exception 'existing Formula v3 five-city-pass3 run is incomplete';
    end if;
    return jsonb_build_object(
      'status','already_committed','runId',target_run,'forecastCount',590
    );
  end if;
  ...
  if (
    select count(*)<>590
      or count(distinct city_id)<>17
      or count(distinct local_date)<>5
      or count(distinct(city_id,species_id))<>118
  ...
  return jsonb_build_object(
    'status','committed','runId',target_run,'forecastCount',590
  );
end;
$$;

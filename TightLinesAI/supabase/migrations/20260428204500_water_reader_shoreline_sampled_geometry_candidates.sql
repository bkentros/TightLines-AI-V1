-- Shoreline-sampled deterministic anchors for Water Reader geometry candidates (RPC output only).
-- Geometry-only: polygon shoreline rings → sampled boundary points → local turn angle → ranked anchors with compact overlays.
-- Does NOT fetch/cache/store/export imagery, depth/bathymetry, contours, CV/pixels, fish zones, access claims, or exact GPS assertions.
...
filtered_parts as (
  ...
  where ST_GeometryType(poly_geom) = 'ST_Polygon'
    and ST_Area(poly_geom::geography) >= 10000::double precision
),
rings as (
  select
    lake_row_id,
    ...
    greatest(24, least(96, floor(ST_Length(ST_Boundary(poly_geom)::geography) / 40.0)::integer))::integer as n_samples
  from filtered_parts
  where ST_Length(ST_Boundary(poly_geom)::geography) >= 120::double precision
),
samples as (
  select
    r.*,
...
anchors_filtered as (
  ...
  from turn_scores t
  where coalesce(t.turn_deg, 0) >= 12::double precision
),
ctx_env as (
  select
    c.lake_row_id,
...
scored as (
  select
    ...
    case
      when coalesce(w.turn_deg, 0::double precision) >= 35::double precision then 'shoreline_complexity'::text
      else 'coverage_distribution'::text
    end as feature_tag,
    case
      when coalesce(w.turn_deg, 0::double precision) >= 35::double precision then 'shoreline_area_geometry_context'::text
      else 'map_region_callout'::text
    end as reason_code
  from water_cut w
  where ST_Contains(w.poly_geom, w.anchor_geom)
    and coalesce(w.dist_shore_m, 999999::double precision) <= 220::double precision
    and coalesce(w.water_frac_overlay, 0::double precision) >= 0.22::double precision
),
ranked_all as (
  select
    s.*,
...
comment on function public.plan_waterbody_aerial_geometry_candidates(uuid, integer, integer) is
  'Shoreline-sampled geometry anchors from polygon boundaries (turn-angle heuristic); no imagery/depth/fish/access claims; Args unchanged: in_lake_id, in_requested_month (echoed), in_requested_max_zones clamped [3,5].';

select pg_notify('pgrst', 'reload schema');

-- Smoke (manual — use a real waterbody_index.id on the target DB):
-- select lake_id, candidate_id, feature_tag, anchor_lon, anchor_lat, overlay_w, overlay_h, geometry_qa
-- from public.plan_waterbody_aerial_geometry_candidates(

...
as $$
  ...
  bbox_padded as (
    select
      ...
      least(0.18, greatest(0.002, ((max_lat - min_lat) / 2.0) * 1.08)) as half_lat,
      least(
...
begin
  ...
  if short_generic_prefix then
    ...
    with candidate_matches as (
      select * from (
        select
          w.id, w.canonical_name, w.state_code, w.county_name, w.waterbody_type,
          w.surface_area_acres, w.centroid, w.geometry,
          case when w.normalized_name = norm_q then 0 when w.normalized_name like norm_q || '%' then 10 else 100 end + w.search_priority as rank_score
        from public.waterbody_index w
        where norm_q <> '' and w.is_named = true and w.is_searchable = true and w.waterbody_type in ('lake', 'pond', 'reservoir') and (state_q is null or w.state_code = state_q) and w.normalized_name like norm_q || '%'
      ...
      select * from (
        select
          w.id, w.canonical_name, w.state_code, w.county_name, w.waterbody_type,
          w.surface_area_acres, w.centroid, w.geometry,
          case when a.normalized_alias_name = norm_q then 2 when a.normalized_alias_name like norm_q || '%' then 12 else 100 end + w.search_priority as rank_score
        from public.waterbody_aliases a
        join public.waterbody_index w on w.id = a.waterbody_id
        where norm_q <> '' and w.is_named = true and w.is_searchable = true and w.waterbody_type in ('lake', 'pond', 'reservoir') and (state_q is null or w.state_code = state_q) and a.normalized_alias_name like norm_q || '%'
        order by w.search_priority, w.canonical_name, w.state_code, coalesce(w.county_name, '')
        limit 50
      ) alias_phrase
    ),
    deduped as (
      select c.*, row_number() over (partition by c.id order by c.rank_score, c.canonical_name, c.state_code, coalesce(c.county_name, '')) as rn
      from candidate_matches c
    ),
    limited as materialized (
      select * from deduped d where d.rn = 1
      order by d.rank_score, d.canonical_name, d.state_code, coalesce(d.county_name, '')
      limit row_limit
    )
    select e.*
    from limited d
    cross join lateral public.search_waterbodies_enrich_row(d.id, d.canonical_name, d.state_code, d.county_name, d.waterbody_type, d.surface_area_acres, d.centroid, d.geometry) e
    order by d.rank_score, d.canonical_name, d.state_code, coalesce(d.county_name, '');
    return;
  end if;
  ...
  with candidate_matches as (
    select
      w.id, w.canonical_name, w.state_code, w.county_name, w.waterbody_type, w.surface_area_acres, w.centroid, w.geometry,
      case when w.normalized_name = norm_q then 0 when w.normalized_name like norm_q || '%' then 10 when (not single_token_generic) and w.normalized_name like '%' || norm_q || '%' then 20 else 100 end + w.search_priority as rank_score
    from public.waterbody_index w
    where norm_q <> '' and w.is_named = true and w.is_searchable = true and w.waterbody_type in ('lake', 'pond', 'reservoir') and (state_q is null or w.state_code = state_q) and (not single_token_generic) and w.normalized_name like '%' || norm_q || '%'

    union all

    select
      w.id, w.canonical_name, w.state_code, w.county_name, w.waterbody_type, w.surface_area_acres, w.centroid, w.geometry,
      case when w.normalized_name = norm_q then 0 when w.normalized_name like norm_q || '%' then 10 when (not single_token_generic) and w.normalized_name like '%' || norm_q || '%' then 20 when coalesce(array_length(specific_tokens, 1), 0) >= 1 and not exists (select 1 from unnest(specific_tokens) as q(tok) where w.normalized_name not like '%' || q.tok || '%') then 24 else 100 end + w.search_priority as rank_score
    from public.waterbody_index w
    where norm_q <> '' and w.is_named = true and w.is_searchable = true and w.waterbody_type in ('lake', 'pond', 'reservoir') and (state_q is null or w.state_code = state_q) and coalesce(array_length(specific_tokens, 1), 0) >= 1 and long_tok is not null and w.normalized_name like '%' || long_tok || '%' and not exists (select 1 from unnest(specific_tokens) as q2(tok) where w.normalized_name not like '%' || q2.tok || '%')

    union all

    select
      w.id, w.canonical_name, w.state_code, w.county_name, w.waterbody_type, w.surface_area_acres, w.centroid, w.geometry,
      case when a.normalized_alias_name = norm_q then 2 when a.normalized_alias_name like norm_q || '%' then 12 when (not single_token_generic) and a.normalized_alias_name like '%' || norm_q || '%' then 22 else 100 end + w.search_priority as rank_score
    from public.waterbody_aliases a join public.waterbody_index w on w.id = a.waterbody_id
    where norm_q <> '' and w.is_named = true and w.is_searchable = true and w.waterbody_type in ('lake', 'pond', 'reservoir') and (state_q is null or w.state_code = state_q) and (not single_token_generic) and a.normalized_alias_name like '%' || norm_q || '%'

    union all

    select
      w.id, w.canonical_name, w.state_code, w.county_name, w.waterbody_type, w.surface_area_acres, w.centroid, w.geometry,
      case when a.normalized_alias_name = norm_q then 2 when a.normalized_alias_name like norm_q || '%' then 12 when (not single_token_generic) and a.normalized_alias_name like '%' || norm_q || '%' then 22 when coalesce(array_length(specific_tokens, 1), 0) >= 1 and not exists (select 1 from unnest(specific_tokens) as q4(tok) where a.normalized_alias_name not like '%' || q4.tok || '%') then 26 else 100 end + w.search_priority as rank_score
    from public.waterbody_aliases a join public.waterbody_index w on w.id = a.waterbody_id
    where norm_q <> '' and w.is_named = true and w.is_searchable = true and w.waterbody_type in ('lake', 'pond', 'reservoir') and (state_q is null or w.state_code = state_q) and coalesce(array_length(specific_tokens, 1), 0) >= 1 and long_tok is not null and a.normalized_alias_name like '%' || long_tok || '%' and not exists (select 1 from unnest(specific_tokens) as q5(tok) where a.normalized_alias_name not like '%' || q5.tok || '%')
  ),
  deduped as (
    select c.*, row_number() over (partition by c.id order by c.rank_score, c.canonical_name, c.state_code, coalesce(c.county_name, '')) as rn
    from candidate_matches c

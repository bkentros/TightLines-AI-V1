-- Water Reader V1: polygon-derived support status on search rows (geometry from waterbody_index only).
-- Baseline rules aligned with get_waterbody_polygon_for_reader. Does not replace data_tier / source fields.

drop function if exists public.search_waterbodies(text, text, integer);
drop function if exists public.search_waterbodies_enrich_row(uuid, text, text, text, text, numeric, geometry, geometry);
...
as $$
  ...
  v1status as (
    select
      vf.poly_acres as polygon_area_acres,
      vf.polygon_qa_flags,
      ...
      case
        ...
        when vf.poly_acres is not null and vf.poly_acres < 0.5 then 'not_supported'
        when vf.poly_components > 1 or vf.poly_pts > 25000 then 'needs_review'
      ...
      case
        ...
        when vf.poly_acres is not null and vf.poly_acres < 0.5 then 'Polygon area is below the minimum threshold for Water Reader V1.'
        when vf.poly_components > 1 or vf.poly_pts > 25000 then 'Geometry may need review (multipart or high complexity).'

-- Add Chicago, Michigan City, Muskegon, Whitehall, and Alpena to the
...
alter table public.pier_cast_v3_shadow_forecast_runs
  add constraint pier_cast_v3_shadow_forecast_runs_forecast_count_check
    check(forecast_count in (180,280,350,470,590,845));
...
begin
  ...
  if p_run->>'configVersion'<>'piercast-v3-twenty-two-city-chicago-alpena-pass3-v8'
     or p_run->>'engineVersion'<>'pier-cast-opportunity-modes-v3-shadow-v1.6.0'
     ...
     or jsonb_array_length(p_forecasts)<>845
     or (select count(distinct item->>'cityId') from jsonb_array_elements(p_forecasts)item)<>22
     or (select count(distinct (item->>'cityId',item->>'speciesId')) from jsonb_array_elements(p_forecasts)item)<>169
     or exists(
  ...
  then
    raise exception 'invalid Formula v3 five-city-pass3 shadow payload';
  end if;
  ...
  if inserted_run is null then
    ...
    if target_run is null or existing_count<>845 then
      raise exception 'existing Formula v3 five-city-pass3 run is incomplete';
    end if;
    return jsonb_build_object(
      'status','already_committed','runId',target_run,'forecastCount',845
    );
  end if;
  ...
  if (
    select count(*)<>845
      or count(distinct city_id)<>22
      or count(distinct local_date)<>5
      or count(distinct(city_id,species_id))<>169
    ...
    raise exception 'Formula v3 five-city-pass3 run manifest is incomplete';
  end if;
  ...
  return jsonb_build_object(
    'status','committed','runId',target_run,'forecastCount',845
  );
end;
$$;
...
comment on function public.piercast_v3_expected_pairs() is
  'Exact private Formula v3 22-city, 169-pair manifest. Absence is not biological absence.';

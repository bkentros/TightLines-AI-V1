-- Add St. Joseph, South Haven, Holland, Lexington, and Harrisville to
...
alter table public.pier_cast_expansion_temperature_samples
  add constraint pier_cast_expansion_temperature_samples_city_id_check check(city_id in ('port_washington_wi','milwaukee_wi','racine_wi','kenosha_wi','harbor_beach_mi','oscoda_mi','port_sanilac_mi','two_rivers_wi','kewaunee_wi','algoma_wi','manitowoc_wi','waukegan_il','chicago_il','michigan_city_in','muskegon_mi','whitehall_mi','alpena_mi','st_joseph_mi','south_haven_mi','holland_mi','lexington_mi','harrisville_mi')),
     ...
     or exists(
       ...
       where item->>'cityId' not in ('st_joseph_mi','south_haven_mi','holland_mi','lexington_mi','harrisville_mi')
          ...
          or not case item->>'cityId'
            when 'st_joseph_mi' then (item->>'gridRow')::integer=52 and (item->>'gridColumn')::integer=156 and (item->>'latitude')::double precision=42.12 and (item->>'longitude')::double precision=-86.5
            when 'south_haven_mi' then (item->>'gridRow')::integer=80 and (item->>'gridColumn')::integer=177 and (item->>'latitude')::double precision=42.4 and (item->>'longitude')::double precision=-86.29
            when 'holland_mi' then (item->>'gridRow')::integer=117 and (item->>'gridColumn')::integer=184 and (item->>'latitude')::double precision=42.77 and (item->>'longitude')::double precision=-86.22
            when 'lexington_mi' then (item->>'gridRow')::integer=167 and (item->>'gridColumn')::integer=554 and (item->>'latitude')::double precision=43.27 and (item->>'longitude')::double precision=-82.52
            when 'harrisville_mi' then (item->>'gridRow')::integer=306 and (item->>'gridColumn')::integer=478 and (item->>'latitude')::double precision=44.66 and (item->>'longitude')::double precision=-83.28
            else false end
     )
...
alter table public.pier_cast_v3_shadow_forecasts
  add constraint pier_cast_v3_shadow_forecasts_city_id_check check(city_id in (
    ...
    'port_washington_wi','milwaukee_wi','racine_wi','kenosha_wi','harbor_beach_mi','oscoda_mi','port_sanilac_mi','two_rivers_wi','kewaunee_wi','algoma_wi','manitowoc_wi','waukegan_il','chicago_il','michigan_city_in','muskegon_mi','whitehall_mi','alpena_mi','st_joseph_mi','south_haven_mi','holland_mi','lexington_mi','harrisville_mi'
  ));
...
begin
  ...
  if p_run->>'configVersion'<>'piercast-v3-twenty-seven-city-st-joseph-harrisville-pass3-v10'
     or p_run->>'engineVersion'<>'pier-cast-opportunity-modes-v3-shadow-v1.8.0'
     ...
     or jsonb_array_length(p_forecasts)<>1110
     or (select count(distinct item->>'cityId') from jsonb_array_elements(p_forecasts)item)<>27
     or (select count(distinct (item->>'cityId',item->>'speciesId')) from jsonb_array_elements(p_forecasts)item)<>222
     or exists(
  ...
  then
    raise exception 'invalid Formula v3 st-joseph-harrisville-pass3 shadow payload';
  end if;
  ...
  if inserted_run is null then
    ...
    if target_run is null or existing_count<>1110 then
      raise exception 'existing Formula v3 st-joseph-harrisville-pass3 run is incomplete';
    end if;
    return jsonb_build_object(
      'status','already_committed','runId',target_run,'forecastCount',1110
    );
  end if;
  ...
  if (
    select count(*)<>1110
      or count(distinct city_id)<>27
      or count(distinct local_date)<>5
      or count(distinct(city_id,species_id))<>222
    ...
    raise exception 'Formula v3 st-joseph-harrisville-pass3 run manifest is incomplete';
  end if;
  ...
  return jsonb_build_object(
    'status','committed','runId',target_run,'forecastCount',1110
  );
end;
$$;

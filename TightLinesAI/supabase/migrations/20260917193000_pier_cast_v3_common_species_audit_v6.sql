-- Private v3 all-city common-species calibration audit. Keep prior run versions and manifests immutable.
-- Formula v2 and public PierCast data are unchanged.
...
begin
  ...
  if p_run->>'configVersion'<>'piercast-v3-twelve-city-common-species-audit-v6'
     or p_run->>'engineVersion'<>'pier-cast-opportunity-modes-v3-shadow-v1.4.0'
  ...
  then
    raise exception 'invalid Formula v3 common-species-audit shadow payload';
  end if;
  ...
  if inserted_run is null then
    ...
    if target_run is null or existing_count<>470 then
      raise exception 'existing Formula v3 common-species-audit run is incomplete';
    end if;
  ...
  if (
    ...
    raise exception 'Formula v3 common-species-audit run manifest is incomplete';
  end if;

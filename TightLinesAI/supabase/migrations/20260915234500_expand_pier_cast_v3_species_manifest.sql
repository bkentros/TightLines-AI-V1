-- Expand the private, promotion-blocked Formula v3 archive from 70 to 94
-- city/species pairs. Historical 180/280/350-row runs remain valid and
-- immutable. Public Formula v2 tables, RPCs, rosters, and scores are untouched.
...
alter table public.pier_cast_v3_shadow_forecast_runs
  ...
  add constraint pier_cast_v3_shadow_forecast_runs_forecast_count_check check (
    forecast_count in (180,280,350,470)
  );
...
alter table public.pier_cast_v3_shadow_forecasts
  add constraint pier_cast_v3_shadow_forecasts_species_id_check check (
    species_id in (
...
begin
  ...
  if p_run->>'configVersion'<>'piercast-v3-twelve-city-species-expansion-v4'
     or p_run->>'engineVersion'<>'pier-cast-opportunity-modes-v3-shadow-v1.3.0'
     ...
     or jsonb_array_length(p_forecasts)<>470
     or (select count(distinct item->>'cityId') from jsonb_array_elements(p_forecasts)item)<>12
     or (select count(distinct (item->>'cityId',item->>'speciesId')) from jsonb_array_elements(p_forecasts)item)<>94
     or exists(
  ...
  then
    raise exception 'invalid Formula v3 species-expansion shadow payload';
  end if;
  ...
  if inserted_run is null then
    ...
    if target_run is null or existing_count<>470 then
      raise exception 'existing Formula v3 species-expansion run is incomplete';
    end if;
    return jsonb_build_object(
      'status','already_committed','runId',target_run,'forecastCount',470
    );
  end if;
  ...
  if (
    select count(*)<>470
      or count(distinct city_id)<>12
      or count(distinct local_date)<>5
      or count(distinct(city_id,species_id))<>94
    ...
    raise exception 'Formula v3 species-expansion run manifest is incomplete';
  end if;
  ...
  return jsonb_build_object(
    'status','committed','runId',target_run,'forecastCount',470
  );
end;
$$;

revoke all on function public.piercast_v3_expected_pairs()
...
comment on table public.pier_cast_v3_shadow_forecast_runs is
  'Private disabled Formula v3 ledger preserving historical manifests and exact 12-city 94-pair species-expansion runs; never a public release source.';
comment on function public.piercast_v3_expected_pairs() is
  'Exact private Formula v3 12-city, 94-pair calibration manifest; absence is not biological absence.';

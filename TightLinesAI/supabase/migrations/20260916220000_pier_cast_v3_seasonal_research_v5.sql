-- Private v3 seasonal research recalibration. Keep prior run versions and manifests immutable.
...
begin
  ...
  if p_run->>'configVersion'<>'piercast-v3-twelve-city-seasonal-research-v5'
  ...
  then
    raise exception 'invalid Formula v3 seasonal-research shadow payload';
  end if;
  ...
  if inserted_run is null then
    ...
    if target_run is null or existing_count<>470 then
      raise exception 'existing Formula v3 seasonal-research run is incomplete';
    end if;
  ...
  if (
    ...
    raise exception 'Formula v3 seasonal-research run manifest is incomplete';
  end if;

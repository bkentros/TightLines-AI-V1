...
begin
  ...
  if p_run->>'configVersion'<>'piercast-v3-twenty-two-city-chicago-alpena-pass3-v9'
     or p_run->>'engineVersion'<>'pier-cast-opportunity-modes-v3-shadow-v1.7.0'
     ...
     or jsonb_array_length(p_forecasts)<>865
     or (select count(distinct item->>'cityId') from jsonb_array_elements(p_forecasts)item)<>22
     or (select count(distinct (item->>'cityId',item->>'speciesId')) from jsonb_array_elements(p_forecasts)item)<>173
     or exists(
  ...
  if inserted_run is null then
    ...
    if target_run is null or existing_count<>865 then
      raise exception 'existing Formula v3 five-city-pass3 run is incomplete';
    end if;
    return jsonb_build_object(
      'status','already_committed','runId',target_run,'forecastCount',865
    );
  end if;
  ...
  if (
    select count(*)<>865
      ...
      or count(distinct(city_id,species_id))<>173
  ...
  return jsonb_build_object(
    'status','committed','runId',target_run,'forecastCount',865
  );
end;
$$;

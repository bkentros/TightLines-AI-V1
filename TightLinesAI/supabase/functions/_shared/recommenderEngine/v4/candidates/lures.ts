import type { EngineContext } from "../../../howFishingEngine/contracts/context.ts";
import type { ArchetypeProfileV4 } from "../contracts.ts";
import { lure } from "./factory.ts";

const L = "freshwater_lake_pond" as const satisfies EngineContext;
const R = "freshwater_river" as const satisfies EngineContext;

/**
 * Full lure catalog — values per Appendix A §22.1 (authoritative),
 * plus a focused trout river surface plug added during recommender QA tuning.
 * `display_name` and `how_to_fish_variants` were carried from the legacy catalog
 * where the id existed; the three new pike archetypes use how_to_fish text adapted
 * from legacy `inline_spinner`,
 * `walking_topwater`, and `football_jig` respectively (Appendix does not specify copy).
 */
export const LURE_ARCHETYPES_V4: readonly ArchetypeProfileV4[] = [
  lure({
    id: "weightless_stick_worm",
    display_name: "Weightless Stick Worm",
    family_group: "soft_plastic_worm",
    presentation_group: "worm_finesse",
    column: "upper",
    primary_pace: "medium",
    secondary_pace: "slow",
    forage_tags: ["leech_worm"],
    clarity_strengths: ["clear", "stained", "dirty"],
    condition_tags: ["clear_subtle", "heat_finesse"],
    goal_tags: ["reliable_action", "versatile_search"],
    species_allowed: ["largemouth_bass", "smallmouth_bass"],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Pitch or skip it tight to cover and let it glide on a slack line — most hits come on the slow shimmying fall through the upper column; reset after each fall and fish the next pocket.",
      "Cast to shade lines and feed slack on the fall so the bait shimmies naturally through the upper/mid column; if it reaches cover without a bite, lift it free and re-pitch rather than dragging.",
      "Work shallow cover with short pitches and slack-line falls; let the bait hang motionless for a beat after each fall, then lift and re-present — keep it in the fall zone, not on the bottom.",
    ],
  }),
  lure({
    id: "carolina_rigged_stick_worm",
    display_name: "Carolina-Rigged Stick Worm",
    family_group: "soft_plastic_worm",
    presentation_group: "worm_finesse",
    column: "bottom",
    primary_pace: "slow",
    forage_tags: ["leech_worm", "baitfish"],
    clarity_strengths: ["clear", "stained"],
    condition_tags: ["clear_subtle", "cold_slow"],
    goal_tags: ["reliable_action", "versatile_search"],
    species_allowed: ["largemouth_bass", "smallmouth_bass", "trout"],
    water_types_allowed: [L],
    how_to_fish_variants: [
      "Drag the rig slowly across hard bottom and transitions so the leader lets the worm hunt above the weight; pause when you feel it tick structure.",
      "Sweep the rod slowly across rocky or hard bottom and watch for the line to go tight; the worm floats and hunts freely above the sinker on a stretched leader.",
      "Long sweeping drags followed by a complete pause; the worm sinks and rises naturally on the leader — focus on structure changes and hard bottom.",
    ],
  }),
  lure({
    id: "shaky_head_worm",
    display_name: "Shaky-Head Worm",
    family_group: "soft_plastic_worm",
    presentation_group: "worm_finesse",
    column: "bottom",
    primary_pace: "slow",
    forage_tags: ["leech_worm"],
    clarity_strengths: ["clear", "stained", "dirty"],
    condition_tags: [
      "clear_subtle",
      "cold_slow",
      "heat_finesse",
      "current_swing",
    ],
    goal_tags: ["reliable_action"],
    species_allowed: ["largemouth_bass", "smallmouth_bass"],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Cast and let it fall to bottom, then drag very slowly with the rod held low; shake the tip just enough to quiver the tail without lifting the head.",
      "Pin the nose to bottom and shake the rod tip while crawling it forward an inch at a time — don't rush the retrieve.",
      "Steady drag along hard bottom with subtle rod shakes; pause on any tick or thump and let it sit a full two seconds before moving again.",
    ],
  }),
  lure({
    id: "magnum_worm",
    display_name: "Magnum Worm",
    family_group: "soft_plastic_worm",
    presentation_group: "worm_power",
    column: "bottom",
    primary_pace: "slow",
    secondary_pace: "medium",
    forage_tags: ["leech_worm"],
    clarity_strengths: ["clear", "stained", "dirty"],
    condition_tags: ["cover_ambush", "heat_finesse"],
    goal_tags: ["big_fish_upside", "high_risk_high_reward"],
    species_allowed: ["largemouth_bass"],
    water_types_allowed: [L],
    how_to_fish_variants: [
      "Drag the oversized worm through brush, grass edges, and deeper cover with a slow sweep; pause after each pull so the tail floats and waves in place.",
      "Cast to ledges or outside weed lines, let it reach bottom, then crawl it with long pauses instead of hopping it out of the strike zone.",
      "Pitch it to big-fish cover and let it fall on a semi-slack line; if it reaches bottom cleanly, shake once and drag slowly before the next target.",
    ],
  }),
  lure({
    id: "drop_shot_worm",
    display_name: "Drop-Shot Worm",
    family_group: "soft_plastic_worm",
    presentation_group: "worm_finesse",
    column: "mid",
    primary_pace: "slow",
    forage_tags: ["leech_worm"],
    clarity_strengths: ["clear", "stained"],
    condition_tags: ["clear_subtle", "heat_finesse"],
    goal_tags: ["reliable_action"],
    species_allowed: ["largemouth_bass", "smallmouth_bass"],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Hold the weight on the bottom and gently shake the rod tip so the worm quivers in place; move it only a foot or two before letting it settle again.",
      "Keep the rig nearly vertical and make tiny rod-tip pulses while the weight stays pinned; the worm should hover and tremble without traveling far.",
      "Shake in place with light tension, then pause completely and let the worm hang still; most bites come when the bait barely moves at all.",
    ],
  }),
  lure({
    id: "drop_shot_minnow",
    display_name: "Drop-Shot Minnow",
    family_group: "soft_plastic_minnow",
    presentation_group: "drop_shot_minnow",
    column: "mid",
    primary_pace: "slow",
    forage_tags: ["baitfish", "crawfish"],
    clarity_strengths: ["clear", "stained"],
    condition_tags: ["clear_subtle", "heat_finesse"],
    goal_tags: ["reliable_action"],
    species_allowed: ["largemouth_bass", "smallmouth_bass", "trout"],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Keep the bait just off bottom with tiny shakes and short glides so the minnow hovers naturally in the strike zone.",
      "Hold the weight down and pulse the rod tip lightly; the minnow should quiver in place, then glide a few inches before the next shake.",
      "Use a subtle shake-pause cadence with the line tight enough to feel the bait; let the minnow suspend and look alive rather than dragging it forward.",
    ],
  }),
  lure({
    id: "ned_rig",
    display_name: "Ned Rig",
    family_group: "finesse_plastic",
    presentation_group: "ned_tube_finesse",
    column: "bottom",
    primary_pace: "slow",
    forage_tags: ["leech_worm", "crawfish"],
    clarity_strengths: ["clear", "stained", "dirty"],
    condition_tags: ["clear_subtle", "cold_slow", "heat_finesse"],
    goal_tags: ["reliable_action"],
    species_allowed: ["largemouth_bass", "smallmouth_bass", "trout"],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Drag and shake the small head along bottom like a tiny craw — short pulls, let it settle, repeat instead of big hops.",
      "Cast and let it settle fully to the bottom, then drag very slowly with the rod held low; pause between moves so the tail stands upright and wiggles.",
      "Inch it along with small rod pops and long pauses; the mushroom head keeps it nose-down and upright — let it sit longer than you think is necessary.",
    ],
  }),
  lure({
    id: "tube_jig",
    display_name: "Tube Jig",
    family_group: "tube_jig",
    presentation_group: "ned_tube_finesse",
    column: "bottom",
    primary_pace: "slow",
    secondary_pace: "medium",
    forage_tags: ["crawfish", "baitfish"],
    clarity_strengths: ["clear", "stained"],
    condition_tags: ["clear_subtle", "cold_slow"],
    goal_tags: ["reliable_action"],
    species_allowed: [
      "largemouth_bass",
      "smallmouth_bass",
      "northern_pike",
    ],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Snap the tube off bottom with short pops so it spirals on the fall, then let it glide back down on slack or semi-slack line.",
      "Hop it erratically off the bottom with sharp pops, then open the bail slightly so it spirals down freely; strikes happen on that spinning fall.",
      "Cast, let it sink, then pop sharply and release line; the tube should corkscrew back to bottom on each drop — keep a watchful eye on the line.",
    ],
  }),
  lure({
    id: "big_smallmouth_tube",
    display_name: "Big Tube Jig",
    family_group: "tube_jig",
    presentation_group: "ned_tube_finesse",
    column: "bottom",
    primary_pace: "slow",
    secondary_pace: "medium",
    forage_tags: ["crawfish", "baitfish"],
    clarity_strengths: ["clear", "stained"],
    condition_tags: ["clear_subtle", "current_swing", "cold_slow"],
    goal_tags: ["big_fish_upside"],
    species_allowed: ["smallmouth_bass"],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Fish the larger tube on bottom with controlled pops and long glides so it spirals like a goby, craw, or stunned baitfish rather than racing away.",
      "Drag it across rock and gravel, then snap it once and feed a bit of slack so the tube falls wide and slow where larger smallmouth can track it.",
      "Cast upstream or across current and keep it near bottom; use short rod lifts and pauses so the bigger profile stays heavy and deliberate.",
    ],
  }),
  lure({
    id: "texas_rigged_soft_plastic_craw",
    display_name: "Texas-Rigged Craw",
    family_group: "soft_plastic_craw",
    presentation_group: "bottom_jig_craw",
    column: "bottom",
    primary_pace: "slow",
    forage_tags: ["crawfish"],
    clarity_strengths: ["clear", "stained", "dirty"],
    condition_tags: [
      "cover_ambush",
      "cold_slow",
      "clear_subtle",
      "heat_finesse",
    ],
    goal_tags: ["reliable_action"],
    species_allowed: ["largemouth_bass", "smallmouth_bass"],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Flip or pitch into cover, let the craw fall straight on semi-slack line, then hop it once or twice before the next pitch.",
      "Drop it into cover or along grass edges and count it down; short one-inch hops with a pause mimics a crawfish on the bottom — don't rush the next cast.",
      "Pitch to cover and let it fall freely; if nothing on the drop, slowly crawl it along the bottom and shake gently before picking up for the next pitch.",
    ],
  }),
  lure({
    id: "football_jig",
    display_name: "Football Jig",
    family_group: "skirted_jig_bottom",
    presentation_group: "bottom_jig_craw",
    column: "bottom",
    primary_pace: "slow",
    forage_tags: ["crawfish"],
    clarity_strengths: ["clear", "stained", "dirty"],
    condition_tags: ["cold_slow", "cover_ambush"],
    goal_tags: ["big_fish_upside"],
    species_allowed: ["largemouth_bass", "smallmouth_bass"],
    water_types_allowed: [L],
    how_to_fish_variants: [
      "Drag it along hard bottom and ledges so the flat head kicks and rocks; lift only slightly on the pull so the trailer stays near the substrate.",
      "Work it with slow rod sweeps across rocky bottom, letting it tick and grind the structure rather than hop above it.",
      "Crawl it across rock, gravel, or shell beds with steady low lifts and long pauses — the football head keeps it upright and loud the whole time.",
    ],
  }),
  lure({
    id: "compact_flipping_jig",
    display_name: "Compact Flipping Jig",
    family_group: "skirted_jig_bottom",
    presentation_group: "bottom_jig_craw",
    column: "bottom",
    primary_pace: "slow",
    forage_tags: ["crawfish", "bluegill_perch"],
    clarity_strengths: ["stained", "dirty"],
    condition_tags: ["cover_ambush", "dirty_vibration"],
    goal_tags: ["big_fish_upside"],
    species_allowed: ["largemouth_bass"],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Pendulum flip or pitch into heavy cover; let it free-fall on a semi-slack line, then hold and lift if the line jumps or goes heavy.",
      "Get the bait right into the cover before moving it — thumb the spool on the drop, and if you don't feel a strike, give it one or two hops then move on.",
      "Pitch tight to lay-downs, dock piles, or mats; give it enough slack line to fall straight, then drag or shake slowly before picking up for the next cast.",
    ],
  }),
  lure({
    id: "finesse_jig",
    display_name: "Finesse Jig",
    family_group: "skirted_jig_bottom",
    presentation_group: "bottom_jig_craw",
    column: "bottom",
    primary_pace: "slow",
    forage_tags: ["crawfish", "leech_worm"],
    clarity_strengths: ["clear", "stained"],
    condition_tags: ["clear_subtle", "cold_slow", "heat_finesse"],
    goal_tags: ["reliable_action"],
    species_allowed: ["largemouth_bass", "smallmouth_bass"],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Crawl it slowly along bottom with short drags and tiny hops; the smaller profile should stay in the fish's face longer than a bulkier jig.",
      "Pitch it to light cover or transitions and let it settle fully before moving it; small lifts and long pauses are the whole point.",
      "Drag it with the rod low and barely shake the skirt in place; keep it compact, quiet, and close to bottom the entire retrieve.",
    ],
  }),
  lure({
    id: "swim_jig",
    display_name: "Swim Jig",
    family_group: "skirted_jig_swimming",
    presentation_group: "swim_jig",
    column: "mid",
    primary_pace: "medium",
    secondary_pace: "fast",
    forage_tags: ["bluegill_perch", "baitfish"],
    clarity_strengths: ["stained", "dirty"],
    condition_tags: ["cover_ambush", "warming_search"],
    goal_tags: ["reliable_action", "versatile_search"],
    species_allowed: ["largemouth_bass", "smallmouth_bass"],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Reel at a steady medium pace just above cover, letting the trailer kick; vary depth by angling the rod up or down to keep it in the strike zone.",
      "Cast past targets and swim it through, reeling just fast enough to feel the head wiggle; slow down near the edge of cover and let it drop.",
      "Run it parallel to grass edges or fallen timber; keep the pace consistent so the skirt breathes, and pause briefly when it reaches the target zone.",
    ],
  }),
  lure({
    id: "hair_jig",
    display_name: "Hair Jig",
    family_group: "hair_jig",
    presentation_group: "hair_jig",
    column: "bottom",
    primary_pace: "slow",
    secondary_pace: "medium",
    forage_tags: ["baitfish", "leech_worm"],
    clarity_strengths: ["clear", "stained"],
    condition_tags: ["clear_subtle", "current_swing", "cold_slow"],
    goal_tags: ["reliable_action"],
    species_allowed: ["smallmouth_bass", "trout"],
    water_types_allowed: [R],
    how_to_fish_variants: [
      "Cast across, let it sink on a controlled slack line, then swim it back with short pauses so the marabou pulses and collapses.",
      "Fish it on a tight line at slow speed — the hair works best with tiny line pulses rather than big lifts; let it hang in the current or slack zone.",
      "Swing it through current seams or drop it in pockets with minimal movement; let the marabou breathe in place before slowly pulling it along.",
    ],
  }),
  lure({
    id: "inline_spinner",
    display_name: "Inline Spinner",
    family_group: "spinner",
    presentation_group: "spinner_vibration",
    column: "mid",
    primary_pace: "medium",
    secondary_pace: "fast",
    forage_tags: ["baitfish"],
    clarity_strengths: ["clear", "stained"],
    condition_tags: ["wind_reaction", "open_water_search", "current_swing"],
    goal_tags: ["reliable_action", "versatile_search"],
    species_allowed: ["smallmouth_bass", "trout", "northern_pike"],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Cast and retrieve at a steady clip just fast enough to keep the blade spinning; vary depth with rod angle and speed to find the feeding zone.",
      "Cast parallel to cover edges and retrieve with a steady pace that keeps the blade thumping; slow a half-beat near cover, then speed back up once it clears.",
      "Cast across current and let the blade flash on a quarter-downstream retrieve; speed up briefly after bumps or short strikes.",
    ],
  }),
  lure({
    id: "spinnerbait",
    display_name: "Spinnerbait",
    family_group: "safety_pin_spinner",
    presentation_group: "spinner_vibration",
    column: "mid",
    primary_pace: "medium",
    secondary_pace: "slow",
    forage_tags: ["baitfish", "bluegill_perch"],
    clarity_strengths: ["stained", "dirty"],
    condition_tags: ["wind_reaction", "dirty_vibration", "cover_ambush"],
    goal_tags: ["reliable_action", "versatile_search"],
    species_allowed: [
      "largemouth_bass",
      "smallmouth_bass",
      "northern_pike",
    ],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Slow-roll it along the bottom edge of cover at a steady pace, keeping the blades just ticking; bump any piece of structure and let it rise.",
      "Work it parallel to grass lines or over submerged timber; slow the retrieve near the target and let the head tick the top.",
      "Cast past the target and retrieve through it, varying speed to find the blade thump fish respond to; hesitate briefly at cover edges.",
    ],
  }),
  lure({
    id: "bladed_jig",
    display_name: "Bladed Jig",
    family_group: "bladed_jig",
    presentation_group: "spinner_vibration",
    column: "mid",
    primary_pace: "medium",
    secondary_pace: "fast",
    forage_tags: ["baitfish", "bluegill_perch"],
    clarity_strengths: ["stained", "dirty"],
    condition_tags: [
      "wind_reaction",
      "dirty_vibration",
      "cover_ambush",
      "warming_search",
    ],
    goal_tags: ["reliable_action", "versatile_search"],
    species_allowed: [
      "largemouth_bass",
      "smallmouth_bass",
      "northern_pike",
    ],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Yo-yo the blade: snap the rod so the head kicks and vibrates, then let it pendulum back with a controlled semi-slack fall.",
      "Work it with a lift-fall cadence; pop the rod to make the blade kick and clack, then let it fall on a controlled line — fish typically strike on the fall.",
      "Snap the rod tip up sharply to activate the blade, then lower it on a semi-tight line so the jig falls at a controlled rate; vary the fall speed with rod angle.",
    ],
  }),
  lure({
    id: "paddle_tail_swimbait",
    display_name: "Paddle-Tail Swimbait",
    family_group: "swimbait",
    presentation_group: "swimbait",
    column: "mid",
    primary_pace: "medium",
    secondary_pace: "fast",
    forage_tags: ["baitfish", "bluegill_perch"],
    clarity_strengths: ["clear", "stained", "dirty"],
    condition_tags: ["open_water_search", "warming_search"],
    goal_tags: ["reliable_action", "versatile_search"],
    species_allowed: [
      "largemouth_bass",
      "smallmouth_bass",
      "northern_pike",
    ],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Reel at a steady pace with the rod low so the tail thumps consistently; speed up slightly over bait schools or slow down after a follow.",
      "Swim it at the speed where you feel a steady kick through the rod; use rod angle to change depth without changing retrieve speed.",
      "Slow-roll near the bottom on a consistent retrieve; the paddle thumps best just barely above the substrate, so keep the rod tip down and the pace even.",
    ],
  }),
  lure({
    id: "glidebait",
    display_name: "Glide Bait",
    family_group: "hard_swimbait",
    presentation_group: "glidebait",
    column: "mid",
    primary_pace: "slow",
    secondary_pace: "medium",
    forage_tags: ["baitfish", "bluegill_perch"],
    clarity_strengths: ["clear", "stained"],
    condition_tags: ["clear_subtle", "open_water_search", "cover_ambush"],
    goal_tags: ["big_fish_upside", "high_risk_high_reward"],
    species_allowed: ["largemouth_bass", "smallmouth_bass"],
    water_types_allowed: [L],
    how_to_fish_variants: [
      "Retrieve slowly enough for a wide S-turn, then pause beside cover or a depth change so the bait glides and hangs before the next handle turn.",
      "Use a slow reel-pause cadence: one or two handle turns to make the bait kick sideways, then a full stop so it stalls in the mid column.",
      "Work it around points, grass edges, docks, or isolated cover with slow sweeps and pauses; keep the bait visible and deliberate instead of burning it back.",
    ],
  }),
  lure({
    id: "compact_glidebait",
    display_name: "Compact Glide Bait",
    family_group: "hard_swimbait",
    presentation_group: "glidebait",
    column: "mid",
    primary_pace: "slow",
    secondary_pace: "medium",
    forage_tags: ["baitfish", "bluegill_perch"],
    clarity_strengths: ["clear", "stained"],
    condition_tags: ["clear_subtle", "open_water_search"],
    goal_tags: ["big_fish_upside", "high_risk_high_reward"],
    species_allowed: ["smallmouth_bass"],
    water_types_allowed: [L],
    how_to_fish_variants: [
      "Use slow half-turns and pauses so the smaller glide bait swings wide without overpowering clear-water smallmouth.",
      "Cast across points, boulders, or bait edges and make the bait glide once or twice, then stall it long enough for followers to close.",
      "Keep it visible and deliberate in the mid column; if a fish follows, slow the cadence and let the bait hang before the next sweep.",
    ],
  }),
  lure({
    id: "soft_jerkbait",
    display_name: "Soft Jerkbait",
    family_group: "jerkbait_soft",
    presentation_group: "jerkbait",
    column: "upper",
    primary_pace: "medium",
    secondary_pace: "slow",
    forage_tags: ["baitfish"],
    clarity_strengths: ["clear", "stained"],
    condition_tags: ["clear_subtle", "open_water_search"],
    goal_tags: ["reliable_action", "versatile_search"],
    species_allowed: [
      "largemouth_bass",
      "smallmouth_bass",
      "northern_pike",
    ],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Twitch it with slack in the line so the bait darts and glides erratically; pause and let it sink a few inches between twitches.",
      "Snap the rod sideways so the bait darts left or right, then drop the rod tip so it glides and sinks on a slack line before the next snap.",
      "Work it with a jerk-sink-jerk cadence; keep the line slightly slack between twitches so the bait moves unpredictably, and vary the pause length.",
    ],
  }),
  lure({
    id: "suspending_jerkbait",
    display_name: "Suspending Jerkbait",
    family_group: "jerkbait",
    presentation_group: "jerkbait",
    column: "mid",
    primary_pace: "medium",
    forage_tags: ["baitfish"],
    clarity_strengths: ["clear", "stained"],
    condition_tags: ["clear_subtle", "cold_slow", "wind_reaction"],
    goal_tags: ["reliable_action", "versatile_search"],
    species_allowed: ["largemouth_bass", "smallmouth_bass", "trout"],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Two or three sharp twitches, then let it suspend perfectly still for a full 3-5 count before the next sequence — the pause is everything.",
      "Jerk, jerk, pause. The bait should dart and glide sideways on each twitch; dial the pause length to water temperature — colder means longer.",
      "Work it with wrist snaps, not big sweeps; keep slack in the line between snaps so the bait swings freely, then pause until you see a follow or feel weight.",
    ],
  }),
  lure({
    id: "magnum_jerkbait",
    display_name: "Magnum Jerkbait",
    family_group: "jerkbait",
    presentation_group: "jerkbait",
    column: "mid",
    primary_pace: "medium",
    secondary_pace: "slow",
    forage_tags: ["baitfish"],
    clarity_strengths: ["clear", "stained"],
    condition_tags: ["clear_subtle", "cold_slow", "open_water_search"],
    goal_tags: ["big_fish_upside", "high_risk_high_reward"],
    species_allowed: ["largemouth_bass", "smallmouth_bass"],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Work it with sharp pulls and longer pauses than a standard jerkbait; the larger body should suspend and hang where a better fish can track it.",
      "Use a jerk-jerk-pause cadence over points, seams, or outside cover, then extend the pause whenever the water is clear or cold.",
      "Sweep it beside bait or structure and kill it completely after each burst; the bigger profile is the draw, so let the stop sell the strike.",
    ],
  }),
  lure({
    id: "squarebill_crankbait",
    display_name: "Squarebill Crankbait",
    family_group: "crankbait_shallow",
    presentation_group: "crankbait",
    column: "upper",
    primary_pace: "medium",
    secondary_pace: "fast",
    forage_tags: ["baitfish", "bluegill_perch"],
    clarity_strengths: ["stained", "dirty"],
    condition_tags: ["cover_ambush", "dirty_vibration", "wind_reaction"],
    goal_tags: ["versatile_search"],
    species_allowed: ["largemouth_bass", "smallmouth_bass", "northern_pike"],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Bang it directly into wood, rock, or stumps so the squared bill deflects the bait sideways — reaction strikes happen on that ricochet.",
      "Cast past structure and crank it down, then steer it right into cover; the deflection is the trigger, so don't try to avoid contact.",
      "Run it tight to cover at a steady pace; when it hits, give a slight rod pause so the bill pries free and the bait kicks up erratically.",
    ],
  }),
  lure({
    id: "flat_sided_crankbait",
    display_name: "Flat-Sided Crankbait",
    family_group: "crankbait_shallow",
    presentation_group: "crankbait",
    column: "upper",
    primary_pace: "medium",
    forage_tags: ["baitfish"],
    clarity_strengths: ["clear", "stained"],
    condition_tags: ["clear_subtle", "cold_slow"],
    goal_tags: ["reliable_action"],
    species_allowed: [
      "largemouth_bass",
      "smallmouth_bass",
      "northern_pike",
    ],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Reel at a slow-to-medium pace; the tight wobble transmits through the line — slow down when you feel the bait tick structure.",
      "Use a steady retrieve with subtle rod-tip pumps to vary the bait's depth and roll; works along transitional depths and current seams.",
      "Work it across hard bottom or rock piles with a consistent slow crank, letting the tight shimmy attract finicky fish holding close to structure.",
    ],
  }),
  lure({
    id: "medium_diving_crankbait",
    display_name: "Medium-Diving Crankbait",
    family_group: "crankbait_medium",
    presentation_group: "crankbait",
    column: "mid",
    primary_pace: "medium",
    forage_tags: ["baitfish", "crawfish"],
    clarity_strengths: ["clear", "stained", "dirty"],
    condition_tags: ["wind_reaction", "open_water_search", "warming_search"],
    goal_tags: ["versatile_search"],
    species_allowed: ["largemouth_bass", "smallmouth_bass"],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Crank it to depth and keep a steady grind-and-stop retrieve; let it tick bottom intermittently to kick up sediment and trigger strikes.",
      "Retrieve at a medium pace along depth contours, pausing after any bottom contact so the bait rises before diving back when you resume.",
      "Work it through mid-column structure at steady speed, varying rod angle to bump the lip on rock ledges or submerged debris.",
    ],
  }),
  lure({
    id: "deep_diving_crankbait",
    display_name: "Deep-Diving Crankbait",
    family_group: "crankbait_deep",
    presentation_group: "crankbait",
    column: "bottom",
    primary_pace: "medium",
    forage_tags: ["baitfish", "crawfish"],
    clarity_strengths: ["clear", "stained", "dirty"],
    condition_tags: ["open_water_search", "cold_slow"],
    goal_tags: ["versatile_search"],
    species_allowed: ["largemouth_bass", "smallmouth_bass", "northern_pike"],
    water_types_allowed: [L],
    how_to_fish_variants: [
      "Make a long cast to reach depth, crank down hard, then keep a slow but steady pace so the bait stays near the bottom; feel for any change in resistance.",
      "Run it along drop-offs and ledges, keeping the line angle shallow so the lip catches bottom occasionally; pause on contact and let it rise.",
      "Long cast, fast crank to get it down, then steady-slow retrieve; bump along bottom transitions and slow down after any tick or thump.",
    ],
  }),
  lure({
    id: "lipless_crankbait",
    display_name: "Lipless Crankbait",
    family_group: "crankbait_lipless",
    presentation_group: "crankbait",
    column: "mid",
    primary_pace: "medium",
    secondary_pace: "fast",
    forage_tags: ["baitfish"],
    clarity_strengths: ["stained", "dirty"],
    condition_tags: ["wind_reaction", "open_water_search"],
    goal_tags: ["versatile_search"],
    species_allowed: [
      "largemouth_bass",
      "smallmouth_bass",
      "northern_pike",
    ],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Yo-yo it through open water: rip up sharply, let it flutter and sink on a semi-slack line, then rip again; vary fall depth by watching the line angle.",
      "Burn it at mid-speed and occasionally pop the rod tip to make the bait jump; over vegetation, rip it free when it ticks the top.",
      "Slow-roll it through grass just above the canopy, and when it hangs up, rip sharply to tear free — that trigger almost always draws a strike.",
    ],
  }),
  lure({
    id: "blade_bait",
    display_name: "Blade Bait",
    family_group: "blade_bait",
    presentation_group: "blade_spoon",
    column: "bottom",
    primary_pace: "slow",
    secondary_pace: "medium",
    forage_tags: ["baitfish"],
    clarity_strengths: ["clear", "stained", "dirty"],
    condition_tags: ["cold_slow", "open_water_search", "current_swing"],
    goal_tags: ["reliable_action"],
    species_allowed: [
      "largemouth_bass",
      "smallmouth_bass",
      "northern_pike",
      "trout",
    ],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Lift with the rod and let the blade flutter back down on a controlled slack line; watch for the tap on the fall and set on anything that interrupts the sink.",
      "Vertical jig it with short, sharp hops over suspended fish; the flutter-fall is the presentation — set hard the moment you feel weight.",
      "Cast and hop it off bottom with quick wrist snaps, counting the flutter back down; strikes usually come on the fall, so stay in contact with semi-tight line.",
    ],
  }),
  lure({
    id: "casting_spoon",
    display_name: "Casting Spoon",
    family_group: "spoon",
    presentation_group: "blade_spoon",
    column: "mid",
    primary_pace: "medium",
    forage_tags: ["baitfish"],
    clarity_strengths: ["clear", "stained"],
    condition_tags: [
      "open_water_search",
      "wind_reaction",
      "cold_slow",
      "current_swing",
    ],
    goal_tags: ["versatile_search"],
    species_allowed: ["northern_pike", "trout"],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Cast, let it flutter to depth, then reel with a rhythmic rise-and-fall so the flashing face covers the full water column; vary sink time to find the depth.",
      "Work it with a lift-and-drop over suspended fish; the concave face spins as it falls — strikes are usually on the drop, so keep a semi-tight line.",
      "Retrieve with a wobbly, wandering cadence by mixing speed; bump into cover and let it flutter down beside structure on the pause.",
    ],
  }),
  lure({
    id: "small_floating_trout_plug",
    display_name: "Floating Trout Plug",
    family_group: "trout_plug",
    presentation_group: "trout_surface_plug",
    column: "surface",
    primary_pace: "medium",
    secondary_pace: "slow",
    forage_tags: ["surface_prey", "baitfish"],
    clarity_strengths: ["clear", "stained"],
    condition_tags: ["calm_surface", "low_light_surface"],
    goal_tags: ["big_fish_upside", "high_risk_high_reward"],
    species_allowed: ["trout"],
    water_types_allowed: [R],
    how_to_fish_variants: [
      "Cast upstream or quartering across, let the plug drift tight to current edges, then twitch just enough to make it wake and dive an inch before floating back up.",
      "Work it along undercut banks and soft seams with short rod-tip taps; pause between twitches so the bait rises like a stunned minnow or terrestrial struggling on top.",
      "Drift it through pocket water on a semi-tight line, adding small pulls to make it wobble, then let it float naturally through the next lane before recasting.",
    ],
  }),
  lure({
    id: "walking_topwater",
    display_name: "Walking Bait",
    family_group: "surface_walking",
    presentation_group: "topwater_open",
    column: "surface",
    primary_pace: "medium",
    forage_tags: ["surface_prey", "baitfish"],
    clarity_strengths: ["clear", "stained"],
    condition_tags: ["calm_surface", "low_light_surface"],
    goal_tags: ["big_fish_upside", "high_risk_high_reward"],
    species_allowed: ["largemouth_bass", "smallmouth_bass", "northern_pike"],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Sweep the rod tip side to side in short, rhythmic twitches while reeling in just enough slack — the bait should walk in a steady side-to-side waddle.",
      "Downrod with light wrist flicks to make it dance; the pause between twitches lets fish track and commit, so give it a beat before the next twitch.",
      "Work it with consistent wrist flicks over open water or above grass; keep the cadence smooth and only slow down if you see fish following.",
    ],
  }),
  lure({
    id: "popping_topwater",
    display_name: "Topwater Popper",
    family_group: "surface_popper",
    presentation_group: "topwater_open",
    column: "surface",
    primary_pace: "medium",
    secondary_pace: "slow",
    forage_tags: ["surface_prey"],
    clarity_strengths: ["clear", "stained"],
    condition_tags: ["calm_surface", "low_light_surface"],
    goal_tags: ["reliable_action", "high_risk_high_reward"],
    species_allowed: ["largemouth_bass", "smallmouth_bass"],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Pop sharply with the rod tip, let the rings settle, then pop again — give it a 2-3 second pause after each spit so fish can locate and attack.",
      "Use rhythmic 1-2 pops followed by a pause; target the shadow of any surface feature and let the bait sit still in the ring for a count of three.",
      "Short sharp pops followed by deliberate pauses near cover edges; slower and louder pops in flat calm, quicker pops when fish are aggressive.",
    ],
  }),
  lure({
    id: "buzzbait",
    display_name: "Buzzbait",
    family_group: "surface_buzz",
    presentation_group: "topwater_open",
    column: "surface",
    primary_pace: "fast",
    secondary_pace: "medium",
    forage_tags: ["surface_prey", "baitfish"],
    clarity_strengths: ["stained", "dirty"],
    condition_tags: ["low_light_surface", "wind_reaction", "dirty_vibration"],
    goal_tags: ["big_fish_upside", "high_risk_high_reward"],
    species_allowed: ["largemouth_bass", "smallmouth_bass", "northern_pike"],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Start the retrieve as soon as it hits the water; keep the blade just breaking the surface and never stop reeling until the bait is at the boat.",
      "Burn it across the surface on a constant retrieve tight to cover edges; vary speed only to keep it on top, and set hard when you feel weight.",
      "Run it on a straight, steady retrieve through open pockets in cover; pause only if a fish swipes and misses — then hold still briefly and resume.",
    ],
  }),
  lure({
    id: "prop_bait",
    display_name: "Prop Bait",
    family_group: "surface_prop",
    presentation_group: "topwater_open",
    column: "surface",
    primary_pace: "medium",
    forage_tags: ["surface_prey"],
    clarity_strengths: ["clear", "stained"],
    condition_tags: ["calm_surface", "low_light_surface"],
    goal_tags: ["big_fish_upside", "high_risk_high_reward"],
    species_allowed: ["largemouth_bass", "smallmouth_bass"],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Twitch it just enough to make the props sputter and spit, then let it sit still until the rings disappear before moving it again.",
      "Use a pull-pause cadence so the prop churns on the pull and goes dead still on the pause; fish often eat it after the commotion stops.",
      "Work it around isolated cover with sharp little pops of the rod tip, letting the bait rest between sputters instead of reeling it straight back.",
    ],
  }),
  lure({
    id: "hollow_body_frog",
    display_name: "Hollow-Body Frog",
    family_group: "surface_frog",
    presentation_group: "topwater_frog",
    column: "surface",
    primary_pace: "slow",
    secondary_pace: "medium",
    forage_tags: ["surface_prey", "bluegill_perch"],
    /** Same rationale as `frog_fly`: hollow frogs shine in stain, but clear vegetated lakes are honest frog water. */
    clarity_strengths: ["clear", "stained", "dirty"],
    condition_tags: ["calm_surface", "low_light_surface", "cover_ambush"],
    goal_tags: ["big_fish_upside", "high_risk_high_reward"],
    species_allowed: ["largemouth_bass", "northern_pike"],
    water_types_allowed: [L],
    how_to_fish_variants: [
      "Walk it across mats or over lily pads with short side-to-side twitches; when it enters a gap or open pocket, let it sit for two full seconds before moving.",
      "Twitch it just enough to make the legs kick; pause over every opening in the cover and count to three before moving — strikes on mats come after the pause.",
      "Use steady walking cadence across open surface, then slow and pause aggressively when near cover; don't set the hook until you feel the fish pulling.",
    ],
  }),
  lure({
    id: "wake_bait",
    display_name: "Wake Bait",
    family_group: "surface_wake",
    presentation_group: "topwater_open",
    column: "surface",
    primary_pace: "slow",
    secondary_pace: "medium",
    forage_tags: ["surface_prey", "baitfish", "bluegill_perch"],
    clarity_strengths: ["clear", "stained"],
    condition_tags: ["calm_surface", "low_light_surface"],
    goal_tags: ["big_fish_upside", "high_risk_high_reward"],
    species_allowed: ["largemouth_bass", "smallmouth_bass"],
    water_types_allowed: [L],
    how_to_fish_variants: [
      "Crawl it slowly enough to bulge the surface and leave a steady V-wake; keep the bait coming over points, grass, or shade without twitching it away.",
      "Make long casts in low light and crawl the bait over shallow cover; pause only briefly so the wake restarts cleanly when you turn the handle again.",
      "Run it parallel to banks, docks, or weed edges with a slow constant retrieve; the quiet surface push is the trigger, not splash.",
    ],
  }),
  lure({
    id: "pike_spinnerbait",
    display_name: "Oversized Spinnerbait",
    family_group: "pike_spinnerbait",
    presentation_group: "spinner_vibration",
    column: "mid",
    primary_pace: "medium",
    secondary_pace: "fast",
    forage_tags: ["baitfish", "bluegill_perch"],
    clarity_strengths: ["stained", "dirty"],
    condition_tags: ["wind_reaction", "dirty_vibration", "cover_ambush"],
    goal_tags: ["reliable_action", "big_fish_upside"],
    species_allowed: ["northern_pike"],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Slow-roll the oversized frame along weed edges, laydowns, or current breaks so the blades thump and the skirt pulses without riding too high.",
      "Cast past cover and retrieve through it at a steady pace; speed up after the bait clears the target to trigger following pike.",
      "Use a medium retrieve in wind or stain, ticking cover when possible and keeping the bait moving so the blades advertise a pike-sized meal.",
    ],
  }),
  lure({
    id: "weedless_spoon",
    display_name: "Weedless Spoon",
    family_group: "spoon",
    presentation_group: "blade_spoon",
    column: "upper",
    primary_pace: "medium",
    secondary_pace: "slow",
    forage_tags: ["baitfish", "bluegill_perch"],
    clarity_strengths: ["clear", "stained", "dirty"],
    condition_tags: ["cover_ambush", "wind_reaction"],
    goal_tags: ["reliable_action", "big_fish_upside"],
    species_allowed: ["northern_pike"],
    water_types_allowed: [L],
    how_to_fish_variants: [
      "Swim it over and through grass, pads, or shallow cabbage in the upper-to-mid lane with a steady wobble, then pause beside openings so it flutters without burying.",
      "Cast across weed flats and keep the spoon ticking the tops or open lanes; when it catches, lift gently and let it flutter free instead of ripping it hard.",
      "Run it through shallow-to-mid cover at a controlled pace where the spoon rolls and flashes but the weed guard keeps it clean.",
    ],
  }),
  lure({
    id: "shallow_minnowbait",
    display_name: "Shallow Twitchbait",
    family_group: "pike_minnowbait",
    presentation_group: "jerkbait",
    column: "upper",
    primary_pace: "medium",
    secondary_pace: "slow",
    forage_tags: ["baitfish"],
    clarity_strengths: ["clear", "stained"],
    condition_tags: ["open_water_search", "warming_search"],
    goal_tags: ["reliable_action", "versatile_search"],
    species_allowed: ["northern_pike"],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Retrieve the shallow twitchbait over weeds or current edges with steady pulls and pauses so it rolls through the upper-to-mid lane.",
      "Twitch it around shallow cover, then let it float up or hang for a beat before the next pull; pike often commit as it stalls above the cover.",
      "Cover water with a medium retrieve and occasional rod taps, keeping the bait above snags while still showing a wounded-minnow roll in the top few feet.",
    ],
  }),
  lure({
    id: "pike_glidebait",
    display_name: "Large Glide Bait",
    family_group: "pike_glidebait",
    presentation_group: "glidebait",
    column: "mid",
    primary_pace: "slow",
    secondary_pace: "medium",
    forage_tags: ["baitfish", "bluegill_perch"],
    clarity_strengths: ["clear", "stained"],
    condition_tags: ["open_water_search", "clear_subtle"],
    goal_tags: ["big_fish_upside", "high_risk_high_reward"],
    species_allowed: ["northern_pike"],
    water_types_allowed: [L],
    how_to_fish_variants: [
      "Work the big glide bait with slow sweeps over weed edges or points, letting it kick sideways and pause in the mid column.",
      "Make deliberate turns of the handle, then stop so the bait hangs broadside; give following pike time to close before moving it again.",
      "Fish it around clear or lightly stained ambush lanes with a patient sweep-pause rhythm instead of burning it back.",
    ],
  }),
  lure({
    id: "large_profile_pike_swimbait",
    display_name: "Large Paddle-Tail Swimbait",
    family_group: "pike_swimbait",
    presentation_group: "swimbait",
    column: "mid",
    primary_pace: "medium",
    secondary_pace: "slow",
    forage_tags: ["baitfish", "bluegill_perch"],
    clarity_strengths: ["clear", "stained", "dirty"],
    condition_tags: ["open_water_search", "cover_ambush", "warming_search"],
    goal_tags: ["big_fish_upside"],
    species_allowed: ["northern_pike"],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Slow-roll at a steady mid-depth pace; pike follow long before committing, so keep your nerve and don't speed up or stop until the line goes tight.",
      "Work it with long, deliberate sweeps that make the body undulate; pause at the end of each sweep and hold position so following pike can close.",
      "Retrieve at a measured pace over weed tops or along the edge; the big profile does the work — don't twitch or jerk, just keep it gliding.",
    ],
  }),
  lure({
    id: "pike_jerkbait",
    display_name: "Large Jerkbait",
    family_group: "pike_jerkbait",
    presentation_group: "jerkbait",
    column: "mid",
    primary_pace: "medium",
    secondary_pace: "fast",
    forage_tags: ["baitfish"],
    clarity_strengths: ["clear", "stained"],
    condition_tags: ["wind_reaction", "open_water_search", "cold_slow"],
    goal_tags: ["big_fish_upside"],
    species_allowed: ["northern_pike"],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Use long, hard sweeps with the rod to make the bait dart and glide; pause fully between sweeps so the bait hangs and following fish can catch up.",
      "Rip it hard with two or three sweeps, then kill it completely — let the bait sink or suspend in place while you count three before the next sequence.",
      "Work it erratically with big pulls and deadstops; the key is the pause after the dash, so hold still for at least two seconds before triggering again.",
    ],
  }),
  lure({
    id: "large_bucktail_spinner",
    display_name: "Large Bucktail Spinner",
    family_group: "large_spinner",
    presentation_group: "spinner_vibration",
    column: "mid",
    primary_pace: "medium",
    secondary_pace: "fast",
    forage_tags: ["baitfish", "bluegill_perch"],
    clarity_strengths: ["clear", "stained"],
    condition_tags: ["wind_reaction", "dirty_vibration", "open_water_search"],
    goal_tags: ["big_fish_upside", "versatile_search"],
    species_allowed: ["northern_pike"],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Burn it over weed tops or along outside edges just fast enough to keep the big blade thumping; keep reeling through follows until the fish turns away or eats.",
      "Cast past points, cabbage pockets, or current breaks and retrieve with a steady pulse; add one speed-up beside the target to trigger a tracking pike.",
      "Run it high enough to stay clean but deep enough to show the whole bucktail profile; finish every cast with a turn or sweep near the boat.",
    ],
  }),
  lure({
    id: "large_pike_topwater",
    display_name: "Large Walking Bait",
    family_group: "large_pike_surface",
    presentation_group: "topwater_open",
    column: "surface",
    primary_pace: "medium",
    secondary_pace: "slow",
    forage_tags: ["surface_prey", "baitfish"],
    clarity_strengths: ["clear", "stained"],
    condition_tags: ["calm_surface", "low_light_surface"],
    goal_tags: ["big_fish_upside", "high_risk_high_reward"],
    species_allowed: ["northern_pike"],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Walk it wide over weed flats, points, or shallow bays with firm rod taps; keep the cadence steady so pike can track the big surface target.",
      "Use a slow walk-pause rhythm and let the bait sit after every few swings; pike often roll under it before committing on the restart.",
      "Make long casts over ambush water and walk it back with deliberate sweeps; if a pike misses, pause, then restart with two sharp twitches.",
    ],
  }),
  lure({
    id: "pike_jig_and_plastic",
    display_name: "Heavy Paddle-Tail Swimbait",
    family_group: "pike_jig",
    presentation_group: "pike_jig",
    column: "bottom",
    primary_pace: "slow",
    secondary_pace: "medium",
    forage_tags: ["baitfish", "bluegill_perch"],
    clarity_strengths: ["stained", "dirty"],
    condition_tags: ["cold_slow", "dirty_vibration"],
    goal_tags: ["big_fish_upside"],
    species_allowed: ["northern_pike"],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Slow-roll the heavy paddle tail just off bottom so the tail thumps steadily; pause when it ticks rock, wood, or weed stems and let it glide back down.",
      "Cast past the edge and count it down, then retrieve with a low rod and steady pressure so it swims deep without turning into a jig hop.",
      "Work it along deep weed lines, current breaks, or rocky edges with a measured retrieve; the big tail kick is the trigger, so keep it swimming and controlled.",
    ],
  }),
  lure({
    id: "large_pike_tube",
    display_name: "Large Tube Jig",
    family_group: "pike_tube",
    presentation_group: "pike_tube",
    column: "bottom",
    primary_pace: "slow",
    secondary_pace: "medium",
    forage_tags: ["baitfish", "bluegill_perch"],
    clarity_strengths: ["clear", "stained", "dirty"],
    condition_tags: ["cold_slow", "current_swing", "cover_ambush"],
    goal_tags: ["big_fish_upside", "reliable_action"],
    species_allowed: ["northern_pike"],
    water_types_allowed: [L, R],
    how_to_fish_variants: [
      "Fish a large tube on a stout jig head with slow bottom contact; drag it through current seams and pause whenever it ticks rock or wood.",
      "Use short lifts and controlled drops so the tube glides and flares near bottom; keep the bait heavy enough to stay down without sweeping it away.",
      "Crawl it along deep bends, current breaks, or sparse cover with long pauses; the big hollow body should look like an easy baitfish-sized meal, not a bass finesse bait.",
    ],
  }),
];

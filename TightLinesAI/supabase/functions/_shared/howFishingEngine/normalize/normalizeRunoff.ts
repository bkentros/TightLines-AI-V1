...
function scaledThresholds(region: RegionKey, month?: number) {
  ...
  const thresholds = baseThresholds(sens);
  let scale = 1;
  if (
    month != null && month >= 4 && month <= 6 &&
    SNOWMELT_RISK_REGIONS.has(region)
  ) {
    scale *= 0.60;
  }
  ...
  return {
    clear: scaleArr(thresholds.clear),
    stable: scaleArr(thresholds.stable),
    slight: scaleArr(thresholds.slight),
    elevated: scaleArr(thresholds.elevated),
  };
}
 ...
 *
 * Public output shape remains unchanged. The optional month argument enables
 * seasonal hydrology sensitivity; legacy callers without it get neutral-season
 * thresholds. Rollback means restoring the previous body without changing
 * callers.
 */
    ...
    return {
      ...
      score: clampEngineScore(
        pieceLinear(
          ...
          1,
          PERFECT_CLEAR_MAX,
          Math.min(0.08, PERFECT_CLEAR_MAX),
        ),
      ),
    };
  }

 ...
 */
export function regionKeyFromLatLon(
  lat: number,
  lon: number,
  stateCode: string | null = resolveStateFromCoords(lat, lon),
): RegionKey | null {
  if (
    !Number.isFinite(lat) || !Number.isFinite(lon) || lat < -90 || lat > 90 ||
    lon < -180 || lon > 180
  ) return null;
  // State polygons prevent the Florida rectangle from capturing south Georgia,
  // and close the inland Deep South gap. Retain existing biological region keys.
  if (stateCode === "FL") return "florida";
  if (stateCode === "GA") return "southeast_atlantic";
  if (stateCode === "AL" || stateCode === "MS" || stateCode === "LA") {
    return lat <= 31.5 ? "gulf_coast" : "south_central";
  }
  if (stateCode === "WV") return "appalachian";
  // Highlands must precede the broad South Central rectangle.
  if (
    (stateCode === "NC" && lat >= 35 && lon <= -81) ||
    (stateCode === "TN" && lon >= -84) ||
    (stateCode === "VA" && lon <= -78)
  ) return "appalachian";
  ...
  if (lat >= 42 && lat <= 49 && lon >= -124.5 && lon <= -120.5) {
  ...
  if (lat >= 31 && lat <= 37 && lon >= -109 && lon <= -103) {
    return "southwest_high_desert";
  }
  ...
  if (lat >= 36 && lat <= 46 && lon >= -120.5 && lon <= -104) {
    return "mountain_west";
  }
  ...
  // An unmatched coordinate must allow the caller's state fallback to run.
  return null;
}

import {
  ...
  type PierCastChicagoAlpenaCityId,
...
const SAMPLE_COUNT = PIER_CAST_CHICAGO_ALPENA_CITY_IDS.length * HOURS_PER_CITY;
...
const locations = new Map(
  PIER_CAST_CHICAGO_ALPENA_CITY_PROFILES.map((profile) => [
    profile.cityId,
    profile.waterTemperatureSource?.configuredLocation,
  ]),
);

export async function archivePierCastChicagoAlpenaLmhofsBatch(
  ...
  if (error) {
    throw new Error(
      error.message?.trim() ||
        "ChicagoAlpena shadow LMHOFS archive commit failed.",
    );
  }
  return {
    issuedAt: batch.issuedAt,
...
export async function readLatestFreshPierCastChicagoAlpenaLmhofsBatch(
  database: PierCastArchiveClient,
  now: Date,
  maxAgeHours = 13,
  ...
  ) throw new Error("ChicagoAlpena archive freshness is invalid.");
  const { data, error } = await database.rpc(
    "read_latest_fresh_pier_cast_chicago_alpena_lmhofs_samples",
    {
  ...
  if (error) {
    throw new Error(
      error.message?.trim() || "ChicagoAlpena shadow archive read failed.",
    );
  }
  if (!Array.isArray(data) || data.length === 0) return null;
  if (data.length !== SAMPLE_COUNT) {
    throw new Error("ChicagoAlpena shadow archive returned a partial cycle.");
  }
  ...
  if (cycleAgeHours < 0 || cycleAgeHours > maxAgeHours) {
    throw new Error("ChicagoAlpena shadow archive returned a stale cycle.");
  }
  const cities = PIER_CAST_CHICAGO_ALPENA_CITY_IDS.map((cityId) => {
    ...
    return {
      ...
      coverageEnd: citySamples[HOURS_PER_CITY - 1].validAt,
      samples: citySamples,
  ...
  return {
    ...
    requestedForecastHours: Array.from(
      { length: HOURS_PER_CITY },
      (_, hour) => hour,
    ),
    cities,
...
function validateSamples(
  ...
  if (samples.length !== SAMPLE_COUNT) {
    throw new Error(
      "ChicagoAlpena LMHOFS archive requires exactly 605 samples.",
    );
  }
  const counts = new Map<PierCastChicagoAlpenaCityId, Set<number>>();
  for (const cityId of PIER_CAST_CHICAGO_ALPENA_CITY_IDS) {
    counts.set(cityId, new Set());
  }
  for (const sample of samples) {
    if (
      !PIER_CAST_CHICAGO_ALPENA_CITY_IDS.includes(
        sample.cityId as PierCastChicagoAlpenaCityId,
      )
    ) {
      throw new Error("ChicagoAlpena LMHOFS sample has an unexpected city.");
    }
    const cityId = sample.cityId as PierCastChicagoAlpenaCityId;
    const location = locations.get(cityId);
    const hours = counts.get(cityId)!;
    if (
      !location || sample.issuedAt !== issuedAt ||
    ...
    ) throw new Error("ChicagoAlpena LMHOFS samples are inconsistent.");
    hours.add(sample.forecastHour);
  }
  if ([...counts.values()].some((hours) => hours.size !== HOURS_PER_CITY)) {
    throw new Error(
      "ChicagoAlpena LMHOFS archive requires 121 unique hours per city.",
    );
  }
}

function parseSample(row: Record<string, unknown>): PierCastLmhofsSample {
  const cityId = String(row.city_id) as PierCastChicagoAlpenaCityId;
  ...
  ) throw new Error("ChicagoAlpena archive returned an invalid sample.");
  return sample;
}

function timestamp(value: unknown, field: string): string {
  if (typeof value !== "string" || !Number.isFinite(Date.parse(value))) {
    throw new Error(`ChicagoAlpena archive ${field} is invalid.`);
  }

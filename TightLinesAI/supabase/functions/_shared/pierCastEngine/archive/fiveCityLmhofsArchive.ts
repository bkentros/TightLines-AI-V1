import {
  ...
  type PierCastFiveCityId,
} from "../config/fiveCityShadow.ts";
import type {
  PierCastLmhofsBatch,
...
const SAMPLE_COUNT = PIER_CAST_FIVE_CITY_IDS.length * HOURS_PER_CITY;
...
const locations = new Map(
  PIER_CAST_FIVE_CITY_PROFILES.map((profile) => [
    profile.cityId,
    profile.waterTemperatureSource?.configuredLocation,
  ]),
);

export async function archivePierCastFiveCityLmhofsBatch(
  ...
  if (error) {
    throw new Error(
      error.message?.trim() || "FiveCity shadow LMHOFS archive commit failed.",
    );
  }
  return {
    issuedAt: batch.issuedAt,
...
export async function readLatestFreshPierCastFiveCityLmhofsBatch(
  database: PierCastArchiveClient,
  now: Date,
  maxAgeHours = 13,
  ...
  ) throw new Error("FiveCity archive freshness is invalid.");
  const { data, error } = await database.rpc(
    "read_latest_fresh_pier_cast_five_city_lmhofs_samples",
    {
  ...
  if (error) {
    throw new Error(
      error.message?.trim() || "FiveCity shadow archive read failed.",
    );
  }
  if (!Array.isArray(data) || data.length === 0) return null;
  if (data.length !== SAMPLE_COUNT) {
    throw new Error("FiveCity shadow archive returned a partial cycle.");
  }
  ...
  if (cycleAgeHours < 0 || cycleAgeHours > maxAgeHours) {
    throw new Error("FiveCity shadow archive returned a stale cycle.");
  }
  const cities = PIER_CAST_FIVE_CITY_IDS.map((cityId) => {
...
function validateSamples(
  ...
  if (samples.length !== SAMPLE_COUNT) {
    throw new Error("FiveCity LMHOFS archive requires exactly 605 samples.");
  }
  const counts = new Map<PierCastFiveCityId, Set<number>>();
  for (const cityId of PIER_CAST_FIVE_CITY_IDS) counts.set(cityId, new Set());
  for (const sample of samples) {
    if (
      !PIER_CAST_FIVE_CITY_IDS.includes(sample.cityId as PierCastFiveCityId)
    ) {
      throw new Error("FiveCity LMHOFS sample has an unexpected city.");
    }
    const cityId = sample.cityId as PierCastFiveCityId;
    ...
    ) throw new Error("FiveCity LMHOFS samples are inconsistent.");
    hours.add(sample.forecastHour);
  }
  if ([...counts.values()].some((hours) => hours.size !== HOURS_PER_CITY)) {
    throw new Error(
      "FiveCity LMHOFS archive requires 121 unique hours per city.",
    );
  }
}

function parseSample(row: Record<string, unknown>): PierCastLmhofsSample {
  const cityId = String(row.city_id) as PierCastFiveCityId;
  ...
  ) throw new Error("FiveCity archive returned an invalid sample.");
  return sample;
}

function timestamp(value: unknown, field: string): string {
  if (typeof value !== "string" || !Number.isFinite(Date.parse(value))) {
    throw new Error(`FiveCity archive ${field} is invalid.`);
  }

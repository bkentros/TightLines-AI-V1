...
} from "../archive/pentwaterCasevilleLmhofsArchive.ts";
import {
...
export async function ingestPierCastPentwaterCasevilleShadowCycle(input: {
  ...
  try {
    live = input.operations?.fetchLive
      ...
      : await fetchPierCastLmhofsBatch({
        ...
        now: () => now,
        cityProfiles: PIER_CAST_PENTWATER_CASEVILLE_CITY_PROFILES,
      });
  } catch (error) {
  ...
  if (complete(live)) {
    try {
      ...
      } else {await archivePierCastPentwaterCasevilleLmhofsBatch(
          input.database,
          live,
          input.engineVersion,
        );}
  ...
  try {
    const cached = input.operations?.readFreshArchive
      ...
      : await readLatestFreshPierCastPentwaterCasevilleLmhofsBatch(
        input.database,
        now,
      );
    if (cached) {
...
function complete(batch: PierCastLmhofsBatch | null): batch is AvailableBatch {
  return batch?.status === "available" && batch.fullHorizonRequested &&
    ...
    PIER_CAST_PENTWATER_CASEVILLE_CITY_IDS.every((cityId) => {

...
import {
  archivePierCastFiveCityLmhofsBatch,
  readLatestFreshPierCastFiveCityLmhofsBatch,
} from "../archive/fiveCityLmhofsArchive.ts";
import {
...
export async function ingestPierCastFiveCityShadowCycle(input: {
  ...
  try {
    live = input.operations?.fetchLive
      ...
      : await fetchPierCastLmhofsBatch({
        ...
        now: () => now,
        cityProfiles: PIER_CAST_FIVE_CITY_PROFILES,
      });
  } catch (error) {
  ...
  if (complete(live)) {
    try {
      if (input.operations?.archiveLive) {
        ...
        await archivePierCastFiveCityLmhofsBatch(
          input.database,
          live,
          input.engineVersion,
        );
  ...
  try {
    const cached = input.operations?.readFreshArchive
      ...
      : await readLatestFreshPierCastFiveCityLmhofsBatch(input.database, now);
    if (cached) {
      return {
...
function complete(batch: PierCastLmhofsBatch | null): batch is AvailableBatch {
  return batch?.status === "available" && batch.fullHorizonRequested &&
    ...
    PIER_CAST_FIVE_CITY_IDS.every((cityId) => {

// This file is generated by scripts/generate-river-run-review-fixtures.ts.
// Do not hand-edit. Every primitive card below comes from production scoring code.

import type { RiverRunReviewGroup } from "./riverRunReviewFixtures.types";
import type { RiverRunSnapshotResponse } from "./riverRunContracts";

const BASE_SNAPSHOT = {
  "riverId": "pere_marquette",
  "runId": "pere_marquette_fall_steelhead",
  "localDate": "2026-10-15",
  "timezone": "America/Detroit",
  "progressionSnapshotAt": "2026-10-15T12:00:00.000Z",
  "conditionRefreshAt": "2026-10-15T20:00:00.000Z",
  "refreshSlot": "16:00",
  "progressionExpiresAt": "2026-10-15T23:59:59.000Z",
  "nextConditionRefreshAt": "2026-10-15T22:00:00.000Z",
  "runStage": {
    "stage": "building",
    "copyStrategy": "pere_marquette",
    "stagingContext": false,
    "broadBuildingContext": false,
    "winterHoldingContext": false,
    "window": {
      "snapshotDate": "2026-10-15",
      "preRunStartDate": "2026-08-15",
      "stagingStartDate": "2026-09-01",
      "startDate": "2026-09-20",
      "beginningEndDate": "2026-10-10",
      "buildingEstablishedStartDate": "2026-10-15",
      "buildingBroadStartDate": "2026-11-01",
      "peakStartDate": "2026-11-15",
      "peakDate": "2026-11-15",
      "peakEndDate": "2026-12-04",
      "taperingEndDate": "2026-12-19",
      "endDate": "2026-12-22",
      "lateEndDate": "2026-12-23",
      "postRunLateCopyEndDate": "2026-12-24",
      "startToPeakDays": 56,
      "peakToEndDays": 37
    },
    "label": "Building",
    "headline": "The PM Steelhead run is becoming established in the Middle river.",
    "detail": "Seasonal timing now favors the Middle river. The Upper river remains a conditional secondary choice.",
    "tip": "Begin in the Middle river. Add the Upper river only after direct fish activity supports it.",
    "whereToStart": "Start in the Middle river (Scottville–Maple Leaf). Add the Upper river (Maple Leaf–M-37) only after direct fish activity supports it.",
    "reasonCodes": [
      "stage_building"
    ],
    "copyVersion": "river-run-copy-v35"
  },
  "conditionsSuggest": {
    "label": "Ahead",
    "timingLabel": "Ahead",
    "candidateLabel": "Ahead",
    "checkpointId": "building_established",
    "checkpointDate": "2026-10-15",
    "cutoffDate": "2026-10-14",
    "observationStartDate": "2026-09-01",
    "nextCheckpointDate": "2026-11-15",
    "completedCheckpointCount": 3,
    "currentIndex": 88,
    "currentPercentile": 80,
    "gaugeResponsePercentile": 100,
    "waterTemperaturePercentile": 80,
    "usableDays": 44,
    "expectedDays": 44,
    "coveragePercent": 1,
    "historicalYears": 5,
    "baselineVersion": "pm-fall-steelhead-conditions-v2",
    "gaugeSiteId": "04122500",
    "temperatureSourceId": "pm_m37_temperature",
    "sourceDates": [
      "2026-09-01",
      "2026-09-02",
      "2026-09-03",
      "2026-09-04",
      "2026-09-05",
      "2026-09-06",
      "2026-09-07",
      "2026-09-08",
      "2026-09-09",
      "2026-09-10",
      "2026-09-11",
      "2026-09-12",
      "2026-09-13",
      "2026-09-14",
      "2026-09-15",
      "2026-09-16",
      "2026-09-17",
      "2026-09-18",
      "2026-09-19",
      "2026-09-20",
      "2026-09-21",
      "2026-09-22",
      "2026-09-23",
      "2026-09-24",
      "2026-09-25",
      "2026-09-26",
      "2026-09-27",
      "2026-09-28",
      "2026-09-29",
      "2026-09-30",
      "2026-10-01",
      "2026-10-02",
      "2026-10-03",
      "2026-10-04",
      "2026-10-05",
      "2026-10-06",
      "2026-10-07",
      "2026-10-08",
      "2026-10-09",
      "2026-10-10",
      "2026-10-11",
      "2026-10-12",
      "2026-10-13",
      "2026-10-14"
    ],
    "sourceRefreshSlots": {
      "2026-09-01": "16:00",
      "2026-09-02": "16:00",
      "2026-09-03": "16:00",
      "2026-09-04": "16:00",
      "2026-09-05": "16:00",
      "2026-09-06": "16:00",
      "2026-09-07": "16:00",
      "2026-09-08": "16:00",
      "2026-09-09": "16:00",
      "2026-09-10": "16:00",
      "2026-09-11": "16:00",
      "2026-09-12": "16:00",
      "2026-09-13": "16:00",
      "2026-09-14": "16:00",
      "2026-09-15": "16:00",
      "2026-09-16": "16:00",
      "2026-09-17": "16:00",
      "2026-09-18": "16:00",
      "2026-09-19": "16:00",
      "2026-09-20": "16:00",
      "2026-09-21": "16:00",
      "2026-09-22": "16:00",
      "2026-09-23": "16:00",
      "2026-09-24": "16:00",
      "2026-09-25": "16:00",
      "2026-09-26": "16:00",
      "2026-09-27": "16:00",
      "2026-09-28": "16:00",
      "2026-09-29": "16:00",
      "2026-09-30": "16:00",
      "2026-10-01": "16:00",
      "2026-10-02": "16:00",
      "2026-10-03": "16:00",
      "2026-10-04": "16:00",
      "2026-10-05": "16:00",
      "2026-10-06": "16:00",
      "2026-10-07": "16:00",
      "2026-10-08": "16:00",
      "2026-10-09": "16:00",
      "2026-10-10": "16:00",
      "2026-10-11": "16:00",
      "2026-10-12": "16:00",
      "2026-10-13": "16:00",
      "2026-10-14": "16:00"
    },
    "headline": "The PM migration is developing earlier than usual.",
    "detail": "Season-to-date Scottville river-rise activity and M-37 cooling are stronger than the usual PM pattern.",
    "tip": "Start one PM section upstream from Migration Stage, capped at the Upper river (Maple Leaf–M-37).",
    "reasonCodes": [
      "gauge_fresh",
      "temperature_measured",
      "conditions_checkpoint_building_established",
      "conditions_ahead"
    ],
    "copyVersion": "river-run-copy-v35",
    "previousCheckpointId": "building_start",
    "previousCheckpointDate": "2026-10-11",
    "previousTimingLabel": "Ahead"
  },
  "push": {
    "score": 35,
    "label": "No clear push",
    "headline": "PM water does not show a clear fresh-movement signal.",
    "detail": "Scottville flow is steady without a meaningful rise. Measured water temperature is favorable for this migration. Recent watershed rainfall is too light to affect the read.",
    "tip": "Keep the section named by Migration Stage. Do not shift lower for a fresh wave this water does not support.",
    "reasonCodes": [
      "gauge_fresh",
      "push_normal_flow_context",
      "push_temperature_supportive",
      "light_rain_48h",
      "push_no_gauge_response_cap"
    ],
    "components": {
      "hydraulicBase": 35,
      "hydraulicAdjustment": 0,
      "temperatureModifier": 0,
      "rainModifier": 0,
      "hydraulicState": "normal",
      "temperatureState": "supportive",
      "rainRole": "neutral",
      "appliedCaps": []
    },
    "rulesVersion": "pm-fall-steelhead-push-v1",
    "copyVersion": "river-run-copy-v35"
  },
  "pushHistory": {
    "status": "none_recorded",
    "minimumSupportiveScore": 50,
    "trackingStartDate": "2026-09-20",
    "trackingEndDate": "2026-12-22",
    "throughDate": "2026-10-15",
    "recentDailyReadsStatus": "available",
    "recentDailyReads": [
      {
        "localDate": "2026-10-14",
        "status": "no_supportive_window",
        "score": null,
        "label": "No supportive window"
      },
      {
        "localDate": "2026-10-13",
        "status": "no_supportive_window",
        "score": null,
        "label": "No supportive window"
      },
      {
        "localDate": "2026-10-12",
        "status": "supportive_window",
        "refreshSlot": "08:00",
        "conditionRefreshAt": "2026-10-12T23:59:00.000Z",
        "score": 63,
        "label": "Possible"
      },
      {
        "localDate": "2026-10-11",
        "status": "no_supportive_window",
        "score": null,
        "label": "No supportive window"
      },
      {
        "localDate": "2026-10-10",
        "status": "supportive_window",
        "refreshSlot": "20:00",
        "conditionRefreshAt": "2026-10-10T23:59:00.000Z",
        "score": 81,
        "label": "Strong"
      },
      {
        "localDate": "2026-10-09",
        "status": "supportive_window",
        "refreshSlot": "16:00",
        "conditionRefreshAt": "2026-10-09T23:59:00.000Z",
        "score": 63,
        "label": "Possible"
      },
      {
        "localDate": "2026-10-08",
        "status": "no_supportive_window",
        "score": null,
        "label": "No supportive window"
      }
    ]
  },
  "fishability": {
    "score": 93,
    "label": "Excellent",
    "headline": "Scottville flow is in its best presentation range.",
    "detail": "The flow band offers the broadest mix of depth, seams, and presentation control. Stable flow should keep presentation lanes consistent. This read applies to the Lower river (Pere Marquette Lake–Scottville), not the full PM.",
    "tip": "Cover readable seams and holding water with a controlled presentation.",
    "reasonCodes": [
      "gauge_fresh",
      "ideal_flow_band"
    ],
    "components": {
      "bandBase": 88,
      "trendModifier": 5,
      "appliedCaps": []
    },
    "rulesVersion": "pm-scottville-fishability-v2",
    "copyVersion": "river-run-copy-v35"
  },
  "activity": null,
  "fishInRiver": {
    "score": 36,
    "displayScore": 35,
    "scoreIsApproximate": true,
    "stage": "building",
    "maximum": 100,
    "riverCeiling": 80,
    "historicalRunStrength": "strong",
    "curveFraction": 0.45,
    "curveDirection": "rising",
    "winterHoldingContext": false,
    "label": "Moderate presence",
    "headline": "Steelhead seasonal presence is moderate presence and building.",
    "detail": "Calendar timing places Steelhead presence at moderate presence, increasing as the fall migration builds. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
    "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
    "reasonCodes": [
      "stage_building",
      "historical_presence_curve"
    ],
    "copyVersion": "river-run-copy-v35"
  },
  "gauge": {
    "provider": "USGS",
    "siteId": "04122500",
    "primaryMetric": "flow_cfs",
    "observedAt": "2026-10-15T19:45:00.000Z",
    "value": 600,
    "band": "ideal",
    "trend": "stable",
    "absoluteChange24h": 0,
    "percentChange24h": 0
  },
  "weather": {
    "provider": "OPEN_METEO",
    "evidenceType": "modeled_grid",
    "weatherPointId": "pere_marquette_primary",
    "rain24hIn": 0.1,
    "rain48hIn": 0.2,
    "rain72hIn": 0.3
  },
  "waterTemperature": {
    "provider": "MONITOR_MY_WATERSHED",
    "sourceId": "pmtu_maple_leaf",
    "siteId": "PMTU-MapleLeaf",
    "seriesId": "4939",
    "observedAt": "2026-10-15T19:30:00.000Z",
    "waterTempF": 60,
    "trend": "steady",
    "sourceType": "same_gauge",
    "attribution": "Measured water temperature"
  },
  "conditionsWaterTemperature": {
    "provider": "MONITOR_MY_WATERSHED",
    "sourceId": "pmtu_m37",
    "siteId": "PMTU-M37",
    "seriesId": "3201",
    "observedAt": "2026-10-15T19:30:00.000Z",
    "waterTempF": 60,
    "trend": "steady",
    "sourceType": "same_gauge",
    "attribution": "Measured water temperature"
  },
  "freshness": {
    "gauge": "fresh",
    "weather": "fresh",
    "waterTemperature": "fresh",
    "conditionsWaterTemperature": "fresh",
    "conditionsSuggestDaysUsable": 40
  },
  "dataQuality": {
    "label": "Fresh",
    "reasonCodes": [
      "data_quality_fresh"
    ]
  },
  "interpretationNote": null,
  "safety": {
    "regulationReminder": "Check current regulations, closures, and access rules before fishing.",
    "gaugeBasis": "Fishability describes the river stretch represented by the Scottville gauge; conditions can differ elsewhere.",
    "activityDisclaimer": "River Run is not a wading, boating, floating, or personal-safety rating."
  },
  "engineVersion": "river-run-v1.9.0-review",
  "configVersion": "2026-08-07.16-review"
} as unknown as RiverRunSnapshotResponse;

const GROUP_SEEDS = [
  {
    "id": "run_stage",
    "label": "Migration Stage",
    "scenarios": [
      {
        "id": "stage_offseason",
        "label": "True offseason after late copy",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-12-25",
          "progressionSnapshotAt": "2026-12-25T12:00:00.000Z",
          "conditionRefreshAt": "2026-12-25T20:00:00.000Z",
          "progressionExpiresAt": "2026-12-25T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-12-25T22:00:00.000Z",
          "runStage": {
            "stage": "post_run",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-12-25",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Fall entry complete",
            "headline": "PM Steelhead fall entry is complete.",
            "detail": "Steelhead may remain in the river. This fall-entry model no longer scores their current presence or activity.",
            "tip": "Check back in early September when PM fall movement tracking resumes.",
            "whereToStart": "There is no active PM starting section in this fall-entry model.",
            "reasonCodes": [
              "stage_post_run",
              "stage_offseason"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Timing complete",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "peak_complete",
            "checkpointDate": "2026-11-21",
            "cutoffDate": "2026-11-20",
            "observationStartDate": "2026-09-01",
            "completedCheckpointCount": 5,
            "currentIndex": 92.5,
            "currentPercentile": 100,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 87.5,
            "usableDays": 81,
            "expectedDays": 81,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14",
              "2026-11-15",
              "2026-11-16",
              "2026-11-17",
              "2026-11-18",
              "2026-11-19",
              "2026-11-20"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00",
              "2026-11-15": "16:00",
              "2026-11-16": "16:00",
              "2026-11-17": "16:00",
              "2026-11-18": "16:00",
              "2026-11-19": "16:00",
              "2026-11-20": "16:00"
            },
            "headline": "PM fall-entry Migration Timing is complete.",
            "detail": "Earlier in fall, PM migration timing was earlier than usual. The fall-entry timing window is now complete.",
            "tip": "Do not use completed fall timing to infer current presence or activity. PM fall monitoring resumes in early September.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_complete",
              "conditions_ahead",
              "conditions_timing_complete"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "peak_start",
            "previousCheckpointDate": "2026-11-15",
            "previousTimingLabel": "Ahead"
          },
          "push": {
            "score": null,
            "label": "Fall entry complete",
            "headline": "PM Steelhead fall-entry Push is complete.",
            "detail": "Current water may affect Steelhead still in the river. This fall model no longer scores fresh-entry support.",
            "tip": "Do not use a completed fall Push to infer current presence. Check back in early September.",
            "reasonCodes": [
              "push_tracking_complete"
            ],
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "complete",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-12-22",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-12-22",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-21",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-20",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-12-20T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-19",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-18",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-12-18T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-12-17",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-12-17T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-16",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": null,
            "scoreIsApproximate": false,
            "stage": "post_run",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0,
            "curveDirection": "outside",
            "winterHoldingContext": false,
            "label": "Fall entry complete",
            "headline": "PM Steelhead fall entry is complete.",
            "detail": "Steelhead may remain in the river. This fall-entry model no longer estimates their current seasonal presence.",
            "tip": "Check back in early September when PM fall movement tracking resumes.",
            "reasonCodes": [
              "stage_post_run",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-12-25T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-12-25T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-12-25T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          }
        }
      },
      {
        "id": "stage_before_staging",
        "label": "Before staging",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-08-15",
          "progressionSnapshotAt": "2026-08-15T12:00:00.000Z",
          "conditionRefreshAt": "2026-08-15T20:00:00.000Z",
          "progressionExpiresAt": "2026-08-15T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-08-15T22:00:00.000Z",
          "runStage": {
            "stage": "pre_run",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-08-15",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Before migration",
            "headline": "The PM Steelhead fall run has not started.",
            "detail": "Seasonal timing does not support dependable Steelhead presence in the PM river yet.",
            "tip": "Keep effort in the lake, harbor, and Pere Marquette Lake until staging begins.",
            "whereToStart": "Stay in Lake Michigan, Ludington harbor, and Pere Marquette Lake. The PM river is not a dependable fall-entry section yet.",
            "reasonCodes": [
              "stage_pre_run"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Not monitoring yet",
            "timingLabel": null,
            "candidateLabel": null,
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-09-20",
            "completedCheckpointCount": 0,
            "currentIndex": null,
            "currentPercentile": null,
            "gaugeResponsePercentile": null,
            "waterTemperaturePercentile": null,
            "usableDays": 0,
            "expectedDays": 0,
            "coveragePercent": 0,
            "historicalYears": 0,
            "sourceDates": [],
            "sourceRefreshSlots": {},
            "headline": "PM Migration Timing is not monitoring yet.",
            "detail": "Season-to-date Scottville flow and M-37 temperature monitoring resumes in early September.",
            "tip": "Check back in early September when PM timing monitoring resumes.",
            "reasonCodes": [
              "conditions_monitoring_inactive"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "push": {
            "score": null,
            "label": "Waiting for migration",
            "headline": "Dependable PM Steelhead fall entry has not started.",
            "detail": "Rain, Scottville flow, and water temperature are not scored as an in-season movement signal yet.",
            "tip": "Use Migration Stage. Do not move inland because offseason water resembles an in-season Push.",
            "reasonCodes": [
              "push_tracking_not_started"
            ],
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "not_started",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-08-15",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": []
          },
          "fishInRiver": {
            "score": 0,
            "displayScore": 0,
            "scoreIsApproximate": false,
            "stage": "pre_run",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0,
            "curveDirection": "outside",
            "winterHoldingContext": false,
            "label": "Not expected yet",
            "headline": "Dependable Steelhead presence is not expected in the PM river yet.",
            "detail": "The seasonal estimate remains at zero. Any fish already in the river would be an early exception.",
            "tip": "Use Migration Stage for nearby staging context. Do not treat one early fish as a dependable river opportunity.",
            "reasonCodes": [
              "stage_pre_run",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-08-15T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-08-15T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-08-15T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "The river should fish well, but seasonal fish presence is still low.",
            "detail": "Good flow makes presentations easier, but it does not put fish in the river. Stay low, fish the first travel lane entering deep resting water, make one complete pass, and move instead of waiting for numbers that have not developed.",
            "reasonCodes": [
              "good_fishability_low_presence"
            ]
          }
        }
      },
      {
        "id": "stage_staging",
        "label": "Staging period",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-09-01",
          "progressionSnapshotAt": "2026-09-01T12:00:00.000Z",
          "conditionRefreshAt": "2026-09-01T20:00:00.000Z",
          "progressionExpiresAt": "2026-09-01T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-09-01T22:00:00.000Z",
          "runStage": {
            "stage": "pre_run",
            "copyStrategy": "pere_marquette",
            "stagingContext": true,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-09-01",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Before migration",
            "headline": "Steelhead may be staging near the PM mouth, but dependable river entry has not begun.",
            "detail": "Seasonal timing supports nearby-lake staging. Any Steelhead already in the Lower river would be an early exception.",
            "tip": "Keep the river check brief. Do not move into the Middle or Upper river from calendar timing alone.",
            "whereToStart": "Start at Pere Marquette Lake and the river mouth. Add the Lower river (Pere Marquette Lake–Scottville) only for an early-fish check.",
            "reasonCodes": [
              "stage_pre_run",
              "stage_pre_run_staging"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Evaluating",
            "timingLabel": null,
            "candidateLabel": null,
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-09-20",
            "completedCheckpointCount": 0,
            "currentIndex": null,
            "currentPercentile": null,
            "gaugeResponsePercentile": null,
            "waterTemperaturePercentile": null,
            "usableDays": 0,
            "expectedDays": 0,
            "coveragePercent": 0,
            "historicalYears": 0,
            "sourceDates": [],
            "sourceRefreshSlots": {},
            "headline": "PM Migration Timing is still taking shape.",
            "detail": "Scottville river-rise activity and M-37 cooling do not yet support an Ahead, Typical, or Delayed call.",
            "tip": "Keep the section named by Migration Stage. Do not shift PM sections until this read has enough evidence.",
            "reasonCodes": [
              "conditions_checkpoint_evaluating"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "push": {
            "score": null,
            "label": "Waiting for migration",
            "headline": "Dependable PM Steelhead fall entry has not started.",
            "detail": "Rain, Scottville flow, and water temperature are not scored as an in-season movement signal yet.",
            "tip": "Use Migration Stage. Do not move inland because offseason water resembles an in-season Push.",
            "reasonCodes": [
              "push_tracking_not_started"
            ],
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "not_started",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-09-01",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": []
          },
          "fishInRiver": {
            "score": 0,
            "displayScore": 0,
            "scoreIsApproximate": false,
            "stage": "pre_run",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0,
            "curveDirection": "outside",
            "winterHoldingContext": false,
            "label": "Not expected yet",
            "headline": "Dependable Steelhead presence is not expected in the PM river yet.",
            "detail": "The seasonal estimate remains at zero. Any fish already in the river would be an early exception.",
            "tip": "Use Migration Stage for nearby staging context. Do not treat one early fish as a dependable river opportunity.",
            "reasonCodes": [
              "stage_pre_run",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-09-01T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-09-01T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-09-01T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "The river should fish well, but seasonal fish presence is still low.",
            "detail": "Good flow makes presentations easier, but it does not put fish in the river. Stay low, fish the first travel lane entering deep resting water, make one complete pass, and move instead of waiting for numbers that have not developed.",
            "reasonCodes": [
              "good_fishability_low_presence"
            ]
          }
        }
      },
      {
        "id": "stage_beginning",
        "label": "Beginning",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-09-20",
          "progressionSnapshotAt": "2026-09-20T12:00:00.000Z",
          "conditionRefreshAt": "2026-09-20T20:00:00.000Z",
          "progressionExpiresAt": "2026-09-20T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-09-20T22:00:00.000Z",
          "runStage": {
            "stage": "beginning",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-09-20",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Beginning",
            "headline": "Seasonal timing supports the first Steelhead entering the Lower river.",
            "detail": "This is the opening river phase. Presence is expected to be scattered and uneven.",
            "tip": "Cover the Lower river first. Move upstream only after direct fish activity supports it.",
            "whereToStart": "Start in the Lower river (Pere Marquette Lake–Scottville).",
            "reasonCodes": [
              "stage_beginning"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Typical",
            "timingLabel": "Typical",
            "candidateLabel": "Typical",
            "checkpointId": "river_start",
            "checkpointDate": "2026-09-20",
            "cutoffDate": "2026-09-19",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-10-11",
            "completedCheckpointCount": 1,
            "currentIndex": 45.4,
            "currentPercentile": 40,
            "gaugeResponsePercentile": 40,
            "waterTemperaturePercentile": 49,
            "usableDays": 19,
            "expectedDays": 19,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00"
            },
            "headline": "The PM migration is progressing at its usual seasonal pace.",
            "detail": "Season-to-date Scottville river-rise activity and M-37 cooling are close to the usual PM pattern.",
            "tip": "Keep the PM section named by Migration Stage.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_river_start",
              "conditions_typical"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-09-20",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": []
          },
          "fishInRiver": {
            "score": 8,
            "displayScore": 10,
            "scoreIsApproximate": true,
            "stage": "beginning",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0.1,
            "curveDirection": "rising",
            "winterHoldingContext": false,
            "label": "Low presence",
            "headline": "Steelhead seasonal presence is low presence and building.",
            "detail": "Calendar timing places Steelhead presence at low presence, increasing as the fall migration builds. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_beginning",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-09-20T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "The river should fish well, but seasonal fish presence is still low.",
            "detail": "Good flow makes presentations easier, but it does not put fish in the river. Stay low, fish the first travel lane entering deep resting water, make one complete pass, and move instead of waiting for numbers that have not developed.",
            "reasonCodes": [
              "good_fishability_low_presence"
            ]
          }
        }
      },
      {
        "id": "stage_building_early",
        "label": "Building — early",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-10-11",
          "progressionSnapshotAt": "2026-10-11T12:00:00.000Z",
          "conditionRefreshAt": "2026-10-11T20:00:00.000Z",
          "progressionExpiresAt": "2026-10-11T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-10-11T22:00:00.000Z",
          "runStage": {
            "stage": "building",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-10-11",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Building",
            "headline": "The PM Steelhead run is building beyond its earliest entry phase.",
            "detail": "Seasonal timing keeps the Lower river primary. Earlier arrivals may begin reaching the Middle river.",
            "tip": "Start low, then make one Middle river check if direct fish activity supports the move.",
            "whereToStart": "Start in the Lower river (Pere Marquette Lake–Scottville). Add the Middle river (Scottville–Maple Leaf) after direct fish activity supports the move.",
            "reasonCodes": [
              "stage_building"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Ahead",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "building_start",
            "checkpointDate": "2026-10-11",
            "cutoffDate": "2026-10-10",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-10-15",
            "completedCheckpointCount": 2,
            "currentIndex": 80,
            "currentPercentile": 80,
            "gaugeResponsePercentile": 80,
            "waterTemperaturePercentile": 80,
            "usableDays": 40,
            "expectedDays": 40,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00"
            },
            "headline": "The PM migration is developing earlier than usual.",
            "detail": "Season-to-date Scottville river-rise activity and M-37 cooling are stronger than the usual PM pattern.",
            "tip": "Start one PM section upstream from Migration Stage, capped at the Upper river (Maple Leaf–M-37).",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_building_start",
              "conditions_ahead"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "river_start",
            "previousCheckpointDate": "2026-09-20",
            "previousTimingLabel": "Typical"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-10-11",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-10-10",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-09",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-08",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-10-08T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-07",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-06",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-10-06T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-10-05",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-10-05T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-04",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": 30,
            "displayScore": 30,
            "scoreIsApproximate": true,
            "stage": "building",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0.37,
            "curveDirection": "rising",
            "winterHoldingContext": false,
            "label": "Limited presence",
            "headline": "Steelhead seasonal presence is limited presence and building.",
            "detail": "Calendar timing places Steelhead presence at limited presence, increasing as the fall migration builds. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_building",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-10-11T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-10-11T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-10-11T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          }
        }
      },
      {
        "id": "stage_building_established",
        "label": "Building — established",
        "note": "Canonical production copy",
        "snapshotOverride": {}
      },
      {
        "id": "stage_building_broad",
        "label": "Building — broadly established",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-11-01",
          "progressionSnapshotAt": "2026-11-01T12:00:00.000Z",
          "conditionRefreshAt": "2026-11-01T20:00:00.000Z",
          "progressionExpiresAt": "2026-11-01T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-11-01T22:00:00.000Z",
          "runStage": {
            "stage": "building",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": true,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-11-01",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Building",
            "headline": "Seasonal timing supports a broader PM Steelhead distribution.",
            "detail": "The Middle river is the dependable calendar choice. Earlier arrivals may also occupy the Upper river.",
            "tip": "Cover one Middle river section completely before comparing the Upper river.",
            "whereToStart": "Start in the Middle river (Scottville–Maple Leaf). Add the Upper river (Maple Leaf–M-37) when direct fish activity supports it.",
            "reasonCodes": [
              "stage_building"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-11-01",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-10-31",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-30",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-29",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-10-29T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-28",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-27",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-10-27T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-10-26",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-10-26T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-25",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": 60,
            "displayScore": 60,
            "scoreIsApproximate": true,
            "stage": "building",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0.75,
            "curveDirection": "rising",
            "winterHoldingContext": false,
            "label": "High presence",
            "headline": "Steelhead seasonal presence is high presence and building.",
            "detail": "Calendar timing places Steelhead presence high for this fall migration, increasing as the fall migration builds. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_building",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-11-01T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-11-01T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-11-01T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          }
        }
      },
      {
        "id": "stage_peak",
        "label": "Peak",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-11-15",
          "progressionSnapshotAt": "2026-11-15T12:00:00.000Z",
          "conditionRefreshAt": "2026-11-15T20:00:00.000Z",
          "progressionExpiresAt": "2026-11-15T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-11-15T22:00:00.000Z",
          "runStage": {
            "stage": "peak",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-11-15",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Peak",
            "headline": "This is typically the PM’s strongest Steelhead migration window.",
            "detail": "Seasonal timing supports the widest dependable Steelhead distribution. It does not confirm fish in every section.",
            "tip": "Start in the Middle river. Compare the Upper river only after a complete first pass.",
            "whereToStart": "Start in the Middle river (Scottville–Maple Leaf). Compare the Upper river (Maple Leaf–M-37) when direct fish activity favors it.",
            "reasonCodes": [
              "stage_peak"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Ahead",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "peak_start",
            "checkpointDate": "2026-11-15",
            "cutoffDate": "2026-11-14",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-11-21",
            "completedCheckpointCount": 4,
            "currentIndex": 92.5,
            "currentPercentile": 100,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 87.5,
            "usableDays": 75,
            "expectedDays": 75,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00"
            },
            "headline": "The PM migration is developing earlier than usual.",
            "detail": "Season-to-date Scottville river-rise activity and M-37 cooling are stronger than the usual PM pattern.",
            "tip": "Start one PM section upstream from Migration Stage, capped at the Upper river (Maple Leaf–M-37).",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_start",
              "conditions_ahead"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "building_established",
            "previousCheckpointDate": "2026-10-15",
            "previousTimingLabel": "Ahead"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-11-15",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-11-14",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-13",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-12",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-11-12T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-11",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-10",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-11-10T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-11-09",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-11-09T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-08",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": 80,
            "displayScore": 80,
            "scoreIsApproximate": false,
            "stage": "peak",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 1,
            "curveDirection": "near_peak",
            "winterHoldingContext": false,
            "label": "Peak presence",
            "headline": "Steelhead seasonal presence is peak presence and near its seasonal high.",
            "detail": "Calendar timing places Steelhead presence at its expected seasonal peak, near its expected seasonal high. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_peak",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-11-15T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-11-15T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-11-15T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "Seasonal presence is near peak, but today's water does not point to a fresh wave.",
            "detail": "Many fish may already be in the river even when rain, river level, and temperature show little support for new movement. Start in established deep holes, fish each one from head to tail, and leave lower entry lanes secondary.",
            "reasonCodes": [
              "peak_presence_weak_push"
            ]
          }
        }
      },
      {
        "id": "stage_tapering",
        "label": "Tapering",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-12-05",
          "progressionSnapshotAt": "2026-12-05T12:00:00.000Z",
          "conditionRefreshAt": "2026-12-05T20:00:00.000Z",
          "progressionExpiresAt": "2026-12-05T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-12-05T22:00:00.000Z",
          "runStage": {
            "stage": "tapering",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-12-05",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Late fall",
            "headline": "PM Steelhead fall entry remains strong but is slowing.",
            "detail": "Seasonal timing still supports broad fall presence. Colder water makes established holding water more important than new entry.",
            "tip": "Start in the Middle river. Shift lower only when Push supports fresh movement.",
            "whereToStart": "Start in the Middle river (Scottville–Maple Leaf). Add the Lower river (Pere Marquette Lake–Scottville) only when Push supports fresh movement.",
            "reasonCodes": [
              "stage_tapering"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Timing complete",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "peak_complete",
            "checkpointDate": "2026-11-21",
            "cutoffDate": "2026-11-20",
            "observationStartDate": "2026-09-01",
            "completedCheckpointCount": 5,
            "currentIndex": 92.5,
            "currentPercentile": 100,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 87.5,
            "usableDays": 81,
            "expectedDays": 81,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14",
              "2026-11-15",
              "2026-11-16",
              "2026-11-17",
              "2026-11-18",
              "2026-11-19",
              "2026-11-20"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00",
              "2026-11-15": "16:00",
              "2026-11-16": "16:00",
              "2026-11-17": "16:00",
              "2026-11-18": "16:00",
              "2026-11-19": "16:00",
              "2026-11-20": "16:00"
            },
            "headline": "The early-season timing read is complete.",
            "detail": "Earlier in the season, the migration was moving earlier than usual. Now that the migration is well underway, current movement and river conditions matter more than an early-or-late call.",
            "tip": "Stop shifting river sections based on early-or-late timing. Begin in established holding water for the current Migration Stage; if Push is Possible or stronger, make lower travel lanes the next stop.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_complete",
              "conditions_ahead",
              "conditions_timing_complete"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "peak_start",
            "previousCheckpointDate": "2026-11-15",
            "previousTimingLabel": "Ahead"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-12-05",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-12-04",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-03",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-02",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-12-02T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-01",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-30",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-11-30T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-11-29",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-11-29T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-28",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": 79,
            "displayScore": 80,
            "scoreIsApproximate": true,
            "stage": "tapering",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0.99,
            "curveDirection": "falling",
            "winterHoldingContext": false,
            "label": "Peak presence",
            "headline": "Steelhead seasonal presence is peak presence and declining.",
            "detail": "Calendar timing places Steelhead presence at its expected seasonal peak, declining as the fall migration advances. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_tapering",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-12-05T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-12-05T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-12-05T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          }
        }
      },
      {
        "id": "stage_ending",
        "label": "Ending",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-12-20",
          "progressionSnapshotAt": "2026-12-20T12:00:00.000Z",
          "conditionRefreshAt": "2026-12-20T20:00:00.000Z",
          "progressionExpiresAt": "2026-12-20T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-12-20T22:00:00.000Z",
          "runStage": {
            "stage": "ending",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-12-20",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Holding transition",
            "headline": "PM Steelhead fall entry is in its final seasonal phase.",
            "detail": "Steelhead may remain after fall entry ends. This state marks the model boundary, not fish leaving the river.",
            "tip": "Prioritize established Middle river holding water and use Activity only through the final fall-entry day.",
            "whereToStart": "Start in the Middle river (Scottville–Maple Leaf). Add the Upper river (Maple Leaf–M-37) only for established late-fall holding water.",
            "reasonCodes": [
              "stage_ending"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Timing complete",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "peak_complete",
            "checkpointDate": "2026-11-21",
            "cutoffDate": "2026-11-20",
            "observationStartDate": "2026-09-01",
            "completedCheckpointCount": 5,
            "currentIndex": 92.5,
            "currentPercentile": 100,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 87.5,
            "usableDays": 81,
            "expectedDays": 81,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14",
              "2026-11-15",
              "2026-11-16",
              "2026-11-17",
              "2026-11-18",
              "2026-11-19",
              "2026-11-20"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00",
              "2026-11-15": "16:00",
              "2026-11-16": "16:00",
              "2026-11-17": "16:00",
              "2026-11-18": "16:00",
              "2026-11-19": "16:00",
              "2026-11-20": "16:00"
            },
            "headline": "The early-season timing read is complete.",
            "detail": "Earlier in the season, the migration was moving earlier than usual. Now that the migration is well underway, current movement and river conditions matter more than an early-or-late call.",
            "tip": "Stop shifting river sections based on early-or-late timing. Begin in established holding water for the current Migration Stage; if Push is Possible or stronger, make lower travel lanes the next stop.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_complete",
              "conditions_ahead",
              "conditions_timing_complete"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "peak_start",
            "previousCheckpointDate": "2026-11-15",
            "previousTimingLabel": "Ahead"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-12-20",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-12-19",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-18",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-17",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-12-17T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-16",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-15",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-12-15T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-12-14",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-12-14T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-13",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": 71,
            "displayScore": 70,
            "scoreIsApproximate": true,
            "stage": "ending",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0.8916666666666667,
            "curveDirection": "falling",
            "winterHoldingContext": false,
            "label": "High presence",
            "headline": "Steelhead seasonal presence is high presence and declining.",
            "detail": "Calendar timing places Steelhead presence high for this fall migration, declining as the fall migration advances. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_ending",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-12-20T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-12-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-12-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          }
        }
      },
      {
        "id": "stage_post",
        "label": "After the main migration window",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-12-23",
          "progressionSnapshotAt": "2026-12-23T12:00:00.000Z",
          "conditionRefreshAt": "2026-12-23T20:00:00.000Z",
          "progressionExpiresAt": "2026-12-23T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-12-23T22:00:00.000Z",
          "runStage": {
            "stage": "post_run",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-12-23",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Fall entry complete",
            "headline": "PM Steelhead fall entry is complete.",
            "detail": "Steelhead may remain in the river. This fall-entry model no longer scores their current presence or activity.",
            "tip": "Check back in early September when PM fall movement tracking resumes.",
            "whereToStart": "There is no active PM starting section in this fall-entry model.",
            "reasonCodes": [
              "stage_post_run",
              "stage_offseason"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Timing complete",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "peak_complete",
            "checkpointDate": "2026-11-21",
            "cutoffDate": "2026-11-20",
            "observationStartDate": "2026-09-01",
            "completedCheckpointCount": 5,
            "currentIndex": 92.5,
            "currentPercentile": 100,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 87.5,
            "usableDays": 81,
            "expectedDays": 81,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14",
              "2026-11-15",
              "2026-11-16",
              "2026-11-17",
              "2026-11-18",
              "2026-11-19",
              "2026-11-20"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00",
              "2026-11-15": "16:00",
              "2026-11-16": "16:00",
              "2026-11-17": "16:00",
              "2026-11-18": "16:00",
              "2026-11-19": "16:00",
              "2026-11-20": "16:00"
            },
            "headline": "PM fall-entry Migration Timing is complete.",
            "detail": "Earlier in fall, PM migration timing was earlier than usual. The fall-entry timing window is now complete.",
            "tip": "Do not use completed fall timing to infer current presence or activity. PM fall monitoring resumes in early September.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_complete",
              "conditions_ahead",
              "conditions_timing_complete"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "peak_start",
            "previousCheckpointDate": "2026-11-15",
            "previousTimingLabel": "Ahead"
          },
          "push": {
            "score": null,
            "label": "Fall entry complete",
            "headline": "PM Steelhead fall-entry Push is complete.",
            "detail": "Current water may affect Steelhead still in the river. This fall model no longer scores fresh-entry support.",
            "tip": "Do not use a completed fall Push to infer current presence. Check back in early September.",
            "reasonCodes": [
              "push_tracking_complete"
            ],
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "complete",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-12-22",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-12-22",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-21",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-20",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-12-20T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-19",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-18",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-12-18T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-12-17",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-12-17T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-16",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": null,
            "scoreIsApproximate": false,
            "stage": "post_run",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0,
            "curveDirection": "outside",
            "winterHoldingContext": false,
            "label": "Fall entry complete",
            "headline": "PM Steelhead fall entry is complete.",
            "detail": "Steelhead may remain in the river. This fall-entry model no longer estimates their current seasonal presence.",
            "tip": "Check back in early September when PM fall movement tracking resumes.",
            "reasonCodes": [
              "stage_post_run",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-12-23T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-12-23T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-12-23T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          }
        }
      }
    ]
  },
  {
    "id": "conditions",
    "label": "Migration Timing",
    "scenarios": [
      {
        "id": "conditions_before_collection",
        "label": "Before evidence collection",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-08-24",
          "progressionSnapshotAt": "2026-08-24T12:00:00.000Z",
          "conditionRefreshAt": "2026-08-24T20:00:00.000Z",
          "progressionExpiresAt": "2026-08-24T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-08-24T22:00:00.000Z",
          "runStage": {
            "stage": "pre_run",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-08-24",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Before migration",
            "headline": "The PM Steelhead fall run has not started.",
            "detail": "Seasonal timing does not support dependable Steelhead presence in the PM river yet.",
            "tip": "Keep effort in the lake, harbor, and Pere Marquette Lake until staging begins.",
            "whereToStart": "Stay in Lake Michigan, Ludington harbor, and Pere Marquette Lake. The PM river is not a dependable fall-entry section yet.",
            "reasonCodes": [
              "stage_pre_run"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Not monitoring yet",
            "timingLabel": null,
            "candidateLabel": null,
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-09-20",
            "completedCheckpointCount": 0,
            "currentIndex": null,
            "currentPercentile": null,
            "gaugeResponsePercentile": null,
            "waterTemperaturePercentile": null,
            "usableDays": 0,
            "expectedDays": 0,
            "coveragePercent": 0,
            "historicalYears": 0,
            "sourceDates": [],
            "sourceRefreshSlots": {},
            "headline": "PM Migration Timing is not monitoring yet.",
            "detail": "Season-to-date Scottville flow and M-37 temperature monitoring resumes in early September.",
            "tip": "Check back in early September when PM timing monitoring resumes.",
            "reasonCodes": [
              "conditions_monitoring_inactive"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "push": {
            "score": null,
            "label": "Waiting for migration",
            "headline": "Dependable PM Steelhead fall entry has not started.",
            "detail": "Rain, Scottville flow, and water temperature are not scored as an in-season movement signal yet.",
            "tip": "Use Migration Stage. Do not move inland because offseason water resembles an in-season Push.",
            "reasonCodes": [
              "push_tracking_not_started"
            ],
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "not_started",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-08-24",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": []
          },
          "fishInRiver": {
            "score": 0,
            "displayScore": 0,
            "scoreIsApproximate": false,
            "stage": "pre_run",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0,
            "curveDirection": "outside",
            "winterHoldingContext": false,
            "label": "Not expected yet",
            "headline": "Dependable Steelhead presence is not expected in the PM river yet.",
            "detail": "The seasonal estimate remains at zero. Any fish already in the river would be an early exception.",
            "tip": "Use Migration Stage for nearby staging context. Do not treat one early fish as a dependable river opportunity.",
            "reasonCodes": [
              "stage_pre_run",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-08-24T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-08-24T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-08-24T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "The river should fish well, but seasonal fish presence is still low.",
            "detail": "Good flow makes presentations easier, but it does not put fish in the river. Stay low, fish the first travel lane entering deep resting water, make one complete pass, and move instead of waiting for numbers that have not developed.",
            "reasonCodes": [
              "good_fishability_low_presence"
            ]
          }
        }
      },
      {
        "id": "conditions_collecting",
        "label": "Collecting first read",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-09-05",
          "progressionSnapshotAt": "2026-09-05T12:00:00.000Z",
          "conditionRefreshAt": "2026-09-05T20:00:00.000Z",
          "progressionExpiresAt": "2026-09-05T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-09-05T22:00:00.000Z",
          "runStage": {
            "stage": "pre_run",
            "copyStrategy": "pere_marquette",
            "stagingContext": true,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-09-05",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Before migration",
            "headline": "Steelhead may be staging near the PM mouth, but dependable river entry has not begun.",
            "detail": "Seasonal timing supports nearby-lake staging. Any Steelhead already in the Lower river would be an early exception.",
            "tip": "Keep the river check brief. Do not move into the Middle or Upper river from calendar timing alone.",
            "whereToStart": "Start at Pere Marquette Lake and the river mouth. Add the Lower river (Pere Marquette Lake–Scottville) only for an early-fish check.",
            "reasonCodes": [
              "stage_pre_run",
              "stage_pre_run_staging"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Evaluating",
            "timingLabel": null,
            "candidateLabel": null,
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-09-20",
            "completedCheckpointCount": 0,
            "currentIndex": null,
            "currentPercentile": null,
            "gaugeResponsePercentile": null,
            "waterTemperaturePercentile": null,
            "usableDays": 0,
            "expectedDays": 0,
            "coveragePercent": 0,
            "historicalYears": 0,
            "sourceDates": [],
            "sourceRefreshSlots": {},
            "headline": "PM Migration Timing is still taking shape.",
            "detail": "Scottville river-rise activity and M-37 cooling do not yet support an Ahead, Typical, or Delayed call.",
            "tip": "Keep the section named by Migration Stage. Do not shift PM sections until this read has enough evidence.",
            "reasonCodes": [
              "conditions_checkpoint_evaluating"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "push": {
            "score": null,
            "label": "Waiting for migration",
            "headline": "Dependable PM Steelhead fall entry has not started.",
            "detail": "Rain, Scottville flow, and water temperature are not scored as an in-season movement signal yet.",
            "tip": "Use Migration Stage. Do not move inland because offseason water resembles an in-season Push.",
            "reasonCodes": [
              "push_tracking_not_started"
            ],
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "not_started",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-09-05",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": []
          },
          "fishInRiver": {
            "score": 0,
            "displayScore": 0,
            "scoreIsApproximate": false,
            "stage": "pre_run",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0,
            "curveDirection": "outside",
            "winterHoldingContext": false,
            "label": "Not expected yet",
            "headline": "Dependable Steelhead presence is not expected in the PM river yet.",
            "detail": "The seasonal estimate remains at zero. Any fish already in the river would be an early exception.",
            "tip": "Use Migration Stage for nearby staging context. Do not treat one early fish as a dependable river opportunity.",
            "reasonCodes": [
              "stage_pre_run",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-09-05T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-09-05T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-09-05T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "The river should fish well, but seasonal fish presence is still low.",
            "detail": "Good flow makes presentations easier, but it does not put fish in the river. Stay low, fish the first travel lane entering deep resting water, make one complete pass, and move instead of waiting for numbers that have not developed.",
            "reasonCodes": [
              "good_fishability_low_presence"
            ]
          }
        }
      },
      {
        "id": "conditions_ahead",
        "label": "Ahead",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-09-20",
          "progressionSnapshotAt": "2026-09-20T12:00:00.000Z",
          "conditionRefreshAt": "2026-09-20T20:00:00.000Z",
          "progressionExpiresAt": "2026-09-20T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-09-20T22:00:00.000Z",
          "runStage": {
            "stage": "beginning",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-09-20",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Beginning",
            "headline": "Seasonal timing supports the first Steelhead entering the Lower river.",
            "detail": "This is the opening river phase. Presence is expected to be scattered and uneven.",
            "tip": "Cover the Lower river first. Move upstream only after direct fish activity supports it.",
            "whereToStart": "Start in the Lower river (Pere Marquette Lake–Scottville).",
            "reasonCodes": [
              "stage_beginning"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Ahead",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "river_start",
            "checkpointDate": "2026-09-20",
            "cutoffDate": "2026-09-19",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-10-11",
            "completedCheckpointCount": 1,
            "currentIndex": 85,
            "currentPercentile": 80,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 75,
            "usableDays": 19,
            "expectedDays": 19,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00"
            },
            "headline": "The PM migration is developing earlier than usual.",
            "detail": "Season-to-date Scottville river-rise activity and M-37 cooling are stronger than the usual PM pattern.",
            "tip": "Start one PM section upstream from Migration Stage, capped at the Upper river (Maple Leaf–M-37).",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_river_start",
              "conditions_ahead"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-09-20",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": []
          },
          "fishInRiver": {
            "score": 8,
            "displayScore": 10,
            "scoreIsApproximate": true,
            "stage": "beginning",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0.1,
            "curveDirection": "rising",
            "winterHoldingContext": false,
            "label": "Low presence",
            "headline": "Steelhead seasonal presence is low presence and building.",
            "detail": "Calendar timing places Steelhead presence at low presence, increasing as the fall migration builds. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_beginning",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-09-20T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "Today's reads are pointing to different parts of the picture.",
            "detail": "The season is in its opening stage, while river rises and cooling are progressing faster than usual. Begin one accessible river section farther upstream than a normal opening-day plan and fish the first established holding water; this is still not a live fish count.\n\nGood flow makes presentations easier, but it does not put fish in the river. Stay low, fish the first travel lane entering deep resting water, make one complete pass, and move instead of waiting for numbers that have not developed.",
            "reasonCodes": [
              "beginning_ahead_conditions",
              "good_fishability_low_presence"
            ]
          }
        }
      },
      {
        "id": "conditions_typical",
        "label": "Typical",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-09-20",
          "progressionSnapshotAt": "2026-09-20T12:00:00.000Z",
          "conditionRefreshAt": "2026-09-20T20:00:00.000Z",
          "progressionExpiresAt": "2026-09-20T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-09-20T22:00:00.000Z",
          "runStage": {
            "stage": "beginning",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-09-20",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Beginning",
            "headline": "Seasonal timing supports the first Steelhead entering the Lower river.",
            "detail": "This is the opening river phase. Presence is expected to be scattered and uneven.",
            "tip": "Cover the Lower river first. Move upstream only after direct fish activity supports it.",
            "whereToStart": "Start in the Lower river (Pere Marquette Lake–Scottville).",
            "reasonCodes": [
              "stage_beginning"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Typical",
            "timingLabel": "Typical",
            "candidateLabel": "Typical",
            "checkpointId": "river_start",
            "checkpointDate": "2026-09-20",
            "cutoffDate": "2026-09-19",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-10-11",
            "completedCheckpointCount": 1,
            "currentIndex": 45.4,
            "currentPercentile": 40,
            "gaugeResponsePercentile": 40,
            "waterTemperaturePercentile": 49,
            "usableDays": 19,
            "expectedDays": 19,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00"
            },
            "headline": "The PM migration is progressing at its usual seasonal pace.",
            "detail": "Season-to-date Scottville river-rise activity and M-37 cooling are close to the usual PM pattern.",
            "tip": "Keep the PM section named by Migration Stage.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_river_start",
              "conditions_typical"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-09-20",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": []
          },
          "fishInRiver": {
            "score": 8,
            "displayScore": 10,
            "scoreIsApproximate": true,
            "stage": "beginning",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0.1,
            "curveDirection": "rising",
            "winterHoldingContext": false,
            "label": "Low presence",
            "headline": "Steelhead seasonal presence is low presence and building.",
            "detail": "Calendar timing places Steelhead presence at low presence, increasing as the fall migration builds. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_beginning",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-09-20T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "The river should fish well, but seasonal fish presence is still low.",
            "detail": "Good flow makes presentations easier, but it does not put fish in the river. Stay low, fish the first travel lane entering deep resting water, make one complete pass, and move instead of waiting for numbers that have not developed.",
            "reasonCodes": [
              "good_fishability_low_presence"
            ]
          }
        }
      },
      {
        "id": "conditions_delayed",
        "label": "Delayed",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-09-20",
          "progressionSnapshotAt": "2026-09-20T12:00:00.000Z",
          "conditionRefreshAt": "2026-09-20T20:00:00.000Z",
          "progressionExpiresAt": "2026-09-20T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-09-20T22:00:00.000Z",
          "runStage": {
            "stage": "beginning",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-09-20",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Beginning",
            "headline": "Seasonal timing supports the first Steelhead entering the Lower river.",
            "detail": "This is the opening river phase. Presence is expected to be scattered and uneven.",
            "tip": "Cover the Lower river first. Move upstream only after direct fish activity supports it.",
            "whereToStart": "Start in the Lower river (Pere Marquette Lake–Scottville).",
            "reasonCodes": [
              "stage_beginning"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Delayed",
            "timingLabel": "Delayed",
            "candidateLabel": "Delayed",
            "checkpointId": "river_start",
            "checkpointDate": "2026-09-20",
            "cutoffDate": "2026-09-19",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-10-11",
            "completedCheckpointCount": 1,
            "currentIndex": 16,
            "currentPercentile": 20,
            "gaugeResponsePercentile": 10,
            "waterTemperaturePercentile": 20,
            "usableDays": 19,
            "expectedDays": 19,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00"
            },
            "headline": "The PM migration is developing later than usual.",
            "detail": "Season-to-date Scottville river-rise activity and M-37 cooling are weaker than the usual PM pattern.",
            "tip": "Start one PM section downstream from Migration Stage, capped at the Lower river (Pere Marquette Lake–Scottville).",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_river_start",
              "conditions_delayed"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-09-20",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": []
          },
          "fishInRiver": {
            "score": 8,
            "displayScore": 10,
            "scoreIsApproximate": true,
            "stage": "beginning",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0.1,
            "curveDirection": "rising",
            "winterHoldingContext": false,
            "label": "Low presence",
            "headline": "Steelhead seasonal presence is low presence and building.",
            "detail": "Calendar timing places Steelhead presence at low presence, increasing as the fall migration builds. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_beginning",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-09-20T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "The river should fish well, but seasonal fish presence is still low.",
            "detail": "Good flow makes presentations easier, but it does not put fish in the river. Stay low, fish the first travel lane entering deep resting water, make one complete pass, and move instead of waiting for numbers that have not developed.",
            "reasonCodes": [
              "good_fishability_low_presence"
            ]
          }
        }
      },
      {
        "id": "conditions_mixed",
        "label": "Typical · mixed signals",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-09-20",
          "progressionSnapshotAt": "2026-09-20T12:00:00.000Z",
          "conditionRefreshAt": "2026-09-20T20:00:00.000Z",
          "progressionExpiresAt": "2026-09-20T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-09-20T22:00:00.000Z",
          "runStage": {
            "stage": "beginning",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-09-20",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Beginning",
            "headline": "Seasonal timing supports the first Steelhead entering the Lower river.",
            "detail": "This is the opening river phase. Presence is expected to be scattered and uneven.",
            "tip": "Cover the Lower river first. Move upstream only after direct fish activity supports it.",
            "whereToStart": "Start in the Lower river (Pere Marquette Lake–Scottville).",
            "reasonCodes": [
              "stage_beginning"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Typical",
            "timingLabel": "Typical",
            "candidateLabel": "Typical",
            "checkpointId": "river_start",
            "checkpointDate": "2026-09-20",
            "cutoffDate": "2026-09-19",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-10-11",
            "completedCheckpointCount": 1,
            "currentIndex": 52,
            "currentPercentile": 60,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 20,
            "usableDays": 19,
            "expectedDays": 19,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00"
            },
            "headline": "PM migration timing remains Typical because the signals are mixed.",
            "detail": "Scottville river-rise activity and M-37 cooling point in opposite directions. Together they do not support an early or late call.",
            "tip": "Keep the section named by Migration Stage. Use Push only for today’s fresh-movement support.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_river_start",
              "conditions_typical",
              "conditions_signals_mixed"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-09-20",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": []
          },
          "fishInRiver": {
            "score": 8,
            "displayScore": 10,
            "scoreIsApproximate": true,
            "stage": "beginning",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0.1,
            "curveDirection": "rising",
            "winterHoldingContext": false,
            "label": "Low presence",
            "headline": "Steelhead seasonal presence is low presence and building.",
            "detail": "Calendar timing places Steelhead presence at low presence, increasing as the fall migration builds. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_beginning",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-09-20T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "The river should fish well, but seasonal fish presence is still low.",
            "detail": "Good flow makes presentations easier, but it does not put fish in the river. Stay low, fish the first travel lane entering deep resting water, make one complete pass, and move instead of waiting for numbers that have not developed.",
            "reasonCodes": [
              "good_fishability_low_presence"
            ]
          }
        }
      },
      {
        "id": "conditions_tempered_reversal",
        "label": "Typical · reversal tempered",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-10-11",
          "progressionSnapshotAt": "2026-10-11T12:00:00.000Z",
          "conditionRefreshAt": "2026-10-11T20:00:00.000Z",
          "progressionExpiresAt": "2026-10-11T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-10-11T22:00:00.000Z",
          "runStage": {
            "stage": "building",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-10-11",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Building",
            "headline": "The PM Steelhead run is building beyond its earliest entry phase.",
            "detail": "Seasonal timing keeps the Lower river primary. Earlier arrivals may begin reaching the Middle river.",
            "tip": "Start low, then make one Middle river check if direct fish activity supports the move.",
            "whereToStart": "Start in the Lower river (Pere Marquette Lake–Scottville). Add the Middle river (Scottville–Maple Leaf) after direct fish activity supports the move.",
            "reasonCodes": [
              "stage_building"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Delayed",
            "timingLabel": "Delayed",
            "candidateLabel": "Delayed",
            "checkpointId": "building_start",
            "checkpointDate": "2026-10-11",
            "cutoffDate": "2026-10-10",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-10-15",
            "completedCheckpointCount": 2,
            "currentIndex": 0,
            "currentPercentile": 0,
            "gaugeResponsePercentile": 0,
            "waterTemperaturePercentile": 0,
            "usableDays": 40,
            "expectedDays": 40,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00"
            },
            "headline": "The PM migration is developing later than usual.",
            "detail": "Season-to-date Scottville river-rise activity and M-37 cooling are weaker than the usual PM pattern.",
            "tip": "Start one PM section downstream from Migration Stage, capped at the Lower river (Pere Marquette Lake–Scottville).",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_building_start",
              "conditions_delayed"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "river_start",
            "previousCheckpointDate": "2026-09-20",
            "previousTimingLabel": "Typical"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-10-11",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-10-10",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-09",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-08",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-10-08T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-07",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-06",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-10-06T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-10-05",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-10-05T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-04",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": 30,
            "displayScore": 30,
            "scoreIsApproximate": true,
            "stage": "building",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0.37,
            "curveDirection": "rising",
            "winterHoldingContext": false,
            "label": "Limited presence",
            "headline": "Steelhead seasonal presence is limited presence and building.",
            "detail": "Calendar timing places Steelhead presence at limited presence, increasing as the fall migration builds. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_building",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-10-11T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-10-11T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-10-11T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          }
        }
      },
      {
        "id": "conditions_complete_underway",
        "label": "Timing complete · underway",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-11-21",
          "progressionSnapshotAt": "2026-11-21T12:00:00.000Z",
          "conditionRefreshAt": "2026-11-21T20:00:00.000Z",
          "progressionExpiresAt": "2026-11-21T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-11-21T22:00:00.000Z",
          "runStage": {
            "stage": "peak",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-11-21",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Peak",
            "headline": "This is typically the PM’s strongest Steelhead migration window.",
            "detail": "Seasonal timing supports the widest dependable Steelhead distribution. It does not confirm fish in every section.",
            "tip": "Start in the Middle river. Compare the Upper river only after a complete first pass.",
            "whereToStart": "Start in the Middle river (Scottville–Maple Leaf). Compare the Upper river (Maple Leaf–M-37) when direct fish activity favors it.",
            "reasonCodes": [
              "stage_peak"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Timing complete",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "peak_complete",
            "checkpointDate": "2026-11-21",
            "cutoffDate": "2026-11-20",
            "observationStartDate": "2026-09-01",
            "completedCheckpointCount": 5,
            "currentIndex": 92.5,
            "currentPercentile": 100,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 87.5,
            "usableDays": 81,
            "expectedDays": 81,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14",
              "2026-11-15",
              "2026-11-16",
              "2026-11-17",
              "2026-11-18",
              "2026-11-19",
              "2026-11-20"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00",
              "2026-11-15": "16:00",
              "2026-11-16": "16:00",
              "2026-11-17": "16:00",
              "2026-11-18": "16:00",
              "2026-11-19": "16:00",
              "2026-11-20": "16:00"
            },
            "headline": "The early-season timing read is complete.",
            "detail": "Earlier in the season, the migration was moving earlier than usual. Now that the migration is well underway, current movement and river conditions matter more than an early-or-late call.",
            "tip": "Stop shifting river sections based on early-or-late timing. Begin in established holding water for the current Migration Stage; if Push is Possible or stronger, make lower travel lanes the next stop.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_complete",
              "conditions_ahead",
              "conditions_timing_complete"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "peak_start",
            "previousCheckpointDate": "2026-11-15",
            "previousTimingLabel": "Ahead"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-11-21",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-11-20",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-19",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-18",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-11-18T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-17",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-16",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-11-16T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-11-15",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-11-15T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-14",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": 80,
            "displayScore": 80,
            "scoreIsApproximate": false,
            "stage": "peak",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 1,
            "curveDirection": "near_peak",
            "winterHoldingContext": false,
            "label": "Peak presence",
            "headline": "Steelhead seasonal presence is peak presence and near its seasonal high.",
            "detail": "Calendar timing places Steelhead presence at its expected seasonal peak, near its expected seasonal high. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_peak",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-11-21T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-11-21T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-11-21T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "Seasonal presence is near peak, but today's water does not point to a fresh wave.",
            "detail": "Many fish may already be in the river even when rain, river level, and temperature show little support for new movement. Start in established deep holes, fish each one from head to tail, and leave lower entry lanes secondary.",
            "reasonCodes": [
              "peak_presence_weak_push"
            ]
          }
        }
      },
      {
        "id": "conditions_complete_post",
        "label": "Timing complete · post-run",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-12-23",
          "progressionSnapshotAt": "2026-12-23T12:00:00.000Z",
          "conditionRefreshAt": "2026-12-23T20:00:00.000Z",
          "progressionExpiresAt": "2026-12-23T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-12-23T22:00:00.000Z",
          "runStage": {
            "stage": "post_run",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-12-23",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Fall entry complete",
            "headline": "PM Steelhead fall entry is complete.",
            "detail": "Steelhead may remain in the river. This fall-entry model no longer scores their current presence or activity.",
            "tip": "Check back in early September when PM fall movement tracking resumes.",
            "whereToStart": "There is no active PM starting section in this fall-entry model.",
            "reasonCodes": [
              "stage_post_run",
              "stage_offseason"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Timing complete",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "peak_complete",
            "checkpointDate": "2026-11-21",
            "cutoffDate": "2026-11-20",
            "observationStartDate": "2026-09-01",
            "completedCheckpointCount": 5,
            "currentIndex": 92.5,
            "currentPercentile": 100,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 87.5,
            "usableDays": 81,
            "expectedDays": 81,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14",
              "2026-11-15",
              "2026-11-16",
              "2026-11-17",
              "2026-11-18",
              "2026-11-19",
              "2026-11-20"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00",
              "2026-11-15": "16:00",
              "2026-11-16": "16:00",
              "2026-11-17": "16:00",
              "2026-11-18": "16:00",
              "2026-11-19": "16:00",
              "2026-11-20": "16:00"
            },
            "headline": "PM fall-entry Migration Timing is complete.",
            "detail": "Earlier in fall, PM migration timing was earlier than usual. The fall-entry timing window is now complete.",
            "tip": "Do not use completed fall timing to infer current presence or activity. PM fall monitoring resumes in early September.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_complete",
              "conditions_ahead",
              "conditions_timing_complete"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "peak_start",
            "previousCheckpointDate": "2026-11-15",
            "previousTimingLabel": "Ahead"
          },
          "push": {
            "score": null,
            "label": "Fall entry complete",
            "headline": "PM Steelhead fall-entry Push is complete.",
            "detail": "Current water may affect Steelhead still in the river. This fall model no longer scores fresh-entry support.",
            "tip": "Do not use a completed fall Push to infer current presence. Check back in early September.",
            "reasonCodes": [
              "push_tracking_complete"
            ],
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "complete",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-12-22",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-12-22",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-21",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-20",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-12-20T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-19",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-18",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-12-18T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-12-17",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-12-17T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-16",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": null,
            "scoreIsApproximate": false,
            "stage": "post_run",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0,
            "curveDirection": "outside",
            "winterHoldingContext": false,
            "label": "Fall entry complete",
            "headline": "PM Steelhead fall entry is complete.",
            "detail": "Steelhead may remain in the river. This fall-entry model no longer estimates their current seasonal presence.",
            "tip": "Check back in early September when PM fall movement tracking resumes.",
            "reasonCodes": [
              "stage_post_run",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-12-23T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-12-23T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-12-23T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          }
        }
      },
      {
        "id": "conditions_complete_insufficient",
        "label": "Timing complete · insufficient",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-11-21",
          "progressionSnapshotAt": "2026-11-21T12:00:00.000Z",
          "conditionRefreshAt": "2026-11-21T20:00:00.000Z",
          "progressionExpiresAt": "2026-11-21T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-11-21T22:00:00.000Z",
          "runStage": {
            "stage": "peak",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-11-21",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Peak",
            "headline": "This is typically the PM’s strongest Steelhead migration window.",
            "detail": "Seasonal timing supports the widest dependable Steelhead distribution. It does not confirm fish in every section.",
            "tip": "Start in the Middle river. Compare the Upper river only after a complete first pass.",
            "whereToStart": "Start in the Middle river (Scottville–Maple Leaf). Compare the Upper river (Maple Leaf–M-37) when direct fish activity favors it.",
            "reasonCodes": [
              "stage_peak"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Timing complete",
            "timingLabel": "Insufficient evidence",
            "candidateLabel": "Insufficient evidence",
            "checkpointId": "peak_complete",
            "checkpointDate": "2026-11-21",
            "cutoffDate": "2026-11-20",
            "observationStartDate": "2026-09-01",
            "completedCheckpointCount": 5,
            "currentIndex": null,
            "currentPercentile": null,
            "gaugeResponsePercentile": null,
            "waterTemperaturePercentile": null,
            "usableDays": 0,
            "expectedDays": 81,
            "coveragePercent": 0,
            "historicalYears": 0,
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14",
              "2026-11-15",
              "2026-11-16",
              "2026-11-17",
              "2026-11-18",
              "2026-11-19",
              "2026-11-20"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00",
              "2026-11-15": "16:00",
              "2026-11-16": "16:00",
              "2026-11-17": "16:00",
              "2026-11-18": "16:00",
              "2026-11-19": "16:00",
              "2026-11-20": "16:00"
            },
            "headline": "The early-season timing read is complete.",
            "tip": "Stop shifting river sections based on early-or-late timing. Begin in established holding water for the current Migration Stage; if Push is Possible or stronger, make lower travel lanes the next stop.",
            "detail": "There was not enough reliable season-long river and temperature information to make an early, normal, or late call.",
            "reasonCodes": [
              "conditions_baseline_missing",
              "conditions_insufficient",
              "conditions_checkpoint_peak_complete",
              "conditions_timing_complete"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "peak_start",
            "previousCheckpointDate": "2026-11-15",
            "previousTimingLabel": "Insufficient evidence"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-11-21",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-11-20",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-19",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-18",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-11-18T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-17",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-16",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-11-16T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-11-15",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-11-15T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-14",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": 80,
            "displayScore": 80,
            "scoreIsApproximate": false,
            "stage": "peak",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 1,
            "curveDirection": "near_peak",
            "winterHoldingContext": false,
            "label": "Peak presence",
            "headline": "Steelhead seasonal presence is peak presence and near its seasonal high.",
            "detail": "Calendar timing places Steelhead presence at its expected seasonal peak, near its expected seasonal high. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_peak",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-11-21T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-11-21T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-11-21T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "Seasonal presence is near peak, but today's water does not point to a fresh wave.",
            "detail": "Many fish may already be in the river even when rain, river level, and temperature show little support for new movement. Start in established deep holes, fish each one from head to tail, and leave lower entry lanes secondary.",
            "reasonCodes": [
              "peak_presence_weak_push"
            ]
          }
        }
      },
      {
        "id": "conditions_missing_baseline",
        "label": "Insufficient · no historical baseline",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-09-20",
          "progressionSnapshotAt": "2026-09-20T12:00:00.000Z",
          "conditionRefreshAt": "2026-09-20T20:00:00.000Z",
          "progressionExpiresAt": "2026-09-20T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-09-20T22:00:00.000Z",
          "runStage": {
            "stage": "beginning",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-09-20",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Beginning",
            "headline": "Seasonal timing supports the first Steelhead entering the Lower river.",
            "detail": "This is the opening river phase. Presence is expected to be scattered and uneven.",
            "tip": "Cover the Lower river first. Move upstream only after direct fish activity supports it.",
            "whereToStart": "Start in the Lower river (Pere Marquette Lake–Scottville).",
            "reasonCodes": [
              "stage_beginning"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Insufficient evidence",
            "timingLabel": "Insufficient evidence",
            "candidateLabel": "Insufficient evidence",
            "checkpointId": "river_start",
            "checkpointDate": "2026-09-20",
            "cutoffDate": "2026-09-19",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-10-11",
            "completedCheckpointCount": 1,
            "currentIndex": null,
            "currentPercentile": null,
            "gaugeResponsePercentile": null,
            "waterTemperaturePercentile": null,
            "usableDays": 0,
            "expectedDays": 19,
            "coveragePercent": 0,
            "historicalYears": 0,
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00"
            },
            "headline": "There is not enough reliable PM evidence for a Migration Timing call.",
            "tip": "Keep the PM section named by Migration Stage. Do not shift upstream or downstream from this timing read.",
            "detail": "There are not enough dependable past seasons to judge whether this migration is early, normal, or late.",
            "reasonCodes": [
              "conditions_baseline_missing",
              "conditions_insufficient",
              "conditions_checkpoint_river_start"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-09-20",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": []
          },
          "fishInRiver": {
            "score": 8,
            "displayScore": 10,
            "scoreIsApproximate": true,
            "stage": "beginning",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0.1,
            "curveDirection": "rising",
            "winterHoldingContext": false,
            "label": "Low presence",
            "headline": "Steelhead seasonal presence is low presence and building.",
            "detail": "Calendar timing places Steelhead presence at low presence, increasing as the fall migration builds. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_beginning",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-09-20T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "The river should fish well, but seasonal fish presence is still low.",
            "detail": "Good flow makes presentations easier, but it does not put fish in the river. Stay low, fish the first travel lane entering deep resting water, make one complete pass, and move instead of waiting for numbers that have not developed.",
            "reasonCodes": [
              "good_fishability_low_presence"
            ]
          }
        }
      },
      {
        "id": "conditions_few_years",
        "label": "Insufficient · fewer than five years",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-09-20",
          "progressionSnapshotAt": "2026-09-20T12:00:00.000Z",
          "conditionRefreshAt": "2026-09-20T20:00:00.000Z",
          "progressionExpiresAt": "2026-09-20T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-09-20T22:00:00.000Z",
          "runStage": {
            "stage": "beginning",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-09-20",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Beginning",
            "headline": "Seasonal timing supports the first Steelhead entering the Lower river.",
            "detail": "This is the opening river phase. Presence is expected to be scattered and uneven.",
            "tip": "Cover the Lower river first. Move upstream only after direct fish activity supports it.",
            "whereToStart": "Start in the Lower river (Pere Marquette Lake–Scottville).",
            "reasonCodes": [
              "stage_beginning"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Insufficient evidence",
            "timingLabel": "Insufficient evidence",
            "candidateLabel": "Insufficient evidence",
            "checkpointId": "river_start",
            "checkpointDate": "2026-09-20",
            "cutoffDate": "2026-09-19",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-10-11",
            "completedCheckpointCount": 1,
            "currentIndex": null,
            "currentPercentile": null,
            "gaugeResponsePercentile": null,
            "waterTemperaturePercentile": null,
            "usableDays": 19,
            "expectedDays": 19,
            "coveragePercent": 1,
            "historicalYears": 4,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00"
            },
            "headline": "There is not enough reliable PM evidence for a Migration Timing call.",
            "tip": "Keep the PM section named by Migration Stage. Do not shift upstream or downstream from this timing read.",
            "detail": "There are not enough dependable past seasons to judge whether this migration is early, normal, or late.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_baseline_insufficient_years",
              "conditions_insufficient",
              "conditions_checkpoint_river_start"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-09-20",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": []
          },
          "fishInRiver": {
            "score": 8,
            "displayScore": 10,
            "scoreIsApproximate": true,
            "stage": "beginning",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0.1,
            "curveDirection": "rising",
            "winterHoldingContext": false,
            "label": "Low presence",
            "headline": "Steelhead seasonal presence is low presence and building.",
            "detail": "Calendar timing places Steelhead presence at low presence, increasing as the fall migration builds. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_beginning",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-09-20T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "The river should fish well, but seasonal fish presence is still low.",
            "detail": "Good flow makes presentations easier, but it does not put fish in the river. Stay low, fish the first travel lane entering deep resting water, make one complete pass, and move instead of waiting for numbers that have not developed.",
            "reasonCodes": [
              "good_fishability_low_presence"
            ]
          }
        }
      },
      {
        "id": "conditions_version_mismatch",
        "label": "Insufficient · baseline version mismatch",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-09-20",
          "progressionSnapshotAt": "2026-09-20T12:00:00.000Z",
          "conditionRefreshAt": "2026-09-20T20:00:00.000Z",
          "progressionExpiresAt": "2026-09-20T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-09-20T22:00:00.000Z",
          "runStage": {
            "stage": "beginning",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-09-20",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Beginning",
            "headline": "Seasonal timing supports the first Steelhead entering the Lower river.",
            "detail": "This is the opening river phase. Presence is expected to be scattered and uneven.",
            "tip": "Cover the Lower river first. Move upstream only after direct fish activity supports it.",
            "whereToStart": "Start in the Lower river (Pere Marquette Lake–Scottville).",
            "reasonCodes": [
              "stage_beginning"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Insufficient evidence",
            "timingLabel": "Insufficient evidence",
            "candidateLabel": "Insufficient evidence",
            "checkpointId": "river_start",
            "checkpointDate": "2026-09-20",
            "cutoffDate": "2026-09-19",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-10-11",
            "completedCheckpointCount": 1,
            "currentIndex": null,
            "currentPercentile": null,
            "gaugeResponsePercentile": null,
            "waterTemperaturePercentile": null,
            "usableDays": 19,
            "expectedDays": 19,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "old-version",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00"
            },
            "headline": "There is not enough reliable PM evidence for a Migration Timing call.",
            "tip": "Keep the PM section named by Migration Stage. Do not shift upstream or downstream from this timing read.",
            "detail": "Some of the long-term comparison data does not line up cleanly with this season, so showing a timing label would be misleading.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_baseline_version_mismatch",
              "conditions_insufficient",
              "conditions_checkpoint_river_start"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-09-20",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": []
          },
          "fishInRiver": {
            "score": 8,
            "displayScore": 10,
            "scoreIsApproximate": true,
            "stage": "beginning",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0.1,
            "curveDirection": "rising",
            "winterHoldingContext": false,
            "label": "Low presence",
            "headline": "Steelhead seasonal presence is low presence and building.",
            "detail": "Calendar timing places Steelhead presence at low presence, increasing as the fall migration builds. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_beginning",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-09-20T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "The river should fish well, but seasonal fish presence is still low.",
            "detail": "Good flow makes presentations easier, but it does not put fish in the river. Stay low, fish the first travel lane entering deep resting water, make one complete pass, and move instead of waiting for numbers that have not developed.",
            "reasonCodes": [
              "good_fishability_low_presence"
            ]
          }
        }
      },
      {
        "id": "conditions_window_mismatch",
        "label": "Insufficient · window mismatch",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-09-20",
          "progressionSnapshotAt": "2026-09-20T12:00:00.000Z",
          "conditionRefreshAt": "2026-09-20T20:00:00.000Z",
          "progressionExpiresAt": "2026-09-20T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-09-20T22:00:00.000Z",
          "runStage": {
            "stage": "beginning",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-09-20",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Beginning",
            "headline": "Seasonal timing supports the first Steelhead entering the Lower river.",
            "detail": "This is the opening river phase. Presence is expected to be scattered and uneven.",
            "tip": "Cover the Lower river first. Move upstream only after direct fish activity supports it.",
            "whereToStart": "Start in the Lower river (Pere Marquette Lake–Scottville).",
            "reasonCodes": [
              "stage_beginning"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Insufficient evidence",
            "timingLabel": "Insufficient evidence",
            "candidateLabel": "Insufficient evidence",
            "checkpointId": "river_start",
            "checkpointDate": "2026-09-20",
            "cutoffDate": "2026-09-19",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-10-11",
            "completedCheckpointCount": 1,
            "currentIndex": null,
            "currentPercentile": null,
            "gaugeResponsePercentile": null,
            "waterTemperaturePercentile": null,
            "usableDays": 19,
            "expectedDays": 19,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00"
            },
            "headline": "There is not enough reliable PM evidence for a Migration Timing call.",
            "tip": "Keep the PM section named by Migration Stage. Do not shift upstream or downstream from this timing read.",
            "detail": "Some of the long-term comparison data does not line up cleanly with this season, so showing a timing label would be misleading.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_baseline_window_mismatch",
              "conditions_insufficient",
              "conditions_checkpoint_river_start"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-09-20",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": []
          },
          "fishInRiver": {
            "score": 8,
            "displayScore": 10,
            "scoreIsApproximate": true,
            "stage": "beginning",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0.1,
            "curveDirection": "rising",
            "winterHoldingContext": false,
            "label": "Low presence",
            "headline": "Steelhead seasonal presence is low presence and building.",
            "detail": "Calendar timing places Steelhead presence at low presence, increasing as the fall migration builds. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_beginning",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-09-20T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "The river should fish well, but seasonal fish presence is still low.",
            "detail": "Good flow makes presentations easier, but it does not put fish in the river. Stay low, fish the first travel lane entering deep resting water, make one complete pass, and move instead of waiting for numbers that have not developed.",
            "reasonCodes": [
              "good_fishability_low_presence"
            ]
          }
        }
      },
      {
        "id": "conditions_missing_gauge",
        "label": "Insufficient · checkpoint gauge missing",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-09-20",
          "progressionSnapshotAt": "2026-09-20T12:00:00.000Z",
          "conditionRefreshAt": "2026-09-20T20:00:00.000Z",
          "progressionExpiresAt": "2026-09-20T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-09-20T22:00:00.000Z",
          "runStage": {
            "stage": "beginning",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-09-20",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Beginning",
            "headline": "Seasonal timing supports the first Steelhead entering the Lower river.",
            "detail": "This is the opening river phase. Presence is expected to be scattered and uneven.",
            "tip": "Cover the Lower river first. Move upstream only after direct fish activity supports it.",
            "whereToStart": "Start in the Lower river (Pere Marquette Lake–Scottville).",
            "reasonCodes": [
              "stage_beginning"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Insufficient evidence",
            "timingLabel": "Insufficient evidence",
            "candidateLabel": "Insufficient evidence",
            "checkpointId": "river_start",
            "checkpointDate": "2026-09-20",
            "cutoffDate": "2026-09-19",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-10-11",
            "completedCheckpointCount": 1,
            "currentIndex": null,
            "currentPercentile": null,
            "gaugeResponsePercentile": null,
            "waterTemperaturePercentile": null,
            "usableDays": 18,
            "expectedDays": 19,
            "coveragePercent": 0.947,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00"
            },
            "headline": "There is not enough reliable PM evidence for a Migration Timing call.",
            "tip": "Keep the PM section named by Migration Stage. Do not shift upstream or downstream from this timing read.",
            "detail": "A dependable river-level reading was missing when this seasonal timing read needed it.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_missing_checkpoint_gauge",
              "conditions_insufficient",
              "conditions_checkpoint_river_start"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-09-20",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": []
          },
          "fishInRiver": {
            "score": 8,
            "displayScore": 10,
            "scoreIsApproximate": true,
            "stage": "beginning",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0.1,
            "curveDirection": "rising",
            "winterHoldingContext": false,
            "label": "Low presence",
            "headline": "Steelhead seasonal presence is low presence and building.",
            "detail": "Calendar timing places Steelhead presence at low presence, increasing as the fall migration builds. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_beginning",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-09-20T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "The river should fish well, but seasonal fish presence is still low.",
            "detail": "Good flow makes presentations easier, but it does not put fish in the river. Stay low, fish the first travel lane entering deep resting water, make one complete pass, and move instead of waiting for numbers that have not developed.",
            "reasonCodes": [
              "good_fishability_low_presence"
            ]
          }
        }
      },
      {
        "id": "conditions_missing_temperature",
        "label": "Insufficient · checkpoint temperature missing",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-09-20",
          "progressionSnapshotAt": "2026-09-20T12:00:00.000Z",
          "conditionRefreshAt": "2026-09-20T20:00:00.000Z",
          "progressionExpiresAt": "2026-09-20T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-09-20T22:00:00.000Z",
          "runStage": {
            "stage": "beginning",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-09-20",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Beginning",
            "headline": "Seasonal timing supports the first Steelhead entering the Lower river.",
            "detail": "This is the opening river phase. Presence is expected to be scattered and uneven.",
            "tip": "Cover the Lower river first. Move upstream only after direct fish activity supports it.",
            "whereToStart": "Start in the Lower river (Pere Marquette Lake–Scottville).",
            "reasonCodes": [
              "stage_beginning"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Insufficient evidence",
            "timingLabel": "Insufficient evidence",
            "candidateLabel": "Insufficient evidence",
            "checkpointId": "river_start",
            "checkpointDate": "2026-09-20",
            "cutoffDate": "2026-09-19",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-10-11",
            "completedCheckpointCount": 1,
            "currentIndex": null,
            "currentPercentile": null,
            "gaugeResponsePercentile": null,
            "waterTemperaturePercentile": null,
            "usableDays": 18,
            "expectedDays": 19,
            "coveragePercent": 0.947,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00"
            },
            "headline": "There is not enough reliable PM evidence for a Migration Timing call.",
            "tip": "Keep the PM section named by Migration Stage. Do not shift upstream or downstream from this timing read.",
            "detail": "A dependable water-temperature reading was missing when this seasonal timing read needed it.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_missing_checkpoint_temperature",
              "conditions_insufficient",
              "conditions_checkpoint_river_start"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-09-20",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": []
          },
          "fishInRiver": {
            "score": 8,
            "displayScore": 10,
            "scoreIsApproximate": true,
            "stage": "beginning",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0.1,
            "curveDirection": "rising",
            "winterHoldingContext": false,
            "label": "Low presence",
            "headline": "Steelhead seasonal presence is low presence and building.",
            "detail": "Calendar timing places Steelhead presence at low presence, increasing as the fall migration builds. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_beginning",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-09-20T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "The river should fish well, but seasonal fish presence is still low.",
            "detail": "Good flow makes presentations easier, but it does not put fish in the river. Stay low, fish the first travel lane entering deep resting water, make one complete pass, and move instead of waiting for numbers that have not developed.",
            "reasonCodes": [
              "good_fishability_low_presence"
            ]
          }
        }
      },
      {
        "id": "conditions_source_mismatch",
        "label": "Insufficient · source mismatch",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-09-20",
          "progressionSnapshotAt": "2026-09-20T12:00:00.000Z",
          "conditionRefreshAt": "2026-09-20T20:00:00.000Z",
          "progressionExpiresAt": "2026-09-20T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-09-20T22:00:00.000Z",
          "runStage": {
            "stage": "beginning",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-09-20",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Beginning",
            "headline": "Seasonal timing supports the first Steelhead entering the Lower river.",
            "detail": "This is the opening river phase. Presence is expected to be scattered and uneven.",
            "tip": "Cover the Lower river first. Move upstream only after direct fish activity supports it.",
            "whereToStart": "Start in the Lower river (Pere Marquette Lake–Scottville).",
            "reasonCodes": [
              "stage_beginning"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Insufficient evidence",
            "timingLabel": "Insufficient evidence",
            "candidateLabel": "Insufficient evidence",
            "checkpointId": "river_start",
            "checkpointDate": "2026-09-20",
            "cutoffDate": "2026-09-19",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-10-11",
            "completedCheckpointCount": 1,
            "currentIndex": null,
            "currentPercentile": null,
            "gaugeResponsePercentile": null,
            "waterTemperaturePercentile": null,
            "usableDays": 18,
            "expectedDays": 19,
            "coveragePercent": 0.947,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00"
            },
            "headline": "There is not enough reliable PM evidence for a Migration Timing call.",
            "tip": "Keep the PM section named by Migration Stage. Do not shift upstream or downstream from this timing read.",
            "detail": "The available river and temperature readings do not line up cleanly with the long-term comparison, so the result stays blank.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_source_mismatch",
              "conditions_insufficient",
              "conditions_checkpoint_river_start"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-09-20",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": []
          },
          "fishInRiver": {
            "score": 8,
            "displayScore": 10,
            "scoreIsApproximate": true,
            "stage": "beginning",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0.1,
            "curveDirection": "rising",
            "winterHoldingContext": false,
            "label": "Low presence",
            "headline": "Steelhead seasonal presence is low presence and building.",
            "detail": "Calendar timing places Steelhead presence at low presence, increasing as the fall migration builds. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_beginning",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-09-20T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "The river should fish well, but seasonal fish presence is still low.",
            "detail": "Good flow makes presentations easier, but it does not put fish in the river. Stay low, fish the first travel lane entering deep resting water, make one complete pass, and move instead of waiting for numbers that have not developed.",
            "reasonCodes": [
              "good_fishability_low_presence"
            ]
          }
        }
      },
      {
        "id": "conditions_limited_days",
        "label": "Insufficient · limited matching days",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-09-20",
          "progressionSnapshotAt": "2026-09-20T12:00:00.000Z",
          "conditionRefreshAt": "2026-09-20T20:00:00.000Z",
          "progressionExpiresAt": "2026-09-20T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-09-20T22:00:00.000Z",
          "runStage": {
            "stage": "beginning",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-09-20",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Beginning",
            "headline": "Seasonal timing supports the first Steelhead entering the Lower river.",
            "detail": "This is the opening river phase. Presence is expected to be scattered and uneven.",
            "tip": "Cover the Lower river first. Move upstream only after direct fish activity supports it.",
            "whereToStart": "Start in the Lower river (Pere Marquette Lake–Scottville).",
            "reasonCodes": [
              "stage_beginning"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Insufficient evidence",
            "timingLabel": "Insufficient evidence",
            "candidateLabel": "Insufficient evidence",
            "checkpointId": "river_start",
            "checkpointDate": "2026-09-20",
            "cutoffDate": "2026-09-19",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-10-11",
            "completedCheckpointCount": 1,
            "currentIndex": null,
            "currentPercentile": null,
            "gaugeResponsePercentile": null,
            "waterTemperaturePercentile": null,
            "usableDays": 4,
            "expectedDays": 19,
            "coveragePercent": 0.211,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-19": "16:00"
            },
            "headline": "There is not enough reliable PM evidence for a Migration Timing call.",
            "tip": "Keep the PM section named by Migration Stage. Do not shift upstream or downstream from this timing read.",
            "detail": "Too many days are missing either river level or water temperature to make a dependable seasonal comparison.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_limited_source_days",
              "conditions_insufficient",
              "conditions_checkpoint_river_start"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-09-20",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": []
          },
          "fishInRiver": {
            "score": 8,
            "displayScore": 10,
            "scoreIsApproximate": true,
            "stage": "beginning",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0.1,
            "curveDirection": "rising",
            "winterHoldingContext": false,
            "label": "Low presence",
            "headline": "Steelhead seasonal presence is low presence and building.",
            "detail": "Calendar timing places Steelhead presence at low presence, increasing as the fall migration builds. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_beginning",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-09-20T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "The river should fish well, but seasonal fish presence is still low.",
            "detail": "Good flow makes presentations easier, but it does not put fish in the river. Stay low, fish the first travel lane entering deep resting water, make one complete pass, and move instead of waiting for numbers that have not developed.",
            "reasonCodes": [
              "good_fishability_low_presence"
            ]
          }
        }
      }
    ]
  },
  {
    "id": "push",
    "label": "Push",
    "scenarios": [
      {
        "id": "push_weak",
        "label": "Weak · warm, dry, falling",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "push": {
            "score": 0,
            "label": "Weak",
            "headline": "PM water shows little support for fresh movement.",
            "detail": "Scottville flow is falling. Measured water temperature is too warm for strong movement support but warming quickly. Recent watershed weather shows little rain.",
            "tip": "Keep the section named by Migration Stage. Leave fresh-entry travel water secondary until measured temperature improves.",
            "reasonCodes": [
              "gauge_fresh",
              "push_normal_flow_context",
              "push_temperature_too_warm",
              "dry_72h",
              "push_no_gauge_response_cap",
              "temperature_too_warm_cap"
            ],
            "components": {
              "hydraulicBase": 20,
              "hydraulicAdjustment": 0,
              "temperatureModifier": -18,
              "rainModifier": -5,
              "hydraulicState": "normal",
              "temperatureState": "too_warm",
              "rainRole": "dry",
              "appliedCaps": []
            },
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "push_no_clear",
        "label": "No clear push · steady",
        "note": "Canonical production copy",
        "snapshotOverride": {}
      },
      {
        "id": "push_possible",
        "label": "Possible · early response",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "push": {
            "score": 61,
            "label": "Possible",
            "headline": "PM water offers some support for fresh movement.",
            "detail": "Scottville flow has started to rise. Measured water temperature is favorable for this migration and cooling. Rain adds limited support while Scottville begins to rise.",
            "tip": "Keep Migration Stage’s section primary. Add one Lower river travel-water check before returning to established holding water.",
            "reasonCodes": [
              "gauge_fresh",
              "push_normal_flow_context",
              "push_temperature_supportive",
              "push_rain_partial_precursor"
            ],
            "components": {
              "hydraulicBase": 52,
              "hydraulicAdjustment": 0,
              "temperatureModifier": 6,
              "rainModifier": 3,
              "hydraulicState": "normal",
              "temperatureState": "supportive",
              "rainRole": "partial_precursor",
              "appliedCaps": []
            },
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "active_now",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-10-15",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-10-14",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-13",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-12",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-10-12T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-11",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-10",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-10-10T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-10-09",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-10-09T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-08",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ],
            "lastSupportiveConditions": {
              "localDate": "2026-10-15",
              "refreshSlot": "16:00",
              "conditionRefreshAt": "2026-10-15T20:00:00.000Z",
              "score": 61,
              "label": "Possible"
            }
          }
        }
      },
      {
        "id": "push_strong",
        "label": "Strong · meaningful rise",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "push": {
            "score": 76,
            "label": "Strong",
            "headline": "PM water strongly supports possible fresh movement.",
            "detail": "Scottville flow has made a clear rise. Measured water temperature is favorable for this migration and cooling. Scottville already reflects the rain response, so rain adds no extra credit.",
            "tip": "Use Lower river travel water as the fresh-movement check, then return to the section named by Migration Stage.",
            "reasonCodes": [
              "gauge_fresh",
              "push_normal_flow_context",
              "push_temperature_supportive",
              "push_rain_absorbed_by_gauge"
            ],
            "components": {
              "hydraulicBase": 70,
              "hydraulicAdjustment": 0,
              "temperatureModifier": 6,
              "rainModifier": 0,
              "hydraulicState": "normal",
              "temperatureState": "supportive",
              "rainRole": "absorbed_by_gauge",
              "appliedCaps": []
            },
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "active_now",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-10-15",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-10-14",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-13",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-12",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-10-12T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-11",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-10",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-10-10T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-10-09",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-10-09T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-08",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ],
            "lastSupportiveConditions": {
              "localDate": "2026-10-15",
              "refreshSlot": "16:00",
              "conditionRefreshAt": "2026-10-15T20:00:00.000Z",
              "score": 76,
              "label": "Strong"
            }
          }
        }
      },
      {
        "id": "push_very_strong",
        "label": "Very strong · sharp rise",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "push": {
            "score": 90,
            "label": "Very strong",
            "headline": "PM water offers its strongest support for fresh movement.",
            "detail": "Scottville flow is rising quickly. Measured water temperature is favorable for this migration and cooling quickly. Scottville already reflects the rain response, so rain adds no extra credit.",
            "tip": "Use Lower river travel water as the fresh-movement check, then return to the section named by Migration Stage.",
            "reasonCodes": [
              "gauge_fresh",
              "push_normal_flow_context",
              "push_temperature_supportive",
              "push_rain_absorbed_by_gauge"
            ],
            "components": {
              "hydraulicBase": 80,
              "hydraulicAdjustment": 0,
              "temperatureModifier": 10,
              "rainModifier": 0,
              "hydraulicState": "normal",
              "temperatureState": "supportive",
              "rainRole": "absorbed_by_gauge",
              "appliedCaps": []
            },
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "active_now",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-10-15",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-10-14",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-13",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-12",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-10-12T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-11",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-10",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-10-10T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-10-09",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-10-09T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-08",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ],
            "lastSupportiveConditions": {
              "localDate": "2026-10-15",
              "refreshSlot": "16:00",
              "conditionRefreshAt": "2026-10-15T20:00:00.000Z",
              "score": 90,
              "label": "Very strong"
            }
          }
        }
      },
      {
        "id": "push_rain_precursor",
        "label": "Rain · early precursor",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "push": {
            "score": 51,
            "label": "Possible",
            "headline": "PM water offers some support for fresh movement.",
            "detail": "Scottville flow is steady without a meaningful rise. Measured water temperature is favorable for this migration and cooling. Rain is only a precursor because Scottville has not shown enough response.",
            "tip": "Keep Migration Stage’s section primary. Add one Lower river travel-water check before returning to established holding water.",
            "reasonCodes": [
              "gauge_fresh",
              "push_normal_flow_context",
              "push_temperature_supportive",
              "push_rain_precursor",
              "push_no_gauge_response_cap"
            ],
            "components": {
              "hydraulicBase": 35,
              "hydraulicAdjustment": 0,
              "temperatureModifier": 6,
              "rainModifier": 10,
              "hydraulicState": "normal",
              "temperatureState": "supportive",
              "rainRole": "precursor",
              "appliedCaps": []
            },
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "active_now",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-10-15",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-10-14",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-13",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-12",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-10-12T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-11",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-10",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-10-10T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-10-09",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-10-09T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-08",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ],
            "lastSupportiveConditions": {
              "localDate": "2026-10-15",
              "refreshSlot": "16:00",
              "conditionRefreshAt": "2026-10-15T20:00:00.000Z",
              "score": 51,
              "label": "Possible"
            }
          }
        }
      },
      {
        "id": "push_rain_absorbed",
        "label": "Rain · reflected in gauge",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "push": {
            "score": 70,
            "label": "Strong",
            "headline": "PM water strongly supports possible fresh movement.",
            "detail": "Scottville flow has made a clear rise. Measured water temperature is favorable for this migration. Scottville already reflects the rain response, so rain adds no extra credit.",
            "tip": "Use Lower river travel water as the fresh-movement check, then return to the section named by Migration Stage.",
            "reasonCodes": [
              "gauge_fresh",
              "push_normal_flow_context",
              "push_temperature_supportive",
              "push_rain_absorbed_by_gauge"
            ],
            "components": {
              "hydraulicBase": 70,
              "hydraulicAdjustment": 0,
              "temperatureModifier": 0,
              "rainModifier": 0,
              "hydraulicState": "normal",
              "temperatureState": "supportive",
              "rainRole": "absorbed_by_gauge",
              "appliedCaps": []
            },
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "active_now",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-10-15",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-10-14",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-13",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-12",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-10-12T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-11",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-10",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-10-10T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-10-09",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-10-09T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-08",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ],
            "lastSupportiveConditions": {
              "localDate": "2026-10-15",
              "refreshSlot": "16:00",
              "conditionRefreshAt": "2026-10-15T20:00:00.000Z",
              "score": 70,
              "label": "Strong"
            }
          }
        }
      },
      {
        "id": "push_rain_suppressed",
        "label": "Rain · river already high",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "push": {
            "score": 47,
            "label": "No clear push",
            "headline": "PM water does not show a clear fresh-movement signal.",
            "detail": "Scottville flow has started to rise, while overall flow is high. Measured water temperature is favorable for this migration. Additional rain adds no support while Scottville flow is already high.",
            "tip": "Keep the section named by Migration Stage. Do not shift lower for a fresh wave this water does not support.",
            "reasonCodes": [
              "gauge_fresh",
              "push_high_flow_context",
              "push_temperature_supportive",
              "push_rain_suppressed_high_flow"
            ],
            "components": {
              "hydraulicBase": 52,
              "hydraulicAdjustment": -5,
              "temperatureModifier": 0,
              "rainModifier": 0,
              "hydraulicState": "high",
              "temperatureState": "supportive",
              "rainRole": "suppressed_high_flow",
              "appliedCaps": []
            },
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "push_rain_missing",
        "label": "Rain · unavailable",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "push": {
            "score": 35,
            "label": "No clear push",
            "headline": "PM water does not show a clear fresh-movement signal.",
            "detail": "Scottville flow is steady without a meaningful rise. Measured water temperature is favorable for this migration. Watershed rainfall data is unavailable and adds no confidence.",
            "tip": "Keep the section named by Migration Stage. Do not shift lower for a fresh wave this water does not support.",
            "reasonCodes": [
              "gauge_fresh",
              "push_normal_flow_context",
              "push_temperature_supportive",
              "rain_missing",
              "push_no_gauge_response_cap"
            ],
            "components": {
              "hydraulicBase": 35,
              "hydraulicAdjustment": 0,
              "temperatureModifier": 0,
              "rainModifier": 0,
              "hydraulicState": "normal",
              "temperatureState": "supportive",
              "rainRole": "missing",
              "appliedCaps": []
            },
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "push_temperature_transitional",
        "label": "Temperature · transitional warm",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "push": {
            "score": 75,
            "label": "Strong",
            "headline": "PM water strongly supports possible fresh movement.",
            "detail": "Scottville flow has made a clear rise. Measured water temperature is warmer than preferred and cooling. Recent watershed rainfall is too light to affect the read.",
            "tip": "Use Lower river travel water as the fresh-movement check, then return to the section named by Migration Stage.",
            "reasonCodes": [
              "gauge_fresh",
              "push_normal_flow_context",
              "push_temperature_transitional",
              "light_rain_48h"
            ],
            "components": {
              "hydraulicBase": 70,
              "hydraulicAdjustment": 0,
              "temperatureModifier": 5,
              "rainModifier": 0,
              "hydraulicState": "normal",
              "temperatureState": "transitional_warm",
              "rainRole": "neutral",
              "appliedCaps": []
            },
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "active_now",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-10-15",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-10-14",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-13",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-12",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-10-12T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-11",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-10",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-10-10T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-10-09",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-10-09T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-08",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ],
            "lastSupportiveConditions": {
              "localDate": "2026-10-15",
              "refreshSlot": "16:00",
              "conditionRefreshAt": "2026-10-15T20:00:00.000Z",
              "score": 75,
              "label": "Strong"
            }
          }
        }
      },
      {
        "id": "push_temperature_too_warm",
        "label": "Temperature · too warm",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "push": {
            "score": 62,
            "label": "Possible",
            "headline": "PM water offers some support for fresh movement.",
            "detail": "Scottville flow has made a clear rise. Measured water temperature is too warm for strong movement support and cooling. Recent watershed rainfall is too light to affect the read.",
            "tip": "Keep the section named by Migration Stage. Leave fresh-entry travel water secondary until measured temperature improves.",
            "reasonCodes": [
              "gauge_fresh",
              "push_normal_flow_context",
              "push_temperature_too_warm",
              "light_rain_48h",
              "temperature_too_warm_cap"
            ],
            "components": {
              "hydraulicBase": 70,
              "hydraulicAdjustment": 0,
              "temperatureModifier": -8,
              "rainModifier": 0,
              "hydraulicState": "normal",
              "temperatureState": "too_warm",
              "rainRole": "neutral",
              "appliedCaps": []
            },
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "active_now",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-10-15",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-10-14",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-13",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-12",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-10-12T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-11",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-10",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-10-10T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-10-09",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-10-09T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-08",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ],
            "lastSupportiveConditions": {
              "localDate": "2026-10-15",
              "refreshSlot": "16:00",
              "conditionRefreshAt": "2026-10-15T20:00:00.000Z",
              "score": 62,
              "label": "Possible"
            }
          }
        }
      },
      {
        "id": "push_temperature_barrier",
        "label": "Temperature · warm barrier",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "push": {
            "score": 49,
            "label": "No clear push",
            "headline": "Warm PM water prevents a dependable fresh-movement signal.",
            "detail": "Scottville flow is rising quickly. Measured water temperature is warm enough to block a dependable movement call and cooling quickly. Recent watershed rainfall is too light to affect the read.",
            "tip": "Keep the section named by Migration Stage. Leave fresh-entry travel water secondary until measured temperature improves.",
            "reasonCodes": [
              "gauge_fresh",
              "push_normal_flow_context",
              "push_temperature_migration_barrier",
              "light_rain_48h",
              "push_temperature_barrier_cap"
            ],
            "components": {
              "hydraulicBase": 80,
              "hydraulicAdjustment": 0,
              "temperatureModifier": -20,
              "rainModifier": 0,
              "hydraulicState": "normal",
              "temperatureState": "migration_barrier",
              "rainRole": "neutral",
              "appliedCaps": [
                49
              ]
            },
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "push_temperature_plateau",
        "label": "Temperature · cool plateau",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "push": {
            "score": 49,
            "label": "No clear push",
            "headline": "Cold PM water limits confidence in continued fall entry.",
            "detail": "Scottville flow has made a clear rise. Measured water temperature is cold enough to favor holding over active fall entry and cooling quickly. Recent watershed rainfall is too light to affect the read.",
            "tip": "Keep the section named by Migration Stage. Do not shift lower for a fresh wave this water does not support.",
            "reasonCodes": [
              "gauge_fresh",
              "push_normal_flow_context",
              "push_temperature_cold_holding",
              "light_rain_48h",
              "push_cold_holding_cap"
            ],
            "components": {
              "hydraulicBase": 70,
              "hydraulicAdjustment": 0,
              "temperatureModifier": -15,
              "rainModifier": 0,
              "hydraulicState": "normal",
              "temperatureState": "cold_holding",
              "rainRole": "neutral",
              "appliedCaps": [
                49
              ]
            },
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "push_temperature_cold_active",
        "label": "Temperature · cold but active",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "push": {
            "score": 71,
            "label": "Strong",
            "headline": "PM water strongly supports possible fresh movement.",
            "detail": "Scottville flow has made a clear rise. Measured water temperature is cold but still compatible with fall Steelhead movement. Recent watershed rainfall is too light to affect the read.",
            "tip": "Use Lower river travel water as the fresh-movement check, then return to the section named by Migration Stage.",
            "reasonCodes": [
              "gauge_fresh",
              "push_normal_flow_context",
              "push_temperature_cold_active",
              "light_rain_48h"
            ],
            "components": {
              "hydraulicBase": 70,
              "hydraulicAdjustment": 0,
              "temperatureModifier": 1,
              "rainModifier": 0,
              "hydraulicState": "normal",
              "temperatureState": "cold_active",
              "rainRole": "neutral",
              "appliedCaps": []
            },
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "active_now",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-10-15",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-10-14",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-13",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-12",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-10-12T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-11",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-10",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-10-10T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-10-09",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-10-09T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-08",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ],
            "lastSupportiveConditions": {
              "localDate": "2026-10-15",
              "refreshSlot": "16:00",
              "conditionRefreshAt": "2026-10-15T20:00:00.000Z",
              "score": 71,
              "label": "Strong"
            }
          }
        }
      },
      {
        "id": "push_temperature_cold_holding",
        "label": "Temperature · cold holding",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "push": {
            "score": 49,
            "label": "No clear push",
            "headline": "Cold PM water limits confidence in continued fall entry.",
            "detail": "Scottville flow has made a clear rise. Measured water temperature is cold enough to favor holding over active fall entry and cooling quickly. Recent watershed rainfall is too light to affect the read.",
            "tip": "Keep the section named by Migration Stage. Do not shift lower for a fresh wave this water does not support.",
            "reasonCodes": [
              "gauge_fresh",
              "push_normal_flow_context",
              "push_temperature_cold_holding",
              "light_rain_48h",
              "push_cold_holding_cap"
            ],
            "components": {
              "hydraulicBase": 70,
              "hydraulicAdjustment": 0,
              "temperatureModifier": -15,
              "rainModifier": 0,
              "hydraulicState": "normal",
              "temperatureState": "cold_holding",
              "rainRole": "neutral",
              "appliedCaps": [
                49
              ]
            },
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "push_upstream_no_credit",
        "label": "Temperature · upstream fallback",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "push": {
            "score": 70,
            "label": "Strong",
            "headline": "PM water strongly supports possible fresh movement.",
            "detail": "Scottville flow has made a clear rise. Measured water temperature is favorable for this migration and cooling quickly. The temperature source is upstream, so its cooling cannot add positive lower-river credit.",
            "tip": "Use Lower river travel water as the fresh-movement check, then return to the section named by Migration Stage.",
            "reasonCodes": [
              "gauge_fresh",
              "push_normal_flow_context",
              "push_temperature_supportive",
              "light_rain_48h"
            ],
            "components": {
              "hydraulicBase": 70,
              "hydraulicAdjustment": 0,
              "temperatureModifier": 0,
              "rainModifier": 0,
              "hydraulicState": "normal",
              "temperatureState": "supportive",
              "rainRole": "neutral",
              "appliedCaps": []
            },
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "active_now",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-10-15",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-10-14",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-13",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-12",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-10-12T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-11",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-10",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-10-10T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-10-09",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-10-09T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-08",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ],
            "lastSupportiveConditions": {
              "localDate": "2026-10-15",
              "refreshSlot": "16:00",
              "conditionRefreshAt": "2026-10-15T20:00:00.000Z",
              "score": 70,
              "label": "Strong"
            }
          }
        }
      },
      {
        "id": "push_unknown_trend",
        "label": "Gauge trend · unresolved",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "push": {
            "score": 46,
            "label": "No clear push",
            "headline": "PM water does not show a clear fresh-movement signal.",
            "detail": "Scottville flow does not have a dependable recent trend. Measured water temperature is favorable for this migration and cooling. Without a dependable Scottville trend, the model cannot call a clear Push.",
            "tip": "Keep the section named by Migration Stage. Do not shift lower for a fresh wave this water does not support.",
            "reasonCodes": [
              "gauge_fresh",
              "push_normal_flow_context",
              "push_temperature_supportive",
              "push_rain_precursor",
              "push_no_gauge_response_cap",
              "push_unknown_trend_cap"
            ],
            "components": {
              "hydraulicBase": 30,
              "hydraulicAdjustment": 0,
              "temperatureModifier": 6,
              "rainModifier": 10,
              "hydraulicState": "normal",
              "temperatureState": "supportive",
              "rainRole": "precursor",
              "appliedCaps": []
            },
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "push_stale_cap",
        "label": "Gauge · stale",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "push": {
            "score": 55,
            "label": "Possible",
            "headline": "An aging Scottville reading limits confidence in PM fresh movement.",
            "detail": "Scottville flow is rising quickly. Measured water temperature is favorable for this migration and cooling quickly. The latest Scottville reading is aging, so confidence is reduced.",
            "tip": "Keep Migration Stage’s section primary. Add one Lower river travel-water check before returning to established holding water.",
            "reasonCodes": [
              "gauge_stale",
              "push_normal_flow_context",
              "push_temperature_supportive",
              "push_rain_absorbed_by_gauge",
              "push_stale_gauge_cap"
            ],
            "components": {
              "hydraulicBase": 80,
              "hydraulicAdjustment": 0,
              "temperatureModifier": 10,
              "rainModifier": 0,
              "hydraulicState": "normal",
              "temperatureState": "supportive",
              "rainRole": "absorbed_by_gauge",
              "appliedCaps": [
                55
              ]
            },
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "active_now",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-10-15",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-10-14",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-13",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-12",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-10-12T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-11",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-10",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-10-10T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-10-09",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-10-09T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-08",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ],
            "lastSupportiveConditions": {
              "localDate": "2026-10-15",
              "refreshSlot": "16:00",
              "conditionRefreshAt": "2026-10-15T20:00:00.000Z",
              "score": 55,
              "label": "Possible"
            }
          }
        }
      },
      {
        "id": "push_severe_high",
        "label": "Gauge · severe high cap",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "push": {
            "score": 49,
            "label": "No clear push",
            "headline": "Extreme Scottville flow prevents a dependable fresh-movement signal.",
            "detail": "Scottville flow is rising quickly, while overall flow is extreme. Measured water temperature is favorable for this migration and cooling quickly. Additional rain adds no support while Scottville flow is already high.",
            "tip": "Do not chase a movement event. Let Fishability determine whether the Scottville reach offers any practical presentation water.",
            "reasonCodes": [
              "gauge_fresh",
              "push_severe_high_flow_context",
              "push_temperature_supportive",
              "push_rain_suppressed_high_flow",
              "push_severe_high_flow_cap"
            ],
            "components": {
              "hydraulicBase": 80,
              "hydraulicAdjustment": -5,
              "temperatureModifier": 10,
              "rainModifier": 0,
              "hydraulicState": "severe_high",
              "temperatureState": "supportive",
              "rainRole": "suppressed_high_flow",
              "appliedCaps": [
                49
              ]
            },
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "push_unavailable_gauge",
        "label": "Unavailable · gauge",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "push": {
            "score": null,
            "label": "Unavailable",
            "headline": "A current Scottville flow reading is unavailable.",
            "detail": "Without Scottville flow and direction, rain cannot produce a dependable PM fresh-movement read.",
            "tip": "Keep the section named by Migration Stage. Do not treat recent rain as proof of movement.",
            "reasonCodes": [
              "gauge_missing"
            ],
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "push_unavailable_temperature",
        "label": "Unavailable · water temperature",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "push": {
            "score": null,
            "label": "Unavailable",
            "headline": "A current measured PM water temperature is unavailable.",
            "detail": "Without measured water temperature, Scottville flow cannot produce a dependable fresh-movement read.",
            "tip": "Keep the section named by Migration Stage. Do not chase a fresh wave until measured temperature returns.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_unavailable"
            ],
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "push_unavailable_engine",
        "label": "Unavailable · run type",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "push": {
            "score": null,
            "label": "Unavailable",
            "headline": "This PM run does not have a supported Push model.",
            "detail": "Another species or season’s water response would produce a misleading fresh-movement read.",
            "tip": "Use Migration Stage for seasonal position. Do not borrow another run’s Push pattern.",
            "reasonCodes": [
              "gauge_fresh"
            ],
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "push_not_started",
        "label": "Waiting for migration",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-09-05",
          "progressionSnapshotAt": "2026-09-05T12:00:00.000Z",
          "conditionRefreshAt": "2026-09-05T20:00:00.000Z",
          "progressionExpiresAt": "2026-09-05T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-09-05T22:00:00.000Z",
          "runStage": {
            "stage": "pre_run",
            "copyStrategy": "pere_marquette",
            "stagingContext": true,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-09-05",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Before migration",
            "headline": "Steelhead may be staging near the PM mouth, but dependable river entry has not begun.",
            "detail": "Seasonal timing supports nearby-lake staging. Any Steelhead already in the Lower river would be an early exception.",
            "tip": "Keep the river check brief. Do not move into the Middle or Upper river from calendar timing alone.",
            "whereToStart": "Start at Pere Marquette Lake and the river mouth. Add the Lower river (Pere Marquette Lake–Scottville) only for an early-fish check.",
            "reasonCodes": [
              "stage_pre_run",
              "stage_pre_run_staging"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Evaluating",
            "timingLabel": null,
            "candidateLabel": null,
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-09-20",
            "completedCheckpointCount": 0,
            "currentIndex": null,
            "currentPercentile": null,
            "gaugeResponsePercentile": null,
            "waterTemperaturePercentile": null,
            "usableDays": 0,
            "expectedDays": 0,
            "coveragePercent": 0,
            "historicalYears": 0,
            "sourceDates": [],
            "sourceRefreshSlots": {},
            "headline": "PM Migration Timing is still taking shape.",
            "detail": "Scottville river-rise activity and M-37 cooling do not yet support an Ahead, Typical, or Delayed call.",
            "tip": "Keep the section named by Migration Stage. Do not shift PM sections until this read has enough evidence.",
            "reasonCodes": [
              "conditions_checkpoint_evaluating"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "push": {
            "score": null,
            "label": "Waiting for migration",
            "headline": "Dependable PM Steelhead fall entry has not started.",
            "detail": "Rain, Scottville flow, and water temperature are not scored as an in-season movement signal yet.",
            "tip": "Use Migration Stage. Do not move inland because offseason water resembles an in-season Push.",
            "reasonCodes": [
              "push_tracking_not_started"
            ],
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "not_started",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-09-05",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": []
          },
          "fishInRiver": {
            "score": 0,
            "displayScore": 0,
            "scoreIsApproximate": false,
            "stage": "pre_run",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0,
            "curveDirection": "outside",
            "winterHoldingContext": false,
            "label": "Not expected yet",
            "headline": "Dependable Steelhead presence is not expected in the PM river yet.",
            "detail": "The seasonal estimate remains at zero. Any fish already in the river would be an early exception.",
            "tip": "Use Migration Stage for nearby staging context. Do not treat one early fish as a dependable river opportunity.",
            "reasonCodes": [
              "stage_pre_run",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-09-05T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-09-05T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-09-05T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "The river should fish well, but seasonal fish presence is still low.",
            "detail": "Good flow makes presentations easier, but it does not put fish in the river. Stay low, fish the first travel lane entering deep resting water, make one complete pass, and move instead of waiting for numbers that have not developed.",
            "reasonCodes": [
              "good_fishability_low_presence"
            ]
          }
        }
      },
      {
        "id": "push_offseason",
        "label": "Offseason",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-12-25",
          "progressionSnapshotAt": "2026-12-25T12:00:00.000Z",
          "conditionRefreshAt": "2026-12-25T20:00:00.000Z",
          "progressionExpiresAt": "2026-12-25T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-12-25T22:00:00.000Z",
          "runStage": {
            "stage": "post_run",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-12-25",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Fall entry complete",
            "headline": "PM Steelhead fall entry is complete.",
            "detail": "Steelhead may remain in the river. This fall-entry model no longer scores their current presence or activity.",
            "tip": "Check back in early September when PM fall movement tracking resumes.",
            "whereToStart": "There is no active PM starting section in this fall-entry model.",
            "reasonCodes": [
              "stage_post_run",
              "stage_offseason"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Timing complete",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "peak_complete",
            "checkpointDate": "2026-11-21",
            "cutoffDate": "2026-11-20",
            "observationStartDate": "2026-09-01",
            "completedCheckpointCount": 5,
            "currentIndex": 92.5,
            "currentPercentile": 100,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 87.5,
            "usableDays": 81,
            "expectedDays": 81,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14",
              "2026-11-15",
              "2026-11-16",
              "2026-11-17",
              "2026-11-18",
              "2026-11-19",
              "2026-11-20"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00",
              "2026-11-15": "16:00",
              "2026-11-16": "16:00",
              "2026-11-17": "16:00",
              "2026-11-18": "16:00",
              "2026-11-19": "16:00",
              "2026-11-20": "16:00"
            },
            "headline": "PM fall-entry Migration Timing is complete.",
            "detail": "Earlier in fall, PM migration timing was earlier than usual. The fall-entry timing window is now complete.",
            "tip": "Do not use completed fall timing to infer current presence or activity. PM fall monitoring resumes in early September.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_complete",
              "conditions_ahead",
              "conditions_timing_complete"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "peak_start",
            "previousCheckpointDate": "2026-11-15",
            "previousTimingLabel": "Ahead"
          },
          "push": {
            "score": null,
            "label": "Offseason",
            "headline": "PM Push is outside its fall movement window.",
            "detail": "Current rain, Scottville flow, and water temperature do not provide an in-season fresh-movement signal for this run.",
            "tip": "Check back in early September when fall movement monitoring resumes.",
            "reasonCodes": [
              "push_tracking_offseason"
            ],
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "complete",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-12-22",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-12-22",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-21",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-20",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-12-20T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-19",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-18",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-12-18T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-12-17",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-12-17T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-16",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": null,
            "scoreIsApproximate": false,
            "stage": "post_run",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0,
            "curveDirection": "outside",
            "winterHoldingContext": false,
            "label": "Fall entry complete",
            "headline": "PM Steelhead fall entry is complete.",
            "detail": "Steelhead may remain in the river. This fall-entry model no longer estimates their current seasonal presence.",
            "tip": "Check back in early September when PM fall movement tracking resumes.",
            "reasonCodes": [
              "stage_post_run",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-12-25T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-12-25T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-12-25T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          }
        }
      },
      {
        "id": "push_first_day",
        "label": "History · first run day",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-09-20",
          "progressionSnapshotAt": "2026-09-20T12:00:00.000Z",
          "conditionRefreshAt": "2026-09-20T20:00:00.000Z",
          "progressionExpiresAt": "2026-09-20T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-09-20T22:00:00.000Z",
          "runStage": {
            "stage": "beginning",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-09-20",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Beginning",
            "headline": "Seasonal timing supports the first Steelhead entering the Lower river.",
            "detail": "This is the opening river phase. Presence is expected to be scattered and uneven.",
            "tip": "Cover the Lower river first. Move upstream only after direct fish activity supports it.",
            "whereToStart": "Start in the Lower river (Pere Marquette Lake–Scottville).",
            "reasonCodes": [
              "stage_beginning"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Typical",
            "timingLabel": "Typical",
            "candidateLabel": "Typical",
            "checkpointId": "river_start",
            "checkpointDate": "2026-09-20",
            "cutoffDate": "2026-09-19",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-10-11",
            "completedCheckpointCount": 1,
            "currentIndex": 45.4,
            "currentPercentile": 40,
            "gaugeResponsePercentile": 40,
            "waterTemperaturePercentile": 49,
            "usableDays": 19,
            "expectedDays": 19,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00"
            },
            "headline": "The PM migration is progressing at its usual seasonal pace.",
            "detail": "Season-to-date Scottville river-rise activity and M-37 cooling are close to the usual PM pattern.",
            "tip": "Keep the PM section named by Migration Stage.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_river_start",
              "conditions_typical"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-09-20",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": []
          },
          "fishInRiver": {
            "score": 8,
            "displayScore": 10,
            "scoreIsApproximate": true,
            "stage": "beginning",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0.1,
            "curveDirection": "rising",
            "winterHoldingContext": false,
            "label": "Low presence",
            "headline": "Steelhead seasonal presence is low presence and building.",
            "detail": "Calendar timing places Steelhead presence at low presence, increasing as the fall migration builds. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_beginning",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-09-20T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "The river should fish well, but seasonal fish presence is still low.",
            "detail": "Good flow makes presentations easier, but it does not put fish in the river. Stay low, fish the first travel lane entering deep resting water, make one complete pass, and move instead of waiting for numbers that have not developed.",
            "reasonCodes": [
              "good_fishability_low_presence"
            ]
          }
        }
      },
      {
        "id": "push_complete",
        "label": "Migration complete",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-12-23",
          "progressionSnapshotAt": "2026-12-23T12:00:00.000Z",
          "conditionRefreshAt": "2026-12-23T20:00:00.000Z",
          "progressionExpiresAt": "2026-12-23T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-12-23T22:00:00.000Z",
          "runStage": {
            "stage": "post_run",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-12-23",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Fall entry complete",
            "headline": "PM Steelhead fall entry is complete.",
            "detail": "Steelhead may remain in the river. This fall-entry model no longer scores their current presence or activity.",
            "tip": "Check back in early September when PM fall movement tracking resumes.",
            "whereToStart": "There is no active PM starting section in this fall-entry model.",
            "reasonCodes": [
              "stage_post_run",
              "stage_offseason"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Timing complete",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "peak_complete",
            "checkpointDate": "2026-11-21",
            "cutoffDate": "2026-11-20",
            "observationStartDate": "2026-09-01",
            "completedCheckpointCount": 5,
            "currentIndex": 92.5,
            "currentPercentile": 100,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 87.5,
            "usableDays": 81,
            "expectedDays": 81,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14",
              "2026-11-15",
              "2026-11-16",
              "2026-11-17",
              "2026-11-18",
              "2026-11-19",
              "2026-11-20"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00",
              "2026-11-15": "16:00",
              "2026-11-16": "16:00",
              "2026-11-17": "16:00",
              "2026-11-18": "16:00",
              "2026-11-19": "16:00",
              "2026-11-20": "16:00"
            },
            "headline": "PM fall-entry Migration Timing is complete.",
            "detail": "Earlier in fall, PM migration timing was earlier than usual. The fall-entry timing window is now complete.",
            "tip": "Do not use completed fall timing to infer current presence or activity. PM fall monitoring resumes in early September.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_complete",
              "conditions_ahead",
              "conditions_timing_complete"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "peak_start",
            "previousCheckpointDate": "2026-11-15",
            "previousTimingLabel": "Ahead"
          },
          "push": {
            "score": null,
            "label": "Fall entry complete",
            "headline": "PM Steelhead fall-entry Push is complete.",
            "detail": "Current water may affect Steelhead still in the river. This fall model no longer scores fresh-entry support.",
            "tip": "Do not use a completed fall Push to infer current presence. Check back in early September.",
            "reasonCodes": [
              "push_tracking_complete"
            ],
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "complete",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-12-22",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-12-22",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-21",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-20",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-12-20T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-19",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-18",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-12-18T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-12-17",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-12-17T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-16",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": null,
            "scoreIsApproximate": false,
            "stage": "post_run",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0,
            "curveDirection": "outside",
            "winterHoldingContext": false,
            "label": "Fall entry complete",
            "headline": "PM Steelhead fall entry is complete.",
            "detail": "Steelhead may remain in the river. This fall-entry model no longer estimates their current seasonal presence.",
            "tip": "Check back in early September when PM fall movement tracking resumes.",
            "reasonCodes": [
              "stage_post_run",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-12-23T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-12-23T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-12-23T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          }
        }
      }
    ]
  },
  {
    "id": "fishability",
    "label": "Fishability",
    "scenarios": [
      {
        "id": "fishability_very_low",
        "label": "Tough · very low",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "fishability": {
            "score": 40,
            "label": "Tough",
            "headline": "Very low Scottville flow leaves limited depth and cover.",
            "detail": "Unusually low flow concentrates useful depth into fewer places. Stable flow should keep presentation lanes consistent. This read applies to the Lower river (Pere Marquette Lake–Scottville), not the full PM.",
            "tip": "Use the deepest connected water and keep disturbance low.",
            "reasonCodes": [
              "gauge_fresh",
              "very_low_flow_band",
              "fishability_very_low_cap"
            ],
            "components": {
              "bandBase": 35,
              "trendModifier": 5,
              "appliedCaps": []
            },
            "rulesVersion": "pm-scottville-fishability-v2",
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "fishability_low",
        "label": "Fishable · low",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "fishability": {
            "score": 60,
            "label": "Fishable",
            "headline": "Low Scottville flow remains workable with less depth and cover.",
            "detail": "Lower flow reduces depth and cover across the reach. Stable flow should keep presentation lanes consistent. This read applies to the Lower river (Pere Marquette Lake–Scottville), not the full PM.",
            "tip": "Prioritize deeper seams and current breaks with enough cover.",
            "reasonCodes": [
              "gauge_fresh",
              "low_flow_band"
            ],
            "components": {
              "bandBase": 55,
              "trendModifier": 5,
              "appliedCaps": []
            },
            "rulesVersion": "pm-scottville-fishability-v2",
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "fishability_normal",
        "label": "Good · normal",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "fishability": {
            "score": 75,
            "label": "Good",
            "headline": "Scottville flow is in a comfortable presentation range.",
            "detail": "The flow band supports readable lanes, seams, and holding water. Stable flow should keep presentation lanes consistent. This read applies to the Lower river (Pere Marquette Lake–Scottville), not the full PM.",
            "tip": "Cover readable seams and holding water with a controlled presentation.",
            "reasonCodes": [
              "gauge_fresh",
              "normal_flow_band"
            ],
            "components": {
              "bandBase": 70,
              "trendModifier": 5,
              "appliedCaps": []
            },
            "rulesVersion": "pm-scottville-fishability-v2",
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "fishability_ideal",
        "label": "Excellent · ideal",
        "note": "Canonical production copy",
        "snapshotOverride": {}
      },
      {
        "id": "fishability_high",
        "label": "Fishable · high",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "fishability": {
            "score": 73,
            "label": "Good",
            "headline": "High Scottville flow remains fishable in slower water.",
            "detail": "Higher flow pushes useful presentation water toward softer edges. Stable flow should keep presentation lanes consistent. This read applies to the Lower river (Pere Marquette Lake–Scottville), not the full PM.",
            "tip": "Prioritize inside seams, protected edges, and current breaks.",
            "reasonCodes": [
              "gauge_fresh",
              "high_fishable_flow_band"
            ],
            "components": {
              "bandBase": 68,
              "trendModifier": 5,
              "appliedCaps": []
            },
            "rulesVersion": "pm-scottville-fishability-v2",
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "fishability_very_high",
        "label": "Tough · very high",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "fishability": {
            "score": 45,
            "label": "Tough",
            "headline": "Very high Scottville flow leaves little controllable presentation water.",
            "detail": "Very high flow compresses controllable water into the slowest protected edges. Stable flow should keep presentation lanes consistent. This read applies to the Lower river (Pere Marquette Lake–Scottville), not the full PM.",
            "tip": "Favor protected margins and short controlled presentations. Choose another day if control is not dependable.",
            "reasonCodes": [
              "gauge_fresh",
              "very_high_flow_band"
            ],
            "components": {
              "bandBase": 40,
              "trendModifier": 5,
              "appliedCaps": []
            },
            "rulesVersion": "pm-scottville-fishability-v2",
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "fishability_blown",
        "label": "Poor · blown out",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "fishability": {
            "score": 20,
            "label": "Poor",
            "headline": "Scottville flow is blown out for a dependable presentation plan.",
            "detail": "Excessive flow overwhelms normal lanes and presentation control. Stable flow should keep presentation lanes consistent. This read applies to the Lower river (Pere Marquette Lake–Scottville), not the full PM.",
            "tip": "Choose another day and verify current conditions through authoritative local sources.",
            "reasonCodes": [
              "gauge_fresh",
              "blown_out_flow_band",
              "fishability_blown_out_cap"
            ],
            "components": {
              "bandBase": 15,
              "trendModifier": 5,
              "appliedCaps": []
            },
            "rulesVersion": "pm-scottville-fishability-v2",
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "fishability_falling",
        "label": "Trend · falling",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "fishability": {
            "score": 90,
            "label": "Excellent",
            "headline": "Scottville flow is in its best presentation range.",
            "detail": "The flow band offers the broadest mix of depth, seams, and presentation control. Falling flow should sharpen established seams as the reach settles. This read applies to the Lower river (Pere Marquette Lake–Scottville), not the full PM.",
            "tip": "Cover readable seams and holding water with a controlled presentation.",
            "reasonCodes": [
              "gauge_fresh",
              "ideal_flow_band"
            ],
            "components": {
              "bandBase": 88,
              "trendModifier": 2,
              "appliedCaps": []
            },
            "rulesVersion": "pm-scottville-fishability-v2",
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "fishability_rising",
        "label": "Trend · early rise",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "fishability": {
            "score": 88,
            "label": "Excellent",
            "headline": "Scottville flow is in its best presentation range.",
            "detail": "The flow band offers the broadest mix of depth, seams, and presentation control. Rising flow is beginning to shift lanes toward softer edges. This read applies to the Lower river (Pere Marquette Lake–Scottville), not the full PM.",
            "tip": "Cover readable seams and holding water with a controlled presentation.",
            "reasonCodes": [
              "gauge_fresh",
              "ideal_flow_band"
            ],
            "components": {
              "bandBase": 88,
              "trendModifier": 0,
              "appliedCaps": []
            },
            "rulesVersion": "pm-scottville-fishability-v2",
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "fishability_meaningful",
        "label": "Trend · meaningful rise",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "fishability": {
            "score": 60,
            "label": "Fishable",
            "headline": "High Scottville flow remains fishable in slower water.",
            "detail": "Higher flow pushes useful presentation water toward softer edges. A clear rise is moving controllable presentations toward inside seams and current breaks. This read applies to the Lower river (Pere Marquette Lake–Scottville), not the full PM.",
            "tip": "Prioritize inside seams, protected edges, and current breaks.",
            "reasonCodes": [
              "gauge_fresh",
              "high_fishable_flow_band"
            ],
            "components": {
              "bandBase": 68,
              "trendModifier": -8,
              "appliedCaps": []
            },
            "rulesVersion": "pm-scottville-fishability-v2",
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "fishability_sharp",
        "label": "Trend · sharp rise",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "fishability": {
            "score": 40,
            "label": "Tough",
            "headline": "High Scottville flow remains fishable in slower water.",
            "detail": "Higher flow pushes useful presentation water toward softer edges. A fast rise is quickly replacing settled lanes with heavier current. This read applies to the Lower river (Pere Marquette Lake–Scottville), not the full PM.",
            "tip": "Favor protected margins and short controlled presentations. Choose another day if control is not dependable.",
            "reasonCodes": [
              "gauge_fresh",
              "high_fishable_flow_band",
              "fishability_sharp_rise_high_cap"
            ],
            "components": {
              "bandBase": 68,
              "trendModifier": -20,
              "appliedCaps": [
                40
              ]
            },
            "rulesVersion": "pm-scottville-fishability-v2",
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "fishability_unknown",
        "label": "Trend · unresolved",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "fishability": {
            "score": 69,
            "label": "Fishable",
            "headline": "Scottville flow is workable, but its direction is unknown.",
            "detail": "The flow band offers the broadest mix of depth, seams, and presentation control. Recent Scottville history cannot show whether presentation water is improving or worsening. This read applies to the Lower river (Pere Marquette Lake–Scottville), not the full PM.",
            "tip": "Cover readable seams and holding water with a controlled presentation.",
            "reasonCodes": [
              "gauge_fresh",
              "ideal_flow_band",
              "fishability_unknown_trend_cap"
            ],
            "components": {
              "bandBase": 88,
              "trendModifier": -10,
              "appliedCaps": [
                69
              ]
            },
            "rulesVersion": "pm-scottville-fishability-v2",
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "fishability_stale",
        "label": "Gauge · stale",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "fishability": {
            "score": 55,
            "label": "Fishable",
            "headline": "The aging Scottville reading limits confidence in Lower river Fishability.",
            "detail": "The flow band offers the broadest mix of depth, seams, and presentation control. The river may have changed since the last Scottville reading. This read applies to the Lower river (Pere Marquette Lake–Scottville), not the full PM.",
            "tip": "Cover readable seams and holding water with a controlled presentation.",
            "reasonCodes": [
              "gauge_stale",
              "ideal_flow_band",
              "fishability_stale_gauge_cap"
            ],
            "components": {
              "bandBase": 88,
              "trendModifier": 5,
              "appliedCaps": [
                55
              ]
            },
            "rulesVersion": "pm-scottville-fishability-v2",
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "fishability_unavailable_gauge",
        "label": "Unavailable · gauge",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "fishability": {
            "score": null,
            "label": "Unavailable",
            "headline": "A current Scottville reading is unavailable.",
            "detail": "Without current Scottville flow and direction, Lower river presentation conditions cannot be determined.",
            "tip": "Do not extend an old or missing Scottville read across the PM. Use current authoritative local information.",
            "reasonCodes": [
              "gauge_missing"
            ],
            "rulesVersion": "pm-scottville-fishability-v2",
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "fishability_unavailable_band",
        "label": "Unavailable · river bands",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "fishability": {
            "score": null,
            "label": "Unavailable",
            "headline": "A Fishability band is not available at Scottville.",
            "detail": "Without a local Fishability band, Scottville flow cannot be translated into Lower river presentation conditions.",
            "tip": "Do not extend an old or missing Scottville read across the PM. Use current authoritative local information.",
            "reasonCodes": [
              "gauge_fresh",
              "baseline_missing"
            ],
            "rulesVersion": "pm-scottville-fishability-v2",
            "copyVersion": "river-run-copy-v35"
          }
        }
      }
    ]
  },
  {
    "id": "activity",
    "label": "Activity Outlook",
    "scenarios": [
      {
        "id": "activity_staging",
        "label": "Staging · conditional early fish",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-09-01",
          "progressionSnapshotAt": "2026-09-01T12:00:00.000Z",
          "conditionRefreshAt": "2026-09-01T20:00:00.000Z",
          "progressionExpiresAt": "2026-09-01T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-09-01T22:00:00.000Z",
          "runStage": {
            "stage": "pre_run",
            "copyStrategy": "pere_marquette",
            "stagingContext": true,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-09-01",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Before migration",
            "headline": "Steelhead may be staging near the PM mouth, but dependable river entry has not begun.",
            "detail": "Seasonal timing supports nearby-lake staging. Any Steelhead already in the Lower river would be an early exception.",
            "tip": "Keep the river check brief. Do not move into the Middle or Upper river from calendar timing alone.",
            "whereToStart": "Start at Pere Marquette Lake and the river mouth. Add the Lower river (Pere Marquette Lake–Scottville) only for an early-fish check.",
            "reasonCodes": [
              "stage_pre_run",
              "stage_pre_run_staging"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Evaluating",
            "timingLabel": null,
            "candidateLabel": null,
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-09-20",
            "completedCheckpointCount": 0,
            "currentIndex": null,
            "currentPercentile": null,
            "gaugeResponsePercentile": null,
            "waterTemperaturePercentile": null,
            "usableDays": 0,
            "expectedDays": 0,
            "coveragePercent": 0,
            "historicalYears": 0,
            "sourceDates": [],
            "sourceRefreshSlots": {},
            "headline": "PM Migration Timing is still taking shape.",
            "detail": "Scottville river-rise activity and M-37 cooling do not yet support an Ahead, Typical, or Delayed call.",
            "tip": "Keep the section named by Migration Stage. Do not shift PM sections until this read has enough evidence.",
            "reasonCodes": [
              "conditions_checkpoint_evaluating"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "push": {
            "score": null,
            "label": "Waiting for migration",
            "headline": "Dependable PM Steelhead fall entry has not started.",
            "detail": "Rain, Scottville flow, and water temperature are not scored as an in-season movement signal yet.",
            "tip": "Use Migration Stage. Do not move inland because offseason water resembles an in-season Push.",
            "reasonCodes": [
              "push_tracking_not_started"
            ],
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "not_started",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-09-01",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": []
          },
          "activity": {
            "score": 20,
            "maximum": 100,
            "label": "Reserved",
            "headline": "Today’s PM outlook is reserved, but dependable Steelhead presence has not begun.",
            "detail": "Steelhead may respond selectively under the current limitations. 5–9 AM and 5–9 PM are the leading windows, but neither has a clear advantage. This outlook applies only to an early Steelhead already in the PM river.",
            "tip": "Fish one short test in 5–9 AM or 5–9 PM. Leave if direct response does not support the outlook.",
            "reasonCodes": [
              "activity_confidence_full",
              "activity_today",
              "activity_conditional_presence",
              "activity_warm_water_cap"
            ],
            "rulesVersion": "pm-fall-steelhead-activity-v2",
            "targetDate": "2026-09-01",
            "targetDayLabel": "Today",
            "confidence": "Full",
            "conditionalPresence": true,
            "blocks": [
              {
                "id": "05-09",
                "label": "5–9 AM",
                "score": 20,
                "activityLabel": "Reserved",
                "positiveDriver": "Clouds or lower light make this window more favorable.",
                "limitingFactor": "Water temperature limits responsiveness.",
                "cloudCoverPct": 80,
                "precipitationIn": 0.01
              },
              {
                "id": "09-13",
                "label": "9 AM–1 PM",
                "score": 18,
                "activityLabel": "Inactive",
                "positiveDriver": "The river level and its recent change are favorable.",
                "limitingFactor": "Water temperature limits responsiveness.",
                "cloudCoverPct": 80,
                "precipitationIn": 0
              },
              {
                "id": "13-17",
                "label": "1–5 PM",
                "score": 18,
                "activityLabel": "Inactive",
                "positiveDriver": "The river level and its recent change are favorable.",
                "limitingFactor": "Water temperature limits responsiveness.",
                "cloudCoverPct": 80,
                "precipitationIn": 0
              },
              {
                "id": "17-21",
                "label": "5–9 PM",
                "score": 20,
                "activityLabel": "Reserved",
                "positiveDriver": "Clouds or lower light make this window more favorable.",
                "limitingFactor": "Water temperature limits responsiveness.",
                "cloudCoverPct": 80,
                "precipitationIn": 0
              }
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "fishInRiver": {
            "score": 0,
            "displayScore": 0,
            "scoreIsApproximate": false,
            "stage": "pre_run",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0,
            "curveDirection": "outside",
            "winterHoldingContext": false,
            "label": "Not expected yet",
            "headline": "Dependable Steelhead presence is not expected in the PM river yet.",
            "detail": "The seasonal estimate remains at zero. Any fish already in the river would be an early exception.",
            "tip": "Use Migration Stage for nearby staging context. Do not treat one early fish as a dependable river opportunity.",
            "reasonCodes": [
              "stage_pre_run",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-09-01T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-09-01T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-09-01T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "The river should fish well, but seasonal fish presence is still low.",
            "detail": "Good flow makes presentations easier, but it does not put fish in the river. Stay low, fish the first travel lane entering deep resting water, make one complete pass, and move instead of waiting for numbers that have not developed.",
            "reasonCodes": [
              "good_fishability_low_presence"
            ]
          }
        }
      },
      {
        "id": "activity_beginning_warm",
        "label": "Beginning · temperature response",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-09-20",
          "progressionSnapshotAt": "2026-09-20T12:00:00.000Z",
          "conditionRefreshAt": "2026-09-20T20:00:00.000Z",
          "progressionExpiresAt": "2026-09-20T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-09-20T22:00:00.000Z",
          "runStage": {
            "stage": "beginning",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-09-20",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Beginning",
            "headline": "Seasonal timing supports the first Steelhead entering the Lower river.",
            "detail": "This is the opening river phase. Presence is expected to be scattered and uneven.",
            "tip": "Cover the Lower river first. Move upstream only after direct fish activity supports it.",
            "whereToStart": "Start in the Lower river (Pere Marquette Lake–Scottville).",
            "reasonCodes": [
              "stage_beginning"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Typical",
            "timingLabel": "Typical",
            "candidateLabel": "Typical",
            "checkpointId": "river_start",
            "checkpointDate": "2026-09-20",
            "cutoffDate": "2026-09-19",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-10-11",
            "completedCheckpointCount": 1,
            "currentIndex": 45.4,
            "currentPercentile": 40,
            "gaugeResponsePercentile": 40,
            "waterTemperaturePercentile": 49,
            "usableDays": 19,
            "expectedDays": 19,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00"
            },
            "headline": "The PM migration is progressing at its usual seasonal pace.",
            "detail": "Season-to-date Scottville river-rise activity and M-37 cooling are close to the usual PM pattern.",
            "tip": "Keep the PM section named by Migration Stage.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_river_start",
              "conditions_typical"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-09-20",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": []
          },
          "activity": {
            "score": 20,
            "maximum": 100,
            "label": "Reserved",
            "headline": "Today’s PM Steelhead responsiveness is reserved if fish are present.",
            "detail": "Steelhead may respond selectively under the current limitations. 5–9 AM and 5–9 PM are the leading windows, but neither has a clear advantage. Full confidence uses current Scottville flow, measured PM temperature, and hourly weather.",
            "tip": "Fish one short test in 5–9 AM or 5–9 PM. Leave if direct response does not support the outlook.",
            "reasonCodes": [
              "activity_confidence_full",
              "activity_today",
              "activity_run_present",
              "activity_warm_water_cap"
            ],
            "rulesVersion": "pm-fall-steelhead-activity-v2",
            "targetDate": "2026-09-20",
            "targetDayLabel": "Today",
            "confidence": "Full",
            "conditionalPresence": false,
            "blocks": [
              {
                "id": "05-09",
                "label": "5–9 AM",
                "score": 20,
                "activityLabel": "Reserved",
                "positiveDriver": "Clouds or lower light make this window more favorable.",
                "limitingFactor": "Water temperature limits responsiveness.",
                "cloudCoverPct": 75,
                "precipitationIn": 0.01
              },
              {
                "id": "09-13",
                "label": "9 AM–1 PM",
                "score": 18,
                "activityLabel": "Inactive",
                "positiveDriver": "The river level and its recent change are favorable.",
                "limitingFactor": "Water temperature limits responsiveness.",
                "cloudCoverPct": 75,
                "precipitationIn": 0
              },
              {
                "id": "13-17",
                "label": "1–5 PM",
                "score": 18,
                "activityLabel": "Inactive",
                "positiveDriver": "The river level and its recent change are favorable.",
                "limitingFactor": "Water temperature limits responsiveness.",
                "cloudCoverPct": 75,
                "precipitationIn": 0
              },
              {
                "id": "17-21",
                "label": "5–9 PM",
                "score": 20,
                "activityLabel": "Reserved",
                "positiveDriver": "Clouds or lower light make this window more favorable.",
                "limitingFactor": "Water temperature limits responsiveness.",
                "cloudCoverPct": 75,
                "precipitationIn": 0
              }
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "fishInRiver": {
            "score": 8,
            "displayScore": 10,
            "scoreIsApproximate": true,
            "stage": "beginning",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0.1,
            "curveDirection": "rising",
            "winterHoldingContext": false,
            "label": "Low presence",
            "headline": "Steelhead seasonal presence is low presence and building.",
            "detail": "Calendar timing places Steelhead presence at low presence, increasing as the fall migration builds. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_beginning",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-09-20T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "The river should fish well, but seasonal fish presence is still low.",
            "detail": "Good flow makes presentations easier, but it does not put fish in the river. Stay low, fish the first travel lane entering deep resting water, make one complete pass, and move instead of waiting for numbers that have not developed.",
            "reasonCodes": [
              "good_fishability_low_presence"
            ]
          }
        }
      },
      {
        "id": "activity_clear_morning_lead",
        "label": "Building · clear strongest window",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "activity": {
            "score": 75,
            "maximum": 100,
            "label": "Active",
            "headline": "Today’s PM Steelhead responsiveness is active if fish are present.",
            "detail": "Conditions support a meaningful Steelhead response if fish are present. 5–9 AM is strongest because the measured water temperature is favorable for steelhead. Full confidence uses current Scottville flow, measured PM temperature, and hourly weather.",
            "tip": "Begin with 5–9 AM. Re-rank the blocks if light or weather changes materially.",
            "reasonCodes": [
              "activity_confidence_full",
              "activity_today",
              "activity_run_present"
            ],
            "rulesVersion": "pm-fall-steelhead-activity-v2",
            "targetDate": "2026-10-15",
            "targetDayLabel": "Today",
            "confidence": "Full",
            "conditionalPresence": false,
            "blocks": [
              {
                "id": "05-09",
                "label": "5–9 AM",
                "score": 82,
                "activityLabel": "Highly active",
                "positiveDriver": "The measured water temperature is favorable for Steelhead.",
                "limitingFactor": "Rain adds little extra cover.",
                "cloudCoverPct": 100,
                "precipitationIn": 0.02
              },
              {
                "id": "09-13",
                "label": "9 AM–1 PM",
                "score": 68,
                "activityLabel": "Active",
                "positiveDriver": "The measured water temperature is favorable for Steelhead.",
                "limitingFactor": "Brighter conditions are the main limitation.",
                "cloudCoverPct": 25,
                "precipitationIn": 0
              },
              {
                "id": "13-17",
                "label": "1–5 PM",
                "score": 68,
                "activityLabel": "Active",
                "positiveDriver": "The measured water temperature is favorable for Steelhead.",
                "limitingFactor": "Brighter conditions are the main limitation.",
                "cloudCoverPct": 25,
                "precipitationIn": 0
              },
              {
                "id": "17-21",
                "label": "5–9 PM",
                "score": 72,
                "activityLabel": "Active",
                "positiveDriver": "The measured water temperature is favorable for Steelhead.",
                "limitingFactor": "Heavier precipitation can unsettle presentation.",
                "cloudCoverPct": 0,
                "precipitationIn": 0.32
              }
            ],
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "activity_building_high",
        "label": "Building · highly active",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "activity": {
            "score": 77,
            "maximum": 100,
            "label": "Active",
            "headline": "Today’s PM Steelhead responsiveness is active if fish are present.",
            "detail": "Conditions support a meaningful Steelhead response if fish are present. 5–9 AM and 1–5 PM are the leading windows, but neither has a clear advantage. Full confidence uses current Scottville flow, measured PM temperature, and hourly weather.",
            "tip": "Choose between 5–9 AM and 1–5 PM using actual light and access. Neither has a dependable advantage.",
            "reasonCodes": [
              "activity_confidence_full",
              "activity_today",
              "activity_run_present"
            ],
            "rulesVersion": "pm-fall-steelhead-activity-v2",
            "targetDate": "2026-10-15",
            "targetDayLabel": "Today",
            "confidence": "Full",
            "conditionalPresence": false,
            "blocks": [
              {
                "id": "05-09",
                "label": "5–9 AM",
                "score": 78,
                "activityLabel": "Active",
                "positiveDriver": "The measured water temperature is favorable for Steelhead.",
                "limitingFactor": "Rain adds little extra cover.",
                "cloudCoverPct": 100,
                "precipitationIn": 0.01
              },
              {
                "id": "09-13",
                "label": "9 AM–1 PM",
                "score": 75,
                "activityLabel": "Active",
                "positiveDriver": "The measured water temperature is favorable for Steelhead.",
                "limitingFactor": "Rain adds little extra cover.",
                "cloudCoverPct": 100,
                "precipitationIn": 0
              },
              {
                "id": "13-17",
                "label": "1–5 PM",
                "score": 76,
                "activityLabel": "Active",
                "positiveDriver": "The measured water temperature is favorable for Steelhead.",
                "limitingFactor": "Rain adds little extra cover.",
                "cloudCoverPct": 100,
                "precipitationIn": 0
              },
              {
                "id": "17-21",
                "label": "5–9 PM",
                "score": 76,
                "activityLabel": "Active",
                "positiveDriver": "The measured water temperature is favorable for Steelhead.",
                "limitingFactor": "Rain adds little extra cover.",
                "cloudCoverPct": 100,
                "precipitationIn": 0
              }
            ],
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "activity_peak_active",
        "label": "Peak · active",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-11-15",
          "progressionSnapshotAt": "2026-11-15T12:00:00.000Z",
          "conditionRefreshAt": "2026-11-15T20:00:00.000Z",
          "progressionExpiresAt": "2026-11-15T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-11-15T22:00:00.000Z",
          "runStage": {
            "stage": "peak",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-11-15",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Peak",
            "headline": "This is typically the PM’s strongest Steelhead migration window.",
            "detail": "Seasonal timing supports the widest dependable Steelhead distribution. It does not confirm fish in every section.",
            "tip": "Start in the Middle river. Compare the Upper river only after a complete first pass.",
            "whereToStart": "Start in the Middle river (Scottville–Maple Leaf). Compare the Upper river (Maple Leaf–M-37) when direct fish activity favors it.",
            "reasonCodes": [
              "stage_peak"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Ahead",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "peak_start",
            "checkpointDate": "2026-11-15",
            "cutoffDate": "2026-11-14",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-11-21",
            "completedCheckpointCount": 4,
            "currentIndex": 92.5,
            "currentPercentile": 100,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 87.5,
            "usableDays": 75,
            "expectedDays": 75,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00"
            },
            "headline": "The PM migration is developing earlier than usual.",
            "detail": "Season-to-date Scottville river-rise activity and M-37 cooling are stronger than the usual PM pattern.",
            "tip": "Start one PM section upstream from Migration Stage, capped at the Upper river (Maple Leaf–M-37).",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_start",
              "conditions_ahead"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "building_established",
            "previousCheckpointDate": "2026-10-15",
            "previousTimingLabel": "Ahead"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-11-15",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-11-14",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-13",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-12",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-11-12T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-11",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-10",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-11-10T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-11-09",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-11-09T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-08",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "activity": {
            "score": 78,
            "maximum": 100,
            "label": "Active",
            "headline": "Today’s PM Steelhead responsiveness is active if fish are present.",
            "detail": "Conditions support a meaningful Steelhead response if fish are present. 5–9 AM and 5–9 PM are the leading windows, but neither has a clear advantage. Full confidence uses current Scottville flow, measured PM temperature, and hourly weather.",
            "tip": "Choose between 5–9 AM and 5–9 PM using actual light and access. Neither has a dependable advantage.",
            "reasonCodes": [
              "activity_confidence_full",
              "activity_today",
              "activity_run_present"
            ],
            "rulesVersion": "pm-fall-steelhead-activity-v2",
            "targetDate": "2026-11-15",
            "targetDayLabel": "Today",
            "confidence": "Full",
            "conditionalPresence": false,
            "blocks": [
              {
                "id": "05-09",
                "label": "5–9 AM",
                "score": 80,
                "activityLabel": "Highly active",
                "positiveDriver": "The measured water temperature is favorable for Steelhead.",
                "limitingFactor": "Rain adds little extra cover.",
                "cloudCoverPct": 75,
                "precipitationIn": 0.01
              },
              {
                "id": "09-13",
                "label": "9 AM–1 PM",
                "score": 74,
                "activityLabel": "Active",
                "positiveDriver": "The measured water temperature is favorable for Steelhead.",
                "limitingFactor": "Rain adds little extra cover.",
                "cloudCoverPct": 75,
                "precipitationIn": 0
              },
              {
                "id": "13-17",
                "label": "1–5 PM",
                "score": 74,
                "activityLabel": "Active",
                "positiveDriver": "The measured water temperature is favorable for Steelhead.",
                "limitingFactor": "Rain adds little extra cover.",
                "cloudCoverPct": 75,
                "precipitationIn": 0
              },
              {
                "id": "17-21",
                "label": "5–9 PM",
                "score": 79,
                "activityLabel": "Active",
                "positiveDriver": "The measured water temperature is favorable for Steelhead.",
                "limitingFactor": "Rain adds little extra cover.",
                "cloudCoverPct": 75,
                "precipitationIn": 0
              }
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "fishInRiver": {
            "score": 80,
            "displayScore": 80,
            "scoreIsApproximate": false,
            "stage": "peak",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 1,
            "curveDirection": "near_peak",
            "winterHoldingContext": false,
            "label": "Peak presence",
            "headline": "Steelhead seasonal presence is peak presence and near its seasonal high.",
            "detail": "Calendar timing places Steelhead presence at its expected seasonal peak, near its expected seasonal high. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_peak",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-11-15T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-11-15T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-11-15T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "Seasonal presence is near peak, but today's water does not point to a fresh wave.",
            "detail": "Many fish may already be in the river even when rain, river level, and temperature show little support for new movement. Start in established deep holes, fish each one from head to tail, and leave lower entry lanes secondary.",
            "reasonCodes": [
              "peak_presence_weak_push"
            ]
          }
        }
      },
      {
        "id": "activity_moderate",
        "label": "Building · moderate mixed window",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "activity": {
            "score": 18,
            "maximum": 100,
            "label": "Inactive",
            "headline": "Today’s PM Steelhead responsiveness is inactive if fish are present.",
            "detail": "No time block broadly supports an aggressive Steelhead response. 5–9 AM and 5–9 PM are the leading windows, but neither has a clear advantage. Full confidence uses current Scottville flow, measured PM temperature, and hourly weather.",
            "tip": "Treat every block as unfavorable. If you fish, limit the test to 5–9 AM or 5–9 PM.",
            "reasonCodes": [
              "activity_confidence_full",
              "activity_today",
              "activity_run_present",
              "activity_warm_water_cap"
            ],
            "rulesVersion": "pm-fall-steelhead-activity-v2",
            "targetDate": "2026-10-15",
            "targetDayLabel": "Today",
            "confidence": "Full",
            "conditionalPresence": false,
            "blocks": [
              {
                "id": "05-09",
                "label": "5–9 AM",
                "score": 19,
                "activityLabel": "Inactive",
                "positiveDriver": "Clouds or lower light make this window more favorable.",
                "limitingFactor": "Water temperature limits responsiveness.",
                "cloudCoverPct": 15,
                "precipitationIn": 0.01
              },
              {
                "id": "09-13",
                "label": "9 AM–1 PM",
                "score": 15,
                "activityLabel": "Inactive",
                "positiveDriver": "The river level and its recent change are favorable.",
                "limitingFactor": "Water temperature limits responsiveness.",
                "cloudCoverPct": 15,
                "precipitationIn": 0
              },
              {
                "id": "13-17",
                "label": "1–5 PM",
                "score": 14,
                "activityLabel": "Inactive",
                "positiveDriver": "The river level and its recent change are favorable.",
                "limitingFactor": "Water temperature limits responsiveness.",
                "cloudCoverPct": 15,
                "precipitationIn": 0
              },
              {
                "id": "17-21",
                "label": "5–9 PM",
                "score": 18,
                "activityLabel": "Inactive",
                "positiveDriver": "Clouds or lower light make this window more favorable.",
                "limitingFactor": "Water temperature limits responsiveness.",
                "cloudCoverPct": 15,
                "precipitationIn": 0
              }
            ],
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "activity_reserved",
        "label": "Peak · reserved warm and high",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-11-15",
          "progressionSnapshotAt": "2026-11-15T12:00:00.000Z",
          "conditionRefreshAt": "2026-11-15T20:00:00.000Z",
          "progressionExpiresAt": "2026-11-15T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-11-15T22:00:00.000Z",
          "runStage": {
            "stage": "peak",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-11-15",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Peak",
            "headline": "This is typically the PM’s strongest Steelhead migration window.",
            "detail": "Seasonal timing supports the widest dependable Steelhead distribution. It does not confirm fish in every section.",
            "tip": "Start in the Middle river. Compare the Upper river only after a complete first pass.",
            "whereToStart": "Start in the Middle river (Scottville–Maple Leaf). Compare the Upper river (Maple Leaf–M-37) when direct fish activity favors it.",
            "reasonCodes": [
              "stage_peak"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Ahead",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "peak_start",
            "checkpointDate": "2026-11-15",
            "cutoffDate": "2026-11-14",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-11-21",
            "completedCheckpointCount": 4,
            "currentIndex": 92.5,
            "currentPercentile": 100,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 87.5,
            "usableDays": 75,
            "expectedDays": 75,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00"
            },
            "headline": "The PM migration is developing earlier than usual.",
            "detail": "Season-to-date Scottville river-rise activity and M-37 cooling are stronger than the usual PM pattern.",
            "tip": "Start one PM section upstream from Migration Stage, capped at the Upper river (Maple Leaf–M-37).",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_start",
              "conditions_ahead"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "building_established",
            "previousCheckpointDate": "2026-10-15",
            "previousTimingLabel": "Ahead"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-11-15",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-11-14",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-13",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-12",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-11-12T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-11",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-10",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-11-10T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-11-09",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-11-09T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-08",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "activity": {
            "score": 8,
            "maximum": 100,
            "label": "Inactive",
            "headline": "Today’s PM Steelhead responsiveness is inactive if fish are present.",
            "detail": "No time block broadly supports an aggressive Steelhead response. 5–9 AM and 5–9 PM are the leading windows, but neither has a clear advantage. Full confidence uses current Scottville flow, measured PM temperature, and hourly weather.",
            "tip": "Treat every block as unfavorable. If you fish, limit the test to 5–9 AM or 5–9 PM.",
            "reasonCodes": [
              "activity_confidence_full",
              "activity_today",
              "activity_run_present",
              "activity_temperature_barrier_cap"
            ],
            "rulesVersion": "pm-fall-steelhead-activity-v2",
            "targetDate": "2026-11-15",
            "targetDayLabel": "Today",
            "confidence": "Full",
            "conditionalPresence": false,
            "blocks": [
              {
                "id": "05-09",
                "label": "5–9 AM",
                "score": 8,
                "activityLabel": "Inactive",
                "positiveDriver": "Clouds or lower light make this window more favorable.",
                "limitingFactor": "Water temperature limits responsiveness.",
                "cloudCoverPct": 5,
                "precipitationIn": 0.01
              },
              {
                "id": "09-13",
                "label": "9 AM–1 PM",
                "score": 6,
                "activityLabel": "Inactive",
                "positiveDriver": "The river level and its recent change are favorable.",
                "limitingFactor": "Water temperature limits responsiveness.",
                "cloudCoverPct": 5,
                "precipitationIn": 0
              },
              {
                "id": "13-17",
                "label": "1–5 PM",
                "score": 6,
                "activityLabel": "Inactive",
                "positiveDriver": "The river level and its recent change are favorable.",
                "limitingFactor": "Water temperature limits responsiveness.",
                "cloudCoverPct": 5,
                "precipitationIn": 0
              },
              {
                "id": "17-21",
                "label": "5–9 PM",
                "score": 8,
                "activityLabel": "Inactive",
                "positiveDriver": "Clouds or lower light make this window more favorable.",
                "limitingFactor": "Water temperature limits responsiveness.",
                "cloudCoverPct": 5,
                "precipitationIn": 0
              }
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "fishInRiver": {
            "score": 80,
            "displayScore": 80,
            "scoreIsApproximate": false,
            "stage": "peak",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 1,
            "curveDirection": "near_peak",
            "winterHoldingContext": false,
            "label": "Peak presence",
            "headline": "Steelhead seasonal presence is peak presence and near its seasonal high.",
            "detail": "Calendar timing places Steelhead presence at its expected seasonal peak, near its expected seasonal high. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_peak",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-11-15T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-11-15T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-11-15T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "Seasonal presence is near peak, but today's water does not point to a fresh wave.",
            "detail": "Many fish may already be in the river even when rain, river level, and temperature show little support for new movement. Start in established deep holes, fish each one from head to tail, and leave lower entry lanes secondary.",
            "reasonCodes": [
              "peak_presence_weak_push"
            ]
          }
        }
      },
      {
        "id": "activity_severe_floor",
        "label": "Peak · severe conditions without salmon floor",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-11-15",
          "progressionSnapshotAt": "2026-11-15T12:00:00.000Z",
          "conditionRefreshAt": "2026-11-15T20:00:00.000Z",
          "progressionExpiresAt": "2026-11-15T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-11-15T22:00:00.000Z",
          "runStage": {
            "stage": "peak",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-11-15",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Peak",
            "headline": "This is typically the PM’s strongest Steelhead migration window.",
            "detail": "Seasonal timing supports the widest dependable Steelhead distribution. It does not confirm fish in every section.",
            "tip": "Start in the Middle river. Compare the Upper river only after a complete first pass.",
            "whereToStart": "Start in the Middle river (Scottville–Maple Leaf). Compare the Upper river (Maple Leaf–M-37) when direct fish activity favors it.",
            "reasonCodes": [
              "stage_peak"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Ahead",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "peak_start",
            "checkpointDate": "2026-11-15",
            "cutoffDate": "2026-11-14",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-11-21",
            "completedCheckpointCount": 4,
            "currentIndex": 92.5,
            "currentPercentile": 100,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 87.5,
            "usableDays": 75,
            "expectedDays": 75,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00"
            },
            "headline": "The PM migration is developing earlier than usual.",
            "detail": "Season-to-date Scottville river-rise activity and M-37 cooling are stronger than the usual PM pattern.",
            "tip": "Start one PM section upstream from Migration Stage, capped at the Upper river (Maple Leaf–M-37).",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_start",
              "conditions_ahead"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "building_established",
            "previousCheckpointDate": "2026-10-15",
            "previousTimingLabel": "Ahead"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-11-15",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-11-14",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-13",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-12",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-11-12T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-11",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-10",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-11-10T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-11-09",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-11-09T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-08",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "activity": {
            "score": 2,
            "maximum": 100,
            "label": "Inactive",
            "headline": "Today’s PM Steelhead responsiveness is inactive if fish are present.",
            "detail": "No time block broadly supports an aggressive Steelhead response. 5–9 AM and 5–9 PM are the leading windows, but neither has a clear advantage. Full confidence uses current Scottville flow, measured PM temperature, and hourly weather.",
            "tip": "Treat every block as unfavorable. If you fish, limit the test to 5–9 AM or 5–9 PM.",
            "reasonCodes": [
              "activity_confidence_full",
              "activity_today",
              "activity_run_present",
              "activity_temperature_barrier_cap",
              "activity_blown_out_cap"
            ],
            "rulesVersion": "pm-fall-steelhead-activity-v2",
            "targetDate": "2026-11-15",
            "targetDayLabel": "Today",
            "confidence": "Full",
            "conditionalPresence": false,
            "blocks": [
              {
                "id": "05-09",
                "label": "5–9 AM",
                "score": 2,
                "activityLabel": "Inactive",
                "positiveDriver": "Clouds or lower light make this window more favorable.",
                "limitingFactor": "Water temperature limits responsiveness.",
                "cloudCoverPct": 0,
                "precipitationIn": 0.3
              },
              {
                "id": "09-13",
                "label": "9 AM–1 PM",
                "score": 1,
                "activityLabel": "Inactive",
                "positiveDriver": "The river level and its recent change are favorable.",
                "limitingFactor": "Water temperature limits responsiveness.",
                "cloudCoverPct": 0,
                "precipitationIn": 0
              },
              {
                "id": "13-17",
                "label": "1–5 PM",
                "score": 1,
                "activityLabel": "Inactive",
                "positiveDriver": "The river level and its recent change are favorable.",
                "limitingFactor": "Water temperature limits responsiveness.",
                "cloudCoverPct": 0,
                "precipitationIn": 0
              },
              {
                "id": "17-21",
                "label": "5–9 PM",
                "score": 2,
                "activityLabel": "Inactive",
                "positiveDriver": "Clouds or lower light make this window more favorable.",
                "limitingFactor": "Water temperature limits responsiveness.",
                "cloudCoverPct": 0,
                "precipitationIn": 0
              }
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "fishInRiver": {
            "score": 80,
            "displayScore": 80,
            "scoreIsApproximate": false,
            "stage": "peak",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 1,
            "curveDirection": "near_peak",
            "winterHoldingContext": false,
            "label": "Peak presence",
            "headline": "Steelhead seasonal presence is peak presence and near its seasonal high.",
            "detail": "Calendar timing places Steelhead presence at its expected seasonal peak, near its expected seasonal high. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_peak",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-11-15T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-11-15T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-11-15T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "Seasonal presence is near peak, but today's water does not point to a fresh wave.",
            "detail": "Many fish may already be in the river even when rain, river level, and temperature show little support for new movement. Start in established deep holes, fish each one from head to tail, and leave lower entry lanes secondary.",
            "reasonCodes": [
              "peak_presence_weak_push"
            ]
          }
        }
      },
      {
        "id": "activity_tapering",
        "label": "Late fall · cold-water response",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-12-05",
          "progressionSnapshotAt": "2026-12-05T12:00:00.000Z",
          "conditionRefreshAt": "2026-12-05T20:00:00.000Z",
          "progressionExpiresAt": "2026-12-05T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-12-05T22:00:00.000Z",
          "runStage": {
            "stage": "tapering",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-12-05",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Late fall",
            "headline": "PM Steelhead fall entry remains strong but is slowing.",
            "detail": "Seasonal timing still supports broad fall presence. Colder water makes established holding water more important than new entry.",
            "tip": "Start in the Middle river. Shift lower only when Push supports fresh movement.",
            "whereToStart": "Start in the Middle river (Scottville–Maple Leaf). Add the Lower river (Pere Marquette Lake–Scottville) only when Push supports fresh movement.",
            "reasonCodes": [
              "stage_tapering"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Timing complete",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "peak_complete",
            "checkpointDate": "2026-11-21",
            "cutoffDate": "2026-11-20",
            "observationStartDate": "2026-09-01",
            "completedCheckpointCount": 5,
            "currentIndex": 92.5,
            "currentPercentile": 100,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 87.5,
            "usableDays": 81,
            "expectedDays": 81,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14",
              "2026-11-15",
              "2026-11-16",
              "2026-11-17",
              "2026-11-18",
              "2026-11-19",
              "2026-11-20"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00",
              "2026-11-15": "16:00",
              "2026-11-16": "16:00",
              "2026-11-17": "16:00",
              "2026-11-18": "16:00",
              "2026-11-19": "16:00",
              "2026-11-20": "16:00"
            },
            "headline": "The early-season timing read is complete.",
            "detail": "Earlier in the season, the migration was moving earlier than usual. Now that the migration is well underway, current movement and river conditions matter more than an early-or-late call.",
            "tip": "Stop shifting river sections based on early-or-late timing. Begin in established holding water for the current Migration Stage; if Push is Possible or stronger, make lower travel lanes the next stop.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_complete",
              "conditions_ahead",
              "conditions_timing_complete"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "peak_start",
            "previousCheckpointDate": "2026-11-15",
            "previousTimingLabel": "Ahead"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-12-05",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-12-04",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-03",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-02",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-12-02T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-01",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-30",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-11-30T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-11-29",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-11-29T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-28",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "activity": {
            "score": 88,
            "maximum": 100,
            "label": "Highly active",
            "headline": "Today’s PM Steelhead responsiveness is highly active if fish are present.",
            "detail": "Conditions strongly support Steelhead responsiveness if fish are present. 5–9 AM and 5–9 PM are the leading windows, but neither has a clear advantage. Cold late-fall water can shorten response windows without meaning Steelhead have left the river.",
            "tip": "Choose between 5–9 AM and 5–9 PM using actual light and access. Neither has a dependable advantage.",
            "reasonCodes": [
              "activity_confidence_full",
              "activity_today",
              "activity_run_present"
            ],
            "rulesVersion": "pm-fall-steelhead-activity-v2",
            "targetDate": "2026-12-05",
            "targetDayLabel": "Today",
            "confidence": "Full",
            "conditionalPresence": false,
            "blocks": [
              {
                "id": "05-09",
                "label": "5–9 AM",
                "score": 90,
                "activityLabel": "Highly active",
                "positiveDriver": "The measured water temperature is favorable for Steelhead.",
                "limitingFactor": "Rain adds little extra cover.",
                "cloudCoverPct": 95,
                "precipitationIn": 0.01
              },
              {
                "id": "09-13",
                "label": "9 AM–1 PM",
                "score": 84,
                "activityLabel": "Highly active",
                "positiveDriver": "The measured water temperature is favorable for Steelhead.",
                "limitingFactor": "Rain adds little extra cover.",
                "cloudCoverPct": 95,
                "precipitationIn": 0
              },
              {
                "id": "13-17",
                "label": "1–5 PM",
                "score": 83,
                "activityLabel": "Highly active",
                "positiveDriver": "The measured water temperature is favorable for Steelhead.",
                "limitingFactor": "Rain adds little extra cover.",
                "cloudCoverPct": 95,
                "precipitationIn": 0
              },
              {
                "id": "17-21",
                "label": "5–9 PM",
                "score": 88,
                "activityLabel": "Highly active",
                "positiveDriver": "The measured water temperature is favorable for Steelhead.",
                "limitingFactor": "Rain adds little extra cover.",
                "cloudCoverPct": 95,
                "precipitationIn": 0
              }
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "fishInRiver": {
            "score": 79,
            "displayScore": 80,
            "scoreIsApproximate": true,
            "stage": "tapering",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0.99,
            "curveDirection": "falling",
            "winterHoldingContext": false,
            "label": "Peak presence",
            "headline": "Steelhead seasonal presence is peak presence and declining.",
            "detail": "Calendar timing places Steelhead presence at its expected seasonal peak, declining as the fall migration advances. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_tapering",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-12-05T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-12-05T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-12-05T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          }
        }
      },
      {
        "id": "activity_ending",
        "label": "Holding transition · living fish",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-12-20",
          "progressionSnapshotAt": "2026-12-20T12:00:00.000Z",
          "conditionRefreshAt": "2026-12-20T20:00:00.000Z",
          "progressionExpiresAt": "2026-12-20T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-12-20T22:00:00.000Z",
          "runStage": {
            "stage": "ending",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-12-20",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Holding transition",
            "headline": "PM Steelhead fall entry is in its final seasonal phase.",
            "detail": "Steelhead may remain after fall entry ends. This state marks the model boundary, not fish leaving the river.",
            "tip": "Prioritize established Middle river holding water and use Activity only through the final fall-entry day.",
            "whereToStart": "Start in the Middle river (Scottville–Maple Leaf). Add the Upper river (Maple Leaf–M-37) only for established late-fall holding water.",
            "reasonCodes": [
              "stage_ending"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Timing complete",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "peak_complete",
            "checkpointDate": "2026-11-21",
            "cutoffDate": "2026-11-20",
            "observationStartDate": "2026-09-01",
            "completedCheckpointCount": 5,
            "currentIndex": 92.5,
            "currentPercentile": 100,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 87.5,
            "usableDays": 81,
            "expectedDays": 81,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14",
              "2026-11-15",
              "2026-11-16",
              "2026-11-17",
              "2026-11-18",
              "2026-11-19",
              "2026-11-20"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00",
              "2026-11-15": "16:00",
              "2026-11-16": "16:00",
              "2026-11-17": "16:00",
              "2026-11-18": "16:00",
              "2026-11-19": "16:00",
              "2026-11-20": "16:00"
            },
            "headline": "The early-season timing read is complete.",
            "detail": "Earlier in the season, the migration was moving earlier than usual. Now that the migration is well underway, current movement and river conditions matter more than an early-or-late call.",
            "tip": "Stop shifting river sections based on early-or-late timing. Begin in established holding water for the current Migration Stage; if Push is Possible or stronger, make lower travel lanes the next stop.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_complete",
              "conditions_ahead",
              "conditions_timing_complete"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "peak_start",
            "previousCheckpointDate": "2026-11-15",
            "previousTimingLabel": "Ahead"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-12-20",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-12-19",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-18",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-17",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-12-17T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-16",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-15",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-12-15T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-12-14",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-12-14T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-13",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "activity": {
            "score": 88,
            "maximum": 100,
            "label": "Highly active",
            "headline": "Today’s PM Steelhead responsiveness is highly active if fish are present.",
            "detail": "Conditions strongly support Steelhead responsiveness if fish are present. 5–9 AM and 5–9 PM are the leading windows, but neither has a clear advantage. Cold late-fall water can shorten response windows without meaning Steelhead have left the river.",
            "tip": "Choose between 5–9 AM and 5–9 PM using actual light and access. Neither has a dependable advantage.",
            "reasonCodes": [
              "activity_confidence_full",
              "activity_today",
              "activity_run_present"
            ],
            "rulesVersion": "pm-fall-steelhead-activity-v2",
            "targetDate": "2026-12-20",
            "targetDayLabel": "Today",
            "confidence": "Full",
            "conditionalPresence": false,
            "blocks": [
              {
                "id": "05-09",
                "label": "5–9 AM",
                "score": 90,
                "activityLabel": "Highly active",
                "positiveDriver": "The measured water temperature is favorable for Steelhead.",
                "limitingFactor": "Rain adds little extra cover.",
                "cloudCoverPct": 95,
                "precipitationIn": 0.01
              },
              {
                "id": "09-13",
                "label": "9 AM–1 PM",
                "score": 84,
                "activityLabel": "Highly active",
                "positiveDriver": "The measured water temperature is favorable for Steelhead.",
                "limitingFactor": "Rain adds little extra cover.",
                "cloudCoverPct": 95,
                "precipitationIn": 0
              },
              {
                "id": "13-17",
                "label": "1–5 PM",
                "score": 83,
                "activityLabel": "Highly active",
                "positiveDriver": "The measured water temperature is favorable for Steelhead.",
                "limitingFactor": "Rain adds little extra cover.",
                "cloudCoverPct": 95,
                "precipitationIn": 0
              },
              {
                "id": "17-21",
                "label": "5–9 PM",
                "score": 88,
                "activityLabel": "Highly active",
                "positiveDriver": "The measured water temperature is favorable for Steelhead.",
                "limitingFactor": "Rain adds little extra cover.",
                "cloudCoverPct": 95,
                "precipitationIn": 0
              }
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "fishInRiver": {
            "score": 71,
            "displayScore": 70,
            "scoreIsApproximate": true,
            "stage": "ending",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0.8916666666666667,
            "curveDirection": "falling",
            "winterHoldingContext": false,
            "label": "High presence",
            "headline": "Steelhead seasonal presence is high presence and declining.",
            "detail": "Calendar timing places Steelhead presence high for this fall migration, declining as the fall migration advances. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_ending",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-12-20T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-12-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-12-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          }
        }
      },
      {
        "id": "activity_post_run",
        "label": "Post-run · winter holding handoff",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-12-23",
          "progressionSnapshotAt": "2026-12-23T12:00:00.000Z",
          "conditionRefreshAt": "2026-12-23T20:00:00.000Z",
          "progressionExpiresAt": "2026-12-23T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-12-23T22:00:00.000Z",
          "runStage": {
            "stage": "post_run",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-12-23",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Fall entry complete",
            "headline": "PM Steelhead fall entry is complete.",
            "detail": "Steelhead may remain in the river. This fall-entry model no longer scores their current presence or activity.",
            "tip": "Check back in early September when PM fall movement tracking resumes.",
            "whereToStart": "There is no active PM starting section in this fall-entry model.",
            "reasonCodes": [
              "stage_post_run",
              "stage_offseason"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Timing complete",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "peak_complete",
            "checkpointDate": "2026-11-21",
            "cutoffDate": "2026-11-20",
            "observationStartDate": "2026-09-01",
            "completedCheckpointCount": 5,
            "currentIndex": 92.5,
            "currentPercentile": 100,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 87.5,
            "usableDays": 81,
            "expectedDays": 81,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14",
              "2026-11-15",
              "2026-11-16",
              "2026-11-17",
              "2026-11-18",
              "2026-11-19",
              "2026-11-20"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00",
              "2026-11-15": "16:00",
              "2026-11-16": "16:00",
              "2026-11-17": "16:00",
              "2026-11-18": "16:00",
              "2026-11-19": "16:00",
              "2026-11-20": "16:00"
            },
            "headline": "PM fall-entry Migration Timing is complete.",
            "detail": "Earlier in fall, PM migration timing was earlier than usual. The fall-entry timing window is now complete.",
            "tip": "Do not use completed fall timing to infer current presence or activity. PM fall monitoring resumes in early September.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_complete",
              "conditions_ahead",
              "conditions_timing_complete"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "peak_start",
            "previousCheckpointDate": "2026-11-15",
            "previousTimingLabel": "Ahead"
          },
          "push": {
            "score": null,
            "label": "Fall entry complete",
            "headline": "PM Steelhead fall-entry Push is complete.",
            "detail": "Current water may affect Steelhead still in the river. This fall model no longer scores fresh-entry support.",
            "tip": "Do not use a completed fall Push to infer current presence. Check back in early September.",
            "reasonCodes": [
              "push_tracking_complete"
            ],
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "complete",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-12-22",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-12-22",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-21",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-20",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-12-20T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-19",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-18",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-12-18T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-12-17",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-12-17T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-16",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "activity": {
            "score": null,
            "maximum": 100,
            "label": "Fall entry complete",
            "headline": "PM Steelhead fall-entry Activity is complete.",
            "detail": "Steelhead may remain in the river. This fall-entry model no longer scores their current responsiveness.",
            "tip": "Do not use this completed fall outlook to infer current activity. Check back in early September.",
            "reasonCodes": [
              "activity_fall_entry_complete"
            ],
            "rulesVersion": "pm-fall-steelhead-activity-v2",
            "targetDate": "2026-12-23",
            "targetDayLabel": "Today",
            "confidence": "Limited",
            "conditionalPresence": false,
            "blocks": [],
            "copyVersion": "river-run-copy-v35"
          },
          "fishInRiver": {
            "score": null,
            "scoreIsApproximate": false,
            "stage": "post_run",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0,
            "curveDirection": "outside",
            "winterHoldingContext": false,
            "label": "Fall entry complete",
            "headline": "PM Steelhead fall entry is complete.",
            "detail": "Steelhead may remain in the river. This fall-entry model no longer estimates their current seasonal presence.",
            "tip": "Check back in early September when PM fall movement tracking resumes.",
            "reasonCodes": [
              "stage_post_run",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-12-23T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-12-23T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-12-23T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          }
        }
      },
      {
        "id": "activity_tomorrow",
        "label": "Tomorrow · after 9 PM",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "activity": {
            "score": 60,
            "maximum": 100,
            "label": "Active",
            "headline": "Tomorrow’s PM Steelhead responsiveness is active if fish are present.",
            "detail": "Conditions support a meaningful Steelhead response if fish are present. 5–9 AM and 5–9 PM are the leading windows, but neither has a clear advantage. Confidence is Moderate because one key input is missing or this is tomorrow’s outlook.",
            "tip": "Choose between 5–9 AM and 5–9 PM using actual light and access. Neither has a dependable advantage.",
            "reasonCodes": [
              "activity_confidence_moderate",
              "activity_tomorrow",
              "activity_run_present"
            ],
            "rulesVersion": "pm-fall-steelhead-activity-v2",
            "targetDate": "2026-10-16",
            "targetDayLabel": "Tomorrow",
            "confidence": "Moderate",
            "conditionalPresence": false,
            "blocks": [
              {
                "id": "05-09",
                "label": "5–9 AM",
                "score": 62,
                "activityLabel": "Active",
                "positiveDriver": "The measured water temperature is favorable for Steelhead.",
                "limitingFactor": "Rain adds little extra cover.",
                "cloudCoverPct": 85,
                "precipitationIn": 0.01
              },
              {
                "id": "09-13",
                "label": "9 AM–1 PM",
                "score": 57,
                "activityLabel": "Moderate",
                "positiveDriver": "The measured water temperature is favorable for Steelhead.",
                "limitingFactor": "Rain adds little extra cover.",
                "cloudCoverPct": 85,
                "precipitationIn": 0
              },
              {
                "id": "13-17",
                "label": "1–5 PM",
                "score": 56,
                "activityLabel": "Moderate",
                "positiveDriver": "The measured water temperature is favorable for Steelhead.",
                "limitingFactor": "Rain adds little extra cover.",
                "cloudCoverPct": 85,
                "precipitationIn": 0
              },
              {
                "id": "17-21",
                "label": "5–9 PM",
                "score": 60,
                "activityLabel": "Active",
                "positiveDriver": "The measured water temperature is favorable for Steelhead.",
                "limitingFactor": "Rain adds little extra cover.",
                "cloudCoverPct": 85,
                "precipitationIn": 0
              }
            ],
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "activity_moderate_no_temp",
        "label": "Moderate data · no measured temperature",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "activity": {
            "score": 54,
            "maximum": 100,
            "label": "Moderate",
            "headline": "Today’s PM Steelhead responsiveness is moderate if fish are present.",
            "detail": "Conditions offer mixed support for Steelhead responsiveness. 5–9 AM and 5–9 PM are the leading windows, but neither has a clear advantage. Confidence is Moderate because one key input is missing or this is tomorrow’s outlook.",
            "tip": "Choose between 5–9 AM and 5–9 PM using actual light and access. Neither has a dependable advantage.",
            "reasonCodes": [
              "activity_confidence_moderate",
              "activity_today",
              "activity_run_present"
            ],
            "rulesVersion": "pm-fall-steelhead-activity-v2",
            "targetDate": "2026-10-15",
            "targetDayLabel": "Today",
            "confidence": "Moderate",
            "conditionalPresence": false,
            "blocks": [
              {
                "id": "05-09",
                "label": "5–9 AM",
                "score": 56,
                "activityLabel": "Moderate",
                "positiveDriver": "Clouds or lower light make this window more favorable.",
                "limitingFactor": "Measured water temperature is unavailable.",
                "cloudCoverPct": 80,
                "precipitationIn": 0.01
              },
              {
                "id": "09-13",
                "label": "9 AM–1 PM",
                "score": 48,
                "activityLabel": "Moderate",
                "positiveDriver": "The river level and its recent change are favorable.",
                "limitingFactor": "Measured water temperature is unavailable.",
                "cloudCoverPct": 80,
                "precipitationIn": 0
              },
              {
                "id": "13-17",
                "label": "1–5 PM",
                "score": 47,
                "activityLabel": "Moderate",
                "positiveDriver": "The river level and its recent change are favorable.",
                "limitingFactor": "Measured water temperature is unavailable.",
                "cloudCoverPct": 80,
                "precipitationIn": 0
              },
              {
                "id": "17-21",
                "label": "5–9 PM",
                "score": 55,
                "activityLabel": "Moderate",
                "positiveDriver": "Clouds or lower light make this window more favorable.",
                "limitingFactor": "Measured water temperature is unavailable.",
                "cloudCoverPct": 80,
                "precipitationIn": 0
              }
            ],
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "activity_limited_weather_only",
        "label": "Limited · weather only",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "activity": {
            "score": 48,
            "maximum": 100,
            "label": "Moderate",
            "headline": "Today’s PM Steelhead responsiveness is moderate if fish are present.",
            "detail": "Conditions offer mixed support for Steelhead responsiveness. 5–9 AM and 5–9 PM are the leading windows, but neither has a clear advantage. Confidence is Limited because several key inputs are unavailable.",
            "tip": "Choose between 5–9 AM and 5–9 PM using actual light and access. Neither has a dependable advantage.",
            "reasonCodes": [
              "activity_confidence_limited",
              "activity_today",
              "activity_run_present"
            ],
            "rulesVersion": "pm-fall-steelhead-activity-v2",
            "targetDate": "2026-10-15",
            "targetDayLabel": "Today",
            "confidence": "Limited",
            "conditionalPresence": false,
            "blocks": [
              {
                "id": "05-09",
                "label": "5–9 AM",
                "score": 52,
                "activityLabel": "Moderate",
                "positiveDriver": "Clouds or lower light make this window more favorable.",
                "limitingFactor": "Measured water temperature is unavailable.",
                "cloudCoverPct": 80,
                "precipitationIn": 0.01
              },
              {
                "id": "09-13",
                "label": "9 AM–1 PM",
                "score": 41,
                "activityLabel": "Moderate",
                "positiveDriver": "The available light is workable in this window.",
                "limitingFactor": "Measured water temperature is unavailable.",
                "cloudCoverPct": 80,
                "precipitationIn": 0
              },
              {
                "id": "13-17",
                "label": "1–5 PM",
                "score": 39,
                "activityLabel": "Reserved",
                "positiveDriver": "The available light is workable in this window.",
                "limitingFactor": "Measured water temperature is unavailable.",
                "cloudCoverPct": 80,
                "precipitationIn": 0
              },
              {
                "id": "17-21",
                "label": "5–9 PM",
                "score": 50,
                "activityLabel": "Moderate",
                "positiveDriver": "Clouds or lower light make this window more favorable.",
                "limitingFactor": "Measured water temperature is unavailable.",
                "cloudCoverPct": 80,
                "precipitationIn": 0
              }
            ],
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "activity_limited_no_weather",
        "label": "Limited · river data only",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "activity": {
            "score": 55,
            "maximum": 100,
            "label": "Moderate",
            "headline": "Today’s PM Steelhead responsiveness is moderate if fish are present.",
            "detail": "Conditions offer mixed support for Steelhead responsiveness. Hourly light and weather data are unavailable, so no time block can be separated. Confidence is Limited because several key inputs are unavailable.",
            "tip": "Choose the block that best fits current access. Hourly conditions cannot separate the windows.",
            "reasonCodes": [
              "activity_confidence_limited",
              "activity_today",
              "activity_run_present"
            ],
            "rulesVersion": "pm-fall-steelhead-activity-v2",
            "targetDate": "2026-10-15",
            "targetDayLabel": "Today",
            "confidence": "Limited",
            "conditionalPresence": false,
            "blocks": [
              {
                "id": "05-09",
                "label": "5–9 AM",
                "score": 55,
                "activityLabel": "Moderate",
                "positiveDriver": "The measured water temperature is favorable for Steelhead.",
                "limitingFactor": "Hourly light and weather data are unavailable.",
                "cloudCoverPct": null,
                "precipitationIn": null
              },
              {
                "id": "09-13",
                "label": "9 AM–1 PM",
                "score": 55,
                "activityLabel": "Moderate",
                "positiveDriver": "The measured water temperature is favorable for Steelhead.",
                "limitingFactor": "Hourly light and weather data are unavailable.",
                "cloudCoverPct": null,
                "precipitationIn": null
              },
              {
                "id": "13-17",
                "label": "1–5 PM",
                "score": 55,
                "activityLabel": "Moderate",
                "positiveDriver": "The measured water temperature is favorable for Steelhead.",
                "limitingFactor": "Hourly light and weather data are unavailable.",
                "cloudCoverPct": null,
                "precipitationIn": null
              },
              {
                "id": "17-21",
                "label": "5–9 PM",
                "score": 55,
                "activityLabel": "Moderate",
                "positiveDriver": "The measured water temperature is favorable for Steelhead.",
                "limitingFactor": "Hourly light and weather data are unavailable.",
                "cloudCoverPct": null,
                "precipitationIn": null
              }
            ],
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "activity_limited_inactive",
        "label": "Limited · inactive severe conditions",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "activity": {
            "score": 1,
            "maximum": 100,
            "label": "Inactive",
            "headline": "Today’s PM Steelhead responsiveness is inactive if fish are present.",
            "detail": "No time block broadly supports an aggressive Steelhead response. Hourly light and weather data are unavailable, so no time block can be separated. Confidence is Limited because several key inputs are unavailable.",
            "tip": "Treat every block as unfavorable. Keep presentations conservative and continue only if direct fish response supports it.",
            "reasonCodes": [
              "activity_confidence_limited",
              "activity_today",
              "activity_run_present",
              "activity_temperature_barrier_cap",
              "activity_blown_out_cap"
            ],
            "rulesVersion": "pm-fall-steelhead-activity-v2",
            "targetDate": "2026-10-15",
            "targetDayLabel": "Today",
            "confidence": "Limited",
            "conditionalPresence": false,
            "blocks": [
              {
                "id": "05-09",
                "label": "5–9 AM",
                "score": 1,
                "activityLabel": "Inactive",
                "positiveDriver": "The river level and its recent change are favorable.",
                "limitingFactor": "Water temperature limits responsiveness.",
                "cloudCoverPct": null,
                "precipitationIn": null
              },
              {
                "id": "09-13",
                "label": "9 AM–1 PM",
                "score": 1,
                "activityLabel": "Inactive",
                "positiveDriver": "The river level and its recent change are favorable.",
                "limitingFactor": "Water temperature limits responsiveness.",
                "cloudCoverPct": null,
                "precipitationIn": null
              },
              {
                "id": "13-17",
                "label": "1–5 PM",
                "score": 1,
                "activityLabel": "Inactive",
                "positiveDriver": "The river level and its recent change are favorable.",
                "limitingFactor": "Water temperature limits responsiveness.",
                "cloudCoverPct": null,
                "precipitationIn": null
              },
              {
                "id": "17-21",
                "label": "5–9 PM",
                "score": 1,
                "activityLabel": "Inactive",
                "positiveDriver": "The river level and its recent change are favorable.",
                "limitingFactor": "Water temperature limits responsiveness.",
                "cloudCoverPct": null,
                "precipitationIn": null
              }
            ],
            "copyVersion": "river-run-copy-v35"
          }
        }
      }
    ]
  },
  {
    "id": "fish_in_river",
    "label": "Fish In River",
    "scenarios": [
      {
        "id": "fish_in_river_pre_run_not_expected_yet",
        "label": "0 / 100 · Outside",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-08-14",
          "progressionSnapshotAt": "2026-08-14T12:00:00.000Z",
          "conditionRefreshAt": "2026-08-14T20:00:00.000Z",
          "progressionExpiresAt": "2026-08-14T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-08-14T22:00:00.000Z",
          "runStage": {
            "stage": "pre_run",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-08-14",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Before migration",
            "headline": "The PM Steelhead fall run has not started.",
            "detail": "Seasonal timing does not support dependable Steelhead presence in the PM river yet.",
            "tip": "Keep effort in the lake, harbor, and Pere Marquette Lake until staging begins.",
            "whereToStart": "Stay in Lake Michigan, Ludington harbor, and Pere Marquette Lake. The PM river is not a dependable fall-entry section yet.",
            "reasonCodes": [
              "stage_pre_run"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Not monitoring yet",
            "timingLabel": null,
            "candidateLabel": null,
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-09-20",
            "completedCheckpointCount": 0,
            "currentIndex": null,
            "currentPercentile": null,
            "gaugeResponsePercentile": null,
            "waterTemperaturePercentile": null,
            "usableDays": 0,
            "expectedDays": 0,
            "coveragePercent": 0,
            "historicalYears": 0,
            "sourceDates": [],
            "sourceRefreshSlots": {},
            "headline": "PM Migration Timing is not monitoring yet.",
            "detail": "Season-to-date Scottville flow and M-37 temperature monitoring resumes in early September.",
            "tip": "Check back in early September when PM timing monitoring resumes.",
            "reasonCodes": [
              "conditions_monitoring_inactive"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "push": {
            "score": null,
            "label": "Waiting for migration",
            "headline": "Dependable PM Steelhead fall entry has not started.",
            "detail": "Rain, Scottville flow, and water temperature are not scored as an in-season movement signal yet.",
            "tip": "Use Migration Stage. Do not move inland because offseason water resembles an in-season Push.",
            "reasonCodes": [
              "push_tracking_not_started"
            ],
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "not_started",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-08-14",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": []
          },
          "fishInRiver": {
            "score": 0,
            "displayScore": 0,
            "scoreIsApproximate": false,
            "stage": "pre_run",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0,
            "curveDirection": "outside",
            "winterHoldingContext": false,
            "label": "Not expected yet",
            "headline": "Dependable Steelhead presence is not expected in the PM river yet.",
            "detail": "The seasonal estimate remains at zero. Any fish already in the river would be an early exception.",
            "tip": "Use Migration Stage for nearby staging context. Do not treat one early fish as a dependable river opportunity.",
            "reasonCodes": [
              "stage_pre_run",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-08-14T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-08-14T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-08-14T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "The river should fish well, but seasonal fish presence is still low.",
            "detail": "Good flow makes presentations easier, but it does not put fish in the river. Stay low, fish the first travel lane entering deep resting water, make one complete pass, and move instead of waiting for numbers that have not developed.",
            "reasonCodes": [
              "good_fishability_low_presence"
            ]
          }
        }
      },
      {
        "id": "fish_in_river_low_presence_rising",
        "label": "8 / 100 · Rising",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-09-20",
          "progressionSnapshotAt": "2026-09-20T12:00:00.000Z",
          "conditionRefreshAt": "2026-09-20T20:00:00.000Z",
          "progressionExpiresAt": "2026-09-20T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-09-20T22:00:00.000Z",
          "runStage": {
            "stage": "beginning",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-09-20",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Beginning",
            "headline": "Seasonal timing supports the first Steelhead entering the Lower river.",
            "detail": "This is the opening river phase. Presence is expected to be scattered and uneven.",
            "tip": "Cover the Lower river first. Move upstream only after direct fish activity supports it.",
            "whereToStart": "Start in the Lower river (Pere Marquette Lake–Scottville).",
            "reasonCodes": [
              "stage_beginning"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Typical",
            "timingLabel": "Typical",
            "candidateLabel": "Typical",
            "checkpointId": "river_start",
            "checkpointDate": "2026-09-20",
            "cutoffDate": "2026-09-19",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-10-11",
            "completedCheckpointCount": 1,
            "currentIndex": 45.4,
            "currentPercentile": 40,
            "gaugeResponsePercentile": 40,
            "waterTemperaturePercentile": 49,
            "usableDays": 19,
            "expectedDays": 19,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00"
            },
            "headline": "The PM migration is progressing at its usual seasonal pace.",
            "detail": "Season-to-date Scottville river-rise activity and M-37 cooling are close to the usual PM pattern.",
            "tip": "Keep the PM section named by Migration Stage.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_river_start",
              "conditions_typical"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-09-20",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": []
          },
          "fishInRiver": {
            "score": 8,
            "displayScore": 10,
            "scoreIsApproximate": true,
            "stage": "beginning",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0.1,
            "curveDirection": "rising",
            "winterHoldingContext": false,
            "label": "Low presence",
            "headline": "Steelhead seasonal presence is low presence and building.",
            "detail": "Calendar timing places Steelhead presence at low presence, increasing as the fall migration builds. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_beginning",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-09-20T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "The river should fish well, but seasonal fish presence is still low.",
            "detail": "Good flow makes presentations easier, but it does not put fish in the river. Stay low, fish the first travel lane entering deep resting water, make one complete pass, and move instead of waiting for numbers that have not developed.",
            "reasonCodes": [
              "good_fishability_low_presence"
            ]
          }
        }
      },
      {
        "id": "fish_in_river_limited_presence_rising",
        "label": "17 / 100 · Rising",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-10-02",
          "progressionSnapshotAt": "2026-10-02T12:00:00.000Z",
          "conditionRefreshAt": "2026-10-02T20:00:00.000Z",
          "progressionExpiresAt": "2026-10-02T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-10-02T22:00:00.000Z",
          "runStage": {
            "stage": "beginning",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-10-02",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Beginning",
            "headline": "Seasonal timing supports the first Steelhead entering the Lower river.",
            "detail": "This is the opening river phase. Presence is expected to be scattered and uneven.",
            "tip": "Cover the Lower river first. Move upstream only after direct fish activity supports it.",
            "whereToStart": "Start in the Lower river (Pere Marquette Lake–Scottville).",
            "reasonCodes": [
              "stage_beginning"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Typical",
            "timingLabel": "Typical",
            "candidateLabel": "Typical",
            "checkpointId": "river_start",
            "checkpointDate": "2026-09-20",
            "cutoffDate": "2026-09-19",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-10-11",
            "completedCheckpointCount": 1,
            "currentIndex": 45.4,
            "currentPercentile": 40,
            "gaugeResponsePercentile": 40,
            "waterTemperaturePercentile": 49,
            "usableDays": 19,
            "expectedDays": 19,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00"
            },
            "headline": "The PM migration is progressing at its usual seasonal pace.",
            "detail": "Season-to-date Scottville river-rise activity and M-37 cooling are close to the usual PM pattern.",
            "tip": "Keep the PM section named by Migration Stage.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_river_start",
              "conditions_typical"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-10-02",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-10-01",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-09-30",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-09-29",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-09-29T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-09-28",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-09-27",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-09-27T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-09-26",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-09-26T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-09-25",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": 17,
            "displayScore": 20,
            "scoreIsApproximate": true,
            "stage": "beginning",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0.21666666666666667,
            "curveDirection": "rising",
            "winterHoldingContext": false,
            "label": "Limited presence",
            "headline": "Steelhead seasonal presence is limited presence and building.",
            "detail": "Calendar timing places Steelhead presence at limited presence, increasing as the fall migration builds. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_beginning",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-10-02T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-10-02T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-10-02T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "The river should fish well, but seasonal fish presence is still low.",
            "detail": "Good flow makes presentations easier, but it does not put fish in the river. Stay low, fish the first travel lane entering deep resting water, make one complete pass, and move instead of waiting for numbers that have not developed.",
            "reasonCodes": [
              "good_fishability_low_presence"
            ]
          }
        }
      },
      {
        "id": "fish_in_river_moderate_presence_rising",
        "label": "33 / 100 · Rising",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-10-13",
          "progressionSnapshotAt": "2026-10-13T12:00:00.000Z",
          "conditionRefreshAt": "2026-10-13T20:00:00.000Z",
          "progressionExpiresAt": "2026-10-13T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-10-13T22:00:00.000Z",
          "runStage": {
            "stage": "building",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-10-13",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Building",
            "headline": "The PM Steelhead run is building beyond its earliest entry phase.",
            "detail": "Seasonal timing keeps the Lower river primary. Earlier arrivals may begin reaching the Middle river.",
            "tip": "Start low, then make one Middle river check if direct fish activity supports the move.",
            "whereToStart": "Start in the Lower river (Pere Marquette Lake–Scottville). Add the Middle river (Scottville–Maple Leaf) after direct fish activity supports the move.",
            "reasonCodes": [
              "stage_building"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Ahead",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "building_start",
            "checkpointDate": "2026-10-11",
            "cutoffDate": "2026-10-10",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-10-15",
            "completedCheckpointCount": 2,
            "currentIndex": 80,
            "currentPercentile": 80,
            "gaugeResponsePercentile": 80,
            "waterTemperaturePercentile": 80,
            "usableDays": 40,
            "expectedDays": 40,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00"
            },
            "headline": "The PM migration is developing earlier than usual.",
            "detail": "Season-to-date Scottville river-rise activity and M-37 cooling are stronger than the usual PM pattern.",
            "tip": "Start one PM section upstream from Migration Stage, capped at the Upper river (Maple Leaf–M-37).",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_building_start",
              "conditions_ahead"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "river_start",
            "previousCheckpointDate": "2026-09-20",
            "previousTimingLabel": "Typical"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-10-13",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-10-12",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-11",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-10",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-10-10T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-09",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-08",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-10-08T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-10-07",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-10-07T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-06",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": 33,
            "displayScore": 35,
            "scoreIsApproximate": true,
            "stage": "building",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0.41,
            "curveDirection": "rising",
            "winterHoldingContext": false,
            "label": "Moderate presence",
            "headline": "Steelhead seasonal presence is moderate presence and building.",
            "detail": "Calendar timing places Steelhead presence at moderate presence, increasing as the fall migration builds. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_building",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-10-13T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-10-13T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-10-13T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          }
        }
      },
      {
        "id": "fish_in_river_high_presence_rising",
        "label": "49 / 100 · Rising",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-10-24",
          "progressionSnapshotAt": "2026-10-24T12:00:00.000Z",
          "conditionRefreshAt": "2026-10-24T20:00:00.000Z",
          "progressionExpiresAt": "2026-10-24T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-10-24T22:00:00.000Z",
          "runStage": {
            "stage": "building",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-10-24",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Building",
            "headline": "The PM Steelhead run is becoming established in the Middle river.",
            "detail": "Seasonal timing now favors the Middle river. The Upper river remains a conditional secondary choice.",
            "tip": "Begin in the Middle river. Add the Upper river only after direct fish activity supports it.",
            "whereToStart": "Start in the Middle river (Scottville–Maple Leaf). Add the Upper river (Maple Leaf–M-37) only after direct fish activity supports it.",
            "reasonCodes": [
              "stage_building"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-10-24",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-10-23",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-22",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-21",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-10-21T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-20",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-19",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-10-19T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-10-18",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-10-18T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-17",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": 49,
            "displayScore": 50,
            "scoreIsApproximate": true,
            "stage": "building",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0.6088235294117648,
            "curveDirection": "rising",
            "winterHoldingContext": false,
            "label": "High presence",
            "headline": "Steelhead seasonal presence is high presence and building.",
            "detail": "Calendar timing places Steelhead presence high for this fall migration, increasing as the fall migration builds. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_building",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-10-24T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-10-24T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-10-24T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          }
        }
      },
      {
        "id": "fish_in_river_high_presence_rising_upper_shoulder",
        "label": "64 / 100 · Rising",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-11-04",
          "progressionSnapshotAt": "2026-11-04T12:00:00.000Z",
          "conditionRefreshAt": "2026-11-04T20:00:00.000Z",
          "progressionExpiresAt": "2026-11-04T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-11-04T22:00:00.000Z",
          "runStage": {
            "stage": "building",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": true,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-11-04",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Building",
            "headline": "Seasonal timing supports a broader PM Steelhead distribution.",
            "detail": "The Middle river is the dependable calendar choice. Earlier arrivals may also occupy the Upper river.",
            "tip": "Cover one Middle river section completely before comparing the Upper river.",
            "whereToStart": "Start in the Middle river (Scottville–Maple Leaf). Add the Upper river (Maple Leaf–M-37) when direct fish activity supports it.",
            "reasonCodes": [
              "stage_building"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-11-04",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-11-03",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-02",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-01",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-11-01T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-31",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-30",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-10-30T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-10-29",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-10-29T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-28",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": 64,
            "displayScore": 65,
            "scoreIsApproximate": true,
            "stage": "building",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0.8035714285714286,
            "curveDirection": "rising",
            "winterHoldingContext": false,
            "label": "High presence",
            "headline": "Steelhead seasonal presence is high presence and building.",
            "detail": "Calendar timing places Steelhead presence high for this fall migration, increasing as the fall migration builds. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_building",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-11-04T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-11-04T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-11-04T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          }
        }
      },
      {
        "id": "fish_in_river_peak_presence_rising",
        "label": "73 / 100 · Rising",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-11-10",
          "progressionSnapshotAt": "2026-11-10T12:00:00.000Z",
          "conditionRefreshAt": "2026-11-10T20:00:00.000Z",
          "progressionExpiresAt": "2026-11-10T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-11-10T22:00:00.000Z",
          "runStage": {
            "stage": "building",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": true,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-11-10",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Building",
            "headline": "Seasonal timing supports a broader PM Steelhead distribution.",
            "detail": "The Middle river is the dependable calendar choice. Earlier arrivals may also occupy the Upper river.",
            "tip": "Cover one Middle river section completely before comparing the Upper river.",
            "whereToStart": "Start in the Middle river (Scottville–Maple Leaf). Add the Upper river (Maple Leaf–M-37) when direct fish activity supports it.",
            "reasonCodes": [
              "stage_building"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-11-10",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-11-09",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-08",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-07",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-11-07T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-06",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-05",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-11-05T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-11-04",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-11-04T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-03",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": 73,
            "displayScore": 75,
            "scoreIsApproximate": true,
            "stage": "building",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0.9107142857142857,
            "curveDirection": "rising",
            "winterHoldingContext": false,
            "label": "Peak presence",
            "headline": "Steelhead seasonal presence is peak presence and building.",
            "detail": "Calendar timing places Steelhead presence at its expected seasonal peak, increasing as the fall migration builds. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_building",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-11-10T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-11-10T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-11-10T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          }
        }
      },
      {
        "id": "fish_in_river_peak_presence_near_peak",
        "label": "80 / 100 · Near peak",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-11-15",
          "progressionSnapshotAt": "2026-11-15T12:00:00.000Z",
          "conditionRefreshAt": "2026-11-15T20:00:00.000Z",
          "progressionExpiresAt": "2026-11-15T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-11-15T22:00:00.000Z",
          "runStage": {
            "stage": "peak",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-11-15",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Peak",
            "headline": "This is typically the PM’s strongest Steelhead migration window.",
            "detail": "Seasonal timing supports the widest dependable Steelhead distribution. It does not confirm fish in every section.",
            "tip": "Start in the Middle river. Compare the Upper river only after a complete first pass.",
            "whereToStart": "Start in the Middle river (Scottville–Maple Leaf). Compare the Upper river (Maple Leaf–M-37) when direct fish activity favors it.",
            "reasonCodes": [
              "stage_peak"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Ahead",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "peak_start",
            "checkpointDate": "2026-11-15",
            "cutoffDate": "2026-11-14",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-11-21",
            "completedCheckpointCount": 4,
            "currentIndex": 92.5,
            "currentPercentile": 100,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 87.5,
            "usableDays": 75,
            "expectedDays": 75,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00"
            },
            "headline": "The PM migration is developing earlier than usual.",
            "detail": "Season-to-date Scottville river-rise activity and M-37 cooling are stronger than the usual PM pattern.",
            "tip": "Start one PM section upstream from Migration Stage, capped at the Upper river (Maple Leaf–M-37).",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_start",
              "conditions_ahead"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "building_established",
            "previousCheckpointDate": "2026-10-15",
            "previousTimingLabel": "Ahead"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-11-15",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-11-14",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-13",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-12",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-11-12T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-11",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-10",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-11-10T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-11-09",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-11-09T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-08",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": 80,
            "displayScore": 80,
            "scoreIsApproximate": false,
            "stage": "peak",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 1,
            "curveDirection": "near_peak",
            "winterHoldingContext": false,
            "label": "Peak presence",
            "headline": "Steelhead seasonal presence is peak presence and near its seasonal high.",
            "detail": "Calendar timing places Steelhead presence at its expected seasonal peak, near its expected seasonal high. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_peak",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-11-15T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-11-15T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-11-15T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "Seasonal presence is near peak, but today's water does not point to a fresh wave.",
            "detail": "Many fish may already be in the river even when rain, river level, and temperature show little support for new movement. Start in established deep holes, fish each one from head to tail, and leave lower entry lanes secondary.",
            "reasonCodes": [
              "peak_presence_weak_push"
            ]
          }
        }
      },
      {
        "id": "fish_in_river_peak_presence_falling",
        "label": "79 / 100 · Falling",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-12-05",
          "progressionSnapshotAt": "2026-12-05T12:00:00.000Z",
          "conditionRefreshAt": "2026-12-05T20:00:00.000Z",
          "progressionExpiresAt": "2026-12-05T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-12-05T22:00:00.000Z",
          "runStage": {
            "stage": "tapering",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-12-05",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Late fall",
            "headline": "PM Steelhead fall entry remains strong but is slowing.",
            "detail": "Seasonal timing still supports broad fall presence. Colder water makes established holding water more important than new entry.",
            "tip": "Start in the Middle river. Shift lower only when Push supports fresh movement.",
            "whereToStart": "Start in the Middle river (Scottville–Maple Leaf). Add the Lower river (Pere Marquette Lake–Scottville) only when Push supports fresh movement.",
            "reasonCodes": [
              "stage_tapering"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Timing complete",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "peak_complete",
            "checkpointDate": "2026-11-21",
            "cutoffDate": "2026-11-20",
            "observationStartDate": "2026-09-01",
            "completedCheckpointCount": 5,
            "currentIndex": 92.5,
            "currentPercentile": 100,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 87.5,
            "usableDays": 81,
            "expectedDays": 81,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14",
              "2026-11-15",
              "2026-11-16",
              "2026-11-17",
              "2026-11-18",
              "2026-11-19",
              "2026-11-20"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00",
              "2026-11-15": "16:00",
              "2026-11-16": "16:00",
              "2026-11-17": "16:00",
              "2026-11-18": "16:00",
              "2026-11-19": "16:00",
              "2026-11-20": "16:00"
            },
            "headline": "The early-season timing read is complete.",
            "detail": "Earlier in the season, the migration was moving earlier than usual. Now that the migration is well underway, current movement and river conditions matter more than an early-or-late call.",
            "tip": "Stop shifting river sections based on early-or-late timing. Begin in established holding water for the current Migration Stage; if Push is Possible or stronger, make lower travel lanes the next stop.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_complete",
              "conditions_ahead",
              "conditions_timing_complete"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "peak_start",
            "previousCheckpointDate": "2026-11-15",
            "previousTimingLabel": "Ahead"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-12-05",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-12-04",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-03",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-02",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-12-02T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-01",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-30",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-11-30T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-11-29",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-11-29T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-28",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": 79,
            "displayScore": 80,
            "scoreIsApproximate": true,
            "stage": "tapering",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0.99,
            "curveDirection": "falling",
            "winterHoldingContext": false,
            "label": "Peak presence",
            "headline": "Steelhead seasonal presence is peak presence and declining.",
            "detail": "Calendar timing places Steelhead presence at its expected seasonal peak, declining as the fall migration advances. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_tapering",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-12-05T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-12-05T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-12-05T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          }
        }
      },
      {
        "id": "fish_in_river_high_presence_falling_upper_shoulder",
        "label": "71 / 100 · Falling",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-12-20",
          "progressionSnapshotAt": "2026-12-20T12:00:00.000Z",
          "conditionRefreshAt": "2026-12-20T20:00:00.000Z",
          "progressionExpiresAt": "2026-12-20T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-12-20T22:00:00.000Z",
          "runStage": {
            "stage": "ending",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-12-20",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Holding transition",
            "headline": "PM Steelhead fall entry is in its final seasonal phase.",
            "detail": "Steelhead may remain after fall entry ends. This state marks the model boundary, not fish leaving the river.",
            "tip": "Prioritize established Middle river holding water and use Activity only through the final fall-entry day.",
            "whereToStart": "Start in the Middle river (Scottville–Maple Leaf). Add the Upper river (Maple Leaf–M-37) only for established late-fall holding water.",
            "reasonCodes": [
              "stage_ending"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Timing complete",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "peak_complete",
            "checkpointDate": "2026-11-21",
            "cutoffDate": "2026-11-20",
            "observationStartDate": "2026-09-01",
            "completedCheckpointCount": 5,
            "currentIndex": 92.5,
            "currentPercentile": 100,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 87.5,
            "usableDays": 81,
            "expectedDays": 81,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14",
              "2026-11-15",
              "2026-11-16",
              "2026-11-17",
              "2026-11-18",
              "2026-11-19",
              "2026-11-20"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00",
              "2026-11-15": "16:00",
              "2026-11-16": "16:00",
              "2026-11-17": "16:00",
              "2026-11-18": "16:00",
              "2026-11-19": "16:00",
              "2026-11-20": "16:00"
            },
            "headline": "The early-season timing read is complete.",
            "detail": "Earlier in the season, the migration was moving earlier than usual. Now that the migration is well underway, current movement and river conditions matter more than an early-or-late call.",
            "tip": "Stop shifting river sections based on early-or-late timing. Begin in established holding water for the current Migration Stage; if Push is Possible or stronger, make lower travel lanes the next stop.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_complete",
              "conditions_ahead",
              "conditions_timing_complete"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "peak_start",
            "previousCheckpointDate": "2026-11-15",
            "previousTimingLabel": "Ahead"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-12-20",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-12-19",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-18",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-17",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-12-17T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-16",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-15",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-12-15T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-12-14",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-12-14T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-13",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": 71,
            "displayScore": 70,
            "scoreIsApproximate": true,
            "stage": "ending",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0.8916666666666667,
            "curveDirection": "falling",
            "winterHoldingContext": false,
            "label": "High presence",
            "headline": "Steelhead seasonal presence is high presence and declining.",
            "detail": "Calendar timing places Steelhead presence high for this fall migration, declining as the fall migration advances. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_ending",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-12-20T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-12-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-12-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          }
        }
      },
      {
        "id": "fish_in_river_fall_entry_complete_outside",
        "label": "null / 100 · Outside",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-12-23",
          "progressionSnapshotAt": "2026-12-23T12:00:00.000Z",
          "conditionRefreshAt": "2026-12-23T20:00:00.000Z",
          "progressionExpiresAt": "2026-12-23T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-12-23T22:00:00.000Z",
          "runStage": {
            "stage": "post_run",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-12-23",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Fall entry complete",
            "headline": "PM Steelhead fall entry is complete.",
            "detail": "Steelhead may remain in the river. This fall-entry model no longer scores their current presence or activity.",
            "tip": "Check back in early September when PM fall movement tracking resumes.",
            "whereToStart": "There is no active PM starting section in this fall-entry model.",
            "reasonCodes": [
              "stage_post_run",
              "stage_offseason"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Timing complete",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "peak_complete",
            "checkpointDate": "2026-11-21",
            "cutoffDate": "2026-11-20",
            "observationStartDate": "2026-09-01",
            "completedCheckpointCount": 5,
            "currentIndex": 92.5,
            "currentPercentile": 100,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 87.5,
            "usableDays": 81,
            "expectedDays": 81,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14",
              "2026-11-15",
              "2026-11-16",
              "2026-11-17",
              "2026-11-18",
              "2026-11-19",
              "2026-11-20"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00",
              "2026-11-15": "16:00",
              "2026-11-16": "16:00",
              "2026-11-17": "16:00",
              "2026-11-18": "16:00",
              "2026-11-19": "16:00",
              "2026-11-20": "16:00"
            },
            "headline": "PM fall-entry Migration Timing is complete.",
            "detail": "Earlier in fall, PM migration timing was earlier than usual. The fall-entry timing window is now complete.",
            "tip": "Do not use completed fall timing to infer current presence or activity. PM fall monitoring resumes in early September.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_complete",
              "conditions_ahead",
              "conditions_timing_complete"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "peak_start",
            "previousCheckpointDate": "2026-11-15",
            "previousTimingLabel": "Ahead"
          },
          "push": {
            "score": null,
            "label": "Fall entry complete",
            "headline": "PM Steelhead fall-entry Push is complete.",
            "detail": "Current water may affect Steelhead still in the river. This fall model no longer scores fresh-entry support.",
            "tip": "Do not use a completed fall Push to infer current presence. Check back in early September.",
            "reasonCodes": [
              "push_tracking_complete"
            ],
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "complete",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-12-22",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-12-22",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-21",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-20",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-12-20T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-19",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-18",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-12-18T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-12-17",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-12-17T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-16",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": null,
            "scoreIsApproximate": false,
            "stage": "post_run",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0,
            "curveDirection": "outside",
            "winterHoldingContext": false,
            "label": "Fall entry complete",
            "headline": "PM Steelhead fall entry is complete.",
            "detail": "Steelhead may remain in the river. This fall-entry model no longer estimates their current seasonal presence.",
            "tip": "Check back in early September when PM fall movement tracking resumes.",
            "reasonCodes": [
              "stage_post_run",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-12-23T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-12-23T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-12-23T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          }
        }
      },
      {
        "id": "fish_in_river_moderate_cap",
        "label": "QA only · moderate sectional run · 60 / 100",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-11-15",
          "progressionSnapshotAt": "2026-11-15T12:00:00.000Z",
          "conditionRefreshAt": "2026-11-15T20:00:00.000Z",
          "progressionExpiresAt": "2026-11-15T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-11-15T22:00:00.000Z",
          "runStage": {
            "stage": "peak",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-11-15",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Peak",
            "headline": "This is typically the PM’s strongest Steelhead migration window.",
            "detail": "Seasonal timing supports the widest dependable Steelhead distribution. It does not confirm fish in every section.",
            "tip": "Start in the Middle river. Compare the Upper river only after a complete first pass.",
            "whereToStart": "Start in the Middle river (Scottville–Maple Leaf). Compare the Upper river (Maple Leaf–M-37) when direct fish activity favors it.",
            "reasonCodes": [
              "stage_peak"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Ahead",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "peak_start",
            "checkpointDate": "2026-11-15",
            "cutoffDate": "2026-11-14",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-11-21",
            "completedCheckpointCount": 4,
            "currentIndex": 92.5,
            "currentPercentile": 100,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 87.5,
            "usableDays": 75,
            "expectedDays": 75,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00"
            },
            "headline": "The PM migration is developing earlier than usual.",
            "detail": "Season-to-date Scottville river-rise activity and M-37 cooling are stronger than the usual PM pattern.",
            "tip": "Start one PM section upstream from Migration Stage, capped at the Upper river (Maple Leaf–M-37).",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_start",
              "conditions_ahead"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "building_established",
            "previousCheckpointDate": "2026-10-15",
            "previousTimingLabel": "Ahead"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-11-15",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-11-14",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-13",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-12",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-11-12T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-11",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-10",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-11-10T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-11-09",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-11-09T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-08",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": 60,
            "displayScore": 60,
            "scoreIsApproximate": false,
            "stage": "peak",
            "maximum": 100,
            "riverCeiling": 60,
            "historicalRunStrength": "moderate",
            "curveFraction": 1,
            "curveDirection": "near_peak",
            "winterHoldingContext": false,
            "label": "Peak presence",
            "headline": "Steelhead seasonal presence is peak presence and near its seasonal high.",
            "detail": "Calendar timing places Steelhead presence at its expected seasonal peak, near its expected seasonal high. For the PM, expected run strength is Moderate, and the opportunity centers on dependable river sections. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_peak",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-11-15T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-11-15T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-11-15T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "Seasonal presence is near peak, but today's water does not point to a fresh wave.",
            "detail": "Fish may already be established in the river's dependable sections even when rain, river level, and temperature show little support for new movement. Start in established deep holes, fish each one from head to tail, and leave lower entry lanes secondary.",
            "reasonCodes": [
              "peak_presence_weak_push"
            ]
          }
        }
      }
    ]
  },
  {
    "id": "opportunity_copy",
    "label": "Opportunity Copy Tiers",
    "scenarios": [
      {
        "id": "opportunity_moderate_stage_building",
        "label": "Moderate / sectional · Stage · Building",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "runId": "qa_moderate_opportunity",
          "fishInRiver": {
            "score": 27,
            "displayScore": 25,
            "scoreIsApproximate": true,
            "stage": "building",
            "maximum": 100,
            "riverCeiling": 60,
            "historicalRunStrength": "moderate",
            "curveFraction": 0.45,
            "curveDirection": "rising",
            "winterHoldingContext": false,
            "label": "Moderate presence",
            "headline": "Steelhead seasonal presence is moderate presence and building.",
            "detail": "Calendar timing places Steelhead presence at moderate presence, increasing as the fall migration builds. For the PM, expected run strength is Moderate, and the opportunity centers on dependable river sections. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_building",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "opportunity_moderate_stage_peak",
        "label": "Moderate / sectional · Stage · Peak",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "runId": "qa_moderate_opportunity",
          "localDate": "2026-11-15",
          "progressionSnapshotAt": "2026-11-15T12:00:00.000Z",
          "conditionRefreshAt": "2026-11-15T20:00:00.000Z",
          "progressionExpiresAt": "2026-11-15T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-11-15T22:00:00.000Z",
          "runStage": {
            "stage": "peak",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-11-15",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Peak",
            "headline": "This is typically the PM’s strongest Coho window, within a moderate river-specific run.",
            "detail": "Seasonal timing supports the PM’s core Steelhead sections. Concentrations may remain uneven between them.",
            "tip": "Start in the Middle river. Compare the Upper river only after a complete first pass.",
            "whereToStart": "Start in the Middle river (Scottville–Maple Leaf). Compare the Upper river (Maple Leaf–M-37) when direct fish activity favors it.",
            "reasonCodes": [
              "stage_peak"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Ahead",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "peak_start",
            "checkpointDate": "2026-11-15",
            "cutoffDate": "2026-11-14",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-11-21",
            "completedCheckpointCount": 4,
            "currentIndex": 92.5,
            "currentPercentile": 100,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 87.5,
            "usableDays": 75,
            "expectedDays": 75,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00"
            },
            "headline": "The PM migration is developing earlier than usual.",
            "detail": "Season-to-date Scottville river-rise activity and M-37 cooling are stronger than the usual PM pattern.",
            "tip": "Start one PM section upstream from Migration Stage, capped at the Upper river (Maple Leaf–M-37).",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_start",
              "conditions_ahead"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "building_established",
            "previousCheckpointDate": "2026-10-15",
            "previousTimingLabel": "Ahead"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-11-15",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-11-14",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-13",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-12",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-11-12T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-11",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-10",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-11-10T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-11-09",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-11-09T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-08",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": 60,
            "displayScore": 60,
            "scoreIsApproximate": false,
            "stage": "peak",
            "maximum": 100,
            "riverCeiling": 60,
            "historicalRunStrength": "moderate",
            "curveFraction": 1,
            "curveDirection": "near_peak",
            "winterHoldingContext": false,
            "label": "Peak presence",
            "headline": "Steelhead seasonal presence is peak presence and near its seasonal high.",
            "detail": "Calendar timing places Steelhead presence at its expected seasonal peak, near its expected seasonal high. For the PM, expected run strength is Moderate, and the opportunity centers on dependable river sections. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_peak",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-11-15T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-11-15T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-11-15T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "Seasonal presence is near peak, but today's water does not point to a fresh wave.",
            "detail": "Fish may already be established in the river's dependable sections even when rain, river level, and temperature show little support for new movement. Start in established deep holes, fish each one from head to tail, and leave lower entry lanes secondary.",
            "reasonCodes": [
              "peak_presence_weak_push"
            ]
          }
        }
      },
      {
        "id": "opportunity_moderate_stage_tapering",
        "label": "Moderate / sectional · Stage · Tapering",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "runId": "qa_moderate_opportunity",
          "localDate": "2026-12-09",
          "progressionSnapshotAt": "2026-12-09T12:00:00.000Z",
          "conditionRefreshAt": "2026-12-09T20:00:00.000Z",
          "progressionExpiresAt": "2026-12-09T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-12-09T22:00:00.000Z",
          "runStage": {
            "stage": "tapering",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-12-09",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Late fall",
            "headline": "PM Steelhead fall entry remains strong but is slowing.",
            "detail": "Seasonal timing still supports broad fall presence. Colder water makes established holding water more important than new entry.",
            "tip": "Start in the Middle river. Shift lower only when Push supports fresh movement.",
            "whereToStart": "Start in the Middle river (Scottville–Maple Leaf). Add the Lower river (Pere Marquette Lake–Scottville) only when Push supports fresh movement.",
            "reasonCodes": [
              "stage_tapering"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Timing complete",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "peak_complete",
            "checkpointDate": "2026-11-21",
            "cutoffDate": "2026-11-20",
            "observationStartDate": "2026-09-01",
            "completedCheckpointCount": 5,
            "currentIndex": 92.5,
            "currentPercentile": 100,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 87.5,
            "usableDays": 81,
            "expectedDays": 81,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14",
              "2026-11-15",
              "2026-11-16",
              "2026-11-17",
              "2026-11-18",
              "2026-11-19",
              "2026-11-20"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00",
              "2026-11-15": "16:00",
              "2026-11-16": "16:00",
              "2026-11-17": "16:00",
              "2026-11-18": "16:00",
              "2026-11-19": "16:00",
              "2026-11-20": "16:00"
            },
            "headline": "The early-season timing read is complete.",
            "detail": "Earlier in the season, the migration was moving earlier than usual. Now that the migration is well underway, current movement and river conditions matter more than an early-or-late call.",
            "tip": "Stop shifting river sections based on early-or-late timing. Begin in established holding water for the current Migration Stage; if Push is Possible or stronger, make lower travel lanes the next stop.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_complete",
              "conditions_ahead",
              "conditions_timing_complete"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "peak_start",
            "previousCheckpointDate": "2026-11-15",
            "previousTimingLabel": "Ahead"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-12-09",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-12-08",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-07",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-06",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-12-06T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-05",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-04",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-12-04T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-12-03",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-12-03T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-02",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": 58,
            "displayScore": 60,
            "scoreIsApproximate": true,
            "stage": "tapering",
            "maximum": 100,
            "riverCeiling": 60,
            "historicalRunStrength": "moderate",
            "curveFraction": 0.9642857142857143,
            "curveDirection": "falling",
            "winterHoldingContext": false,
            "label": "Peak presence",
            "headline": "Steelhead seasonal presence is peak presence and declining.",
            "detail": "Calendar timing places Steelhead presence at its expected seasonal peak, declining as the fall migration advances. For the PM, expected run strength is Moderate, and the opportunity centers on dependable river sections. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_tapering",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-12-09T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-12-09T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-12-09T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          }
        }
      },
      {
        "id": "opportunity_moderate_stage_ending",
        "label": "Moderate / sectional · Stage · Ending",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "runId": "qa_moderate_opportunity",
          "localDate": "2026-12-23",
          "progressionSnapshotAt": "2026-12-23T12:00:00.000Z",
          "conditionRefreshAt": "2026-12-23T20:00:00.000Z",
          "progressionExpiresAt": "2026-12-23T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-12-23T22:00:00.000Z",
          "runStage": {
            "stage": "post_run",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-12-23",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Fall entry complete",
            "headline": "PM Steelhead fall entry is complete.",
            "detail": "Steelhead may remain in the river. This fall-entry model no longer scores their current presence or activity.",
            "tip": "Check back in early September when PM fall movement tracking resumes.",
            "whereToStart": "There is no active PM starting section in this fall-entry model.",
            "reasonCodes": [
              "stage_post_run",
              "stage_offseason"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Timing complete",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "peak_complete",
            "checkpointDate": "2026-11-21",
            "cutoffDate": "2026-11-20",
            "observationStartDate": "2026-09-01",
            "completedCheckpointCount": 5,
            "currentIndex": 92.5,
            "currentPercentile": 100,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 87.5,
            "usableDays": 81,
            "expectedDays": 81,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14",
              "2026-11-15",
              "2026-11-16",
              "2026-11-17",
              "2026-11-18",
              "2026-11-19",
              "2026-11-20"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00",
              "2026-11-15": "16:00",
              "2026-11-16": "16:00",
              "2026-11-17": "16:00",
              "2026-11-18": "16:00",
              "2026-11-19": "16:00",
              "2026-11-20": "16:00"
            },
            "headline": "PM fall-entry Migration Timing is complete.",
            "detail": "Earlier in fall, PM migration timing was earlier than usual. The fall-entry timing window is now complete.",
            "tip": "Do not use completed fall timing to infer current presence or activity. PM fall monitoring resumes in early September.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_complete",
              "conditions_ahead",
              "conditions_timing_complete"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "peak_start",
            "previousCheckpointDate": "2026-11-15",
            "previousTimingLabel": "Ahead"
          },
          "push": {
            "score": null,
            "label": "Fall entry complete",
            "headline": "PM Steelhead fall-entry Push is complete.",
            "detail": "Current water may affect Steelhead still in the river. This fall model no longer scores fresh-entry support.",
            "tip": "Do not use a completed fall Push to infer current presence. Check back in early September.",
            "reasonCodes": [
              "push_tracking_complete"
            ],
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "complete",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-12-22",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-12-22",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-21",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-20",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-12-20T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-19",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-18",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-12-18T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-12-17",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-12-17T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-16",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": null,
            "scoreIsApproximate": false,
            "stage": "post_run",
            "maximum": 100,
            "riverCeiling": 60,
            "historicalRunStrength": "moderate",
            "curveFraction": 0,
            "curveDirection": "outside",
            "winterHoldingContext": false,
            "label": "Fall entry complete",
            "headline": "PM Steelhead fall entry is complete.",
            "detail": "Steelhead may remain in the river. This fall-entry model no longer estimates their current seasonal presence.",
            "tip": "Check back in early September when PM fall movement tracking resumes.",
            "reasonCodes": [
              "stage_post_run",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-12-23T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-12-23T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-12-23T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          }
        }
      },
      {
        "id": "opportunity_moderate_presence_rising",
        "label": "Moderate / sectional · Presence · rising",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "runId": "qa_moderate_opportunity",
          "localDate": "2026-11-14",
          "progressionSnapshotAt": "2026-11-14T12:00:00.000Z",
          "conditionRefreshAt": "2026-11-14T20:00:00.000Z",
          "progressionExpiresAt": "2026-11-14T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-11-14T22:00:00.000Z",
          "runStage": {
            "stage": "building",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": true,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-11-14",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Building",
            "headline": "Seasonal timing supports a broader PM Steelhead distribution.",
            "detail": "The Middle river is the dependable calendar choice. Earlier arrivals may also occupy the Upper river.",
            "tip": "Cover one Middle river section completely before comparing the Upper river.",
            "whereToStart": "Start in the Middle river (Scottville–Maple Leaf). Add the Upper river (Maple Leaf–M-37) when direct fish activity supports it.",
            "reasonCodes": [
              "stage_building"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-11-14",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-11-13",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-12",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-11",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-11-11T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-10",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-09",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-11-09T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-11-08",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-11-08T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-07",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": 59,
            "displayScore": 60,
            "scoreIsApproximate": true,
            "stage": "building",
            "maximum": 100,
            "riverCeiling": 60,
            "historicalRunStrength": "moderate",
            "curveFraction": 0.9821428571428572,
            "curveDirection": "rising",
            "winterHoldingContext": false,
            "label": "Peak presence",
            "headline": "Steelhead seasonal presence is peak presence and building.",
            "detail": "Calendar timing places Steelhead presence at its expected seasonal peak, increasing as the fall migration builds. For the PM, expected run strength is Moderate, and the opportunity centers on dependable river sections. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_building",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-11-14T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-11-14T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-11-14T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          }
        }
      },
      {
        "id": "opportunity_moderate_presence_peak",
        "label": "Moderate / sectional · Presence · peak",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "runId": "qa_moderate_opportunity",
          "localDate": "2026-11-15",
          "progressionSnapshotAt": "2026-11-15T12:00:00.000Z",
          "conditionRefreshAt": "2026-11-15T20:00:00.000Z",
          "progressionExpiresAt": "2026-11-15T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-11-15T22:00:00.000Z",
          "runStage": {
            "stage": "peak",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-11-15",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Peak",
            "headline": "This is typically the PM’s strongest Coho window, within a moderate river-specific run.",
            "detail": "Seasonal timing supports the PM’s core Steelhead sections. Concentrations may remain uneven between them.",
            "tip": "Start in the Middle river. Compare the Upper river only after a complete first pass.",
            "whereToStart": "Start in the Middle river (Scottville–Maple Leaf). Compare the Upper river (Maple Leaf–M-37) when direct fish activity favors it.",
            "reasonCodes": [
              "stage_peak"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Ahead",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "peak_start",
            "checkpointDate": "2026-11-15",
            "cutoffDate": "2026-11-14",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-11-21",
            "completedCheckpointCount": 4,
            "currentIndex": 92.5,
            "currentPercentile": 100,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 87.5,
            "usableDays": 75,
            "expectedDays": 75,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00"
            },
            "headline": "The PM migration is developing earlier than usual.",
            "detail": "Season-to-date Scottville river-rise activity and M-37 cooling are stronger than the usual PM pattern.",
            "tip": "Start one PM section upstream from Migration Stage, capped at the Upper river (Maple Leaf–M-37).",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_start",
              "conditions_ahead"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "building_established",
            "previousCheckpointDate": "2026-10-15",
            "previousTimingLabel": "Ahead"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-11-15",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-11-14",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-13",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-12",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-11-12T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-11",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-10",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-11-10T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-11-09",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-11-09T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-08",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": 60,
            "displayScore": 60,
            "scoreIsApproximate": false,
            "stage": "peak",
            "maximum": 100,
            "riverCeiling": 60,
            "historicalRunStrength": "moderate",
            "curveFraction": 1,
            "curveDirection": "near_peak",
            "winterHoldingContext": false,
            "label": "Peak presence",
            "headline": "Steelhead seasonal presence is peak presence and near its seasonal high.",
            "detail": "Calendar timing places Steelhead presence at its expected seasonal peak, near its expected seasonal high. For the PM, expected run strength is Moderate, and the opportunity centers on dependable river sections. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_peak",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-11-15T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-11-15T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-11-15T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "Seasonal presence is near peak, but today's water does not point to a fresh wave.",
            "detail": "Fish may already be established in the river's dependable sections even when rain, river level, and temperature show little support for new movement. Start in established deep holes, fish each one from head to tail, and leave lower entry lanes secondary.",
            "reasonCodes": [
              "peak_presence_weak_push"
            ]
          }
        }
      },
      {
        "id": "opportunity_moderate_presence_falling",
        "label": "Moderate / sectional · Presence · falling",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "runId": "qa_moderate_opportunity",
          "localDate": "2026-12-06",
          "progressionSnapshotAt": "2026-12-06T12:00:00.000Z",
          "conditionRefreshAt": "2026-12-06T20:00:00.000Z",
          "progressionExpiresAt": "2026-12-06T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-12-06T22:00:00.000Z",
          "runStage": {
            "stage": "tapering",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-12-06",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Late fall",
            "headline": "PM Steelhead fall entry remains strong but is slowing.",
            "detail": "Seasonal timing still supports broad fall presence. Colder water makes established holding water more important than new entry.",
            "tip": "Start in the Middle river. Shift lower only when Push supports fresh movement.",
            "whereToStart": "Start in the Middle river (Scottville–Maple Leaf). Add the Lower river (Pere Marquette Lake–Scottville) only when Push supports fresh movement.",
            "reasonCodes": [
              "stage_tapering"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Timing complete",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "peak_complete",
            "checkpointDate": "2026-11-21",
            "cutoffDate": "2026-11-20",
            "observationStartDate": "2026-09-01",
            "completedCheckpointCount": 5,
            "currentIndex": 92.5,
            "currentPercentile": 100,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 87.5,
            "usableDays": 81,
            "expectedDays": 81,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14",
              "2026-11-15",
              "2026-11-16",
              "2026-11-17",
              "2026-11-18",
              "2026-11-19",
              "2026-11-20"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00",
              "2026-11-15": "16:00",
              "2026-11-16": "16:00",
              "2026-11-17": "16:00",
              "2026-11-18": "16:00",
              "2026-11-19": "16:00",
              "2026-11-20": "16:00"
            },
            "headline": "The early-season timing read is complete.",
            "detail": "Earlier in the season, the migration was moving earlier than usual. Now that the migration is well underway, current movement and river conditions matter more than an early-or-late call.",
            "tip": "Stop shifting river sections based on early-or-late timing. Begin in established holding water for the current Migration Stage; if Push is Possible or stronger, make lower travel lanes the next stop.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_complete",
              "conditions_ahead",
              "conditions_timing_complete"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "peak_start",
            "previousCheckpointDate": "2026-11-15",
            "previousTimingLabel": "Ahead"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-12-06",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-12-05",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-04",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-03",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-12-03T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-02",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-01",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-12-01T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-11-30",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-11-30T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-29",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": 59,
            "displayScore": 60,
            "scoreIsApproximate": true,
            "stage": "tapering",
            "maximum": 100,
            "riverCeiling": 60,
            "historicalRunStrength": "moderate",
            "curveFraction": 0.9835714285714285,
            "curveDirection": "falling",
            "winterHoldingContext": false,
            "label": "Peak presence",
            "headline": "Steelhead seasonal presence is peak presence and declining.",
            "detail": "Calendar timing places Steelhead presence at its expected seasonal peak, declining as the fall migration advances. For the PM, expected run strength is Moderate, and the opportunity centers on dependable river sections. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_tapering",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-12-06T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-12-06T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-12-06T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          }
        }
      },
      {
        "id": "opportunity_limited_stage_building",
        "label": "Limited / concentrated · Stage · Building",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "runId": "qa_limited_opportunity",
          "fishInRiver": {
            "score": 14,
            "displayScore": 15,
            "scoreIsApproximate": true,
            "stage": "building",
            "maximum": 100,
            "riverCeiling": 30,
            "historicalRunStrength": "limited",
            "curveFraction": 0.45,
            "curveDirection": "rising",
            "winterHoldingContext": false,
            "label": "Moderate presence",
            "headline": "Steelhead seasonal presence is moderate presence and building.",
            "detail": "Calendar timing places Steelhead presence at moderate presence, increasing as the fall migration builds. For the PM, expected run strength is Limited, and the opportunity concentrates in dependable holding water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_building",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          }
        }
      },
      {
        "id": "opportunity_limited_stage_peak",
        "label": "Limited / concentrated · Stage · Peak",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "runId": "qa_limited_opportunity",
          "localDate": "2026-11-15",
          "progressionSnapshotAt": "2026-11-15T12:00:00.000Z",
          "conditionRefreshAt": "2026-11-15T20:00:00.000Z",
          "progressionExpiresAt": "2026-11-15T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-11-15T22:00:00.000Z",
          "runStage": {
            "stage": "peak",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-11-15",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Peak",
            "headline": "This is typically the PM’s strongest Steelhead window, within a Limited river-specific run.",
            "detail": "Seasonal timing favors the most dependable Steelhead holding water. Expected distribution is concentrated and uneven.",
            "tip": "Start in the Middle river. Compare the Upper river only after a complete first pass.",
            "whereToStart": "Start in the Middle river (Scottville–Maple Leaf). Compare the Upper river (Maple Leaf–M-37) when direct fish activity favors it.",
            "reasonCodes": [
              "stage_peak"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Ahead",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "peak_start",
            "checkpointDate": "2026-11-15",
            "cutoffDate": "2026-11-14",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-11-21",
            "completedCheckpointCount": 4,
            "currentIndex": 92.5,
            "currentPercentile": 100,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 87.5,
            "usableDays": 75,
            "expectedDays": 75,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00"
            },
            "headline": "The PM migration is developing earlier than usual.",
            "detail": "Season-to-date Scottville river-rise activity and M-37 cooling are stronger than the usual PM pattern.",
            "tip": "Start one PM section upstream from Migration Stage, capped at the Upper river (Maple Leaf–M-37).",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_start",
              "conditions_ahead"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "building_established",
            "previousCheckpointDate": "2026-10-15",
            "previousTimingLabel": "Ahead"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-11-15",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-11-14",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-13",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-12",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-11-12T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-11",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-10",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-11-10T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-11-09",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-11-09T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-08",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": 30,
            "displayScore": 30,
            "scoreIsApproximate": false,
            "stage": "peak",
            "maximum": 100,
            "riverCeiling": 30,
            "historicalRunStrength": "limited",
            "curveFraction": 1,
            "curveDirection": "near_peak",
            "winterHoldingContext": false,
            "label": "Peak presence",
            "headline": "Steelhead seasonal presence is peak presence and near its seasonal high.",
            "detail": "Calendar timing places Steelhead presence at its expected seasonal peak, near its expected seasonal high. For the PM, expected run strength is Limited, and the opportunity concentrates in dependable holding water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_peak",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-11-15T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-11-15T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-11-15T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "Seasonal presence is near peak, but today's water does not point to a fresh wave.",
            "detail": "Some fish may already be established in the river's most dependable holding water even when rain, river level, and temperature show little support for new movement. Start in established deep holes, fish each one from head to tail, and leave lower entry lanes secondary.",
            "reasonCodes": [
              "peak_presence_weak_push"
            ]
          }
        }
      },
      {
        "id": "opportunity_limited_stage_tapering",
        "label": "Limited / concentrated · Stage · Tapering",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "runId": "qa_limited_opportunity",
          "localDate": "2026-12-09",
          "progressionSnapshotAt": "2026-12-09T12:00:00.000Z",
          "conditionRefreshAt": "2026-12-09T20:00:00.000Z",
          "progressionExpiresAt": "2026-12-09T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-12-09T22:00:00.000Z",
          "runStage": {
            "stage": "tapering",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-12-09",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Late fall",
            "headline": "PM Steelhead fall entry remains strong but is slowing.",
            "detail": "Seasonal timing still supports broad fall presence. Colder water makes established holding water more important than new entry.",
            "tip": "Start in the Middle river. Shift lower only when Push supports fresh movement.",
            "whereToStart": "Start in the Middle river (Scottville–Maple Leaf). Add the Lower river (Pere Marquette Lake–Scottville) only when Push supports fresh movement.",
            "reasonCodes": [
              "stage_tapering"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Timing complete",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "peak_complete",
            "checkpointDate": "2026-11-21",
            "cutoffDate": "2026-11-20",
            "observationStartDate": "2026-09-01",
            "completedCheckpointCount": 5,
            "currentIndex": 92.5,
            "currentPercentile": 100,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 87.5,
            "usableDays": 81,
            "expectedDays": 81,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14",
              "2026-11-15",
              "2026-11-16",
              "2026-11-17",
              "2026-11-18",
              "2026-11-19",
              "2026-11-20"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00",
              "2026-11-15": "16:00",
              "2026-11-16": "16:00",
              "2026-11-17": "16:00",
              "2026-11-18": "16:00",
              "2026-11-19": "16:00",
              "2026-11-20": "16:00"
            },
            "headline": "The early-season timing read is complete.",
            "detail": "Earlier in the season, the migration was moving earlier than usual. Now that the migration is well underway, current movement and river conditions matter more than an early-or-late call.",
            "tip": "Stop shifting river sections based on early-or-late timing. Begin in established holding water for the current Migration Stage; if Push is Possible or stronger, make lower travel lanes the next stop.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_complete",
              "conditions_ahead",
              "conditions_timing_complete"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "peak_start",
            "previousCheckpointDate": "2026-11-15",
            "previousTimingLabel": "Ahead"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-12-09",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-12-08",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-07",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-06",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-12-06T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-05",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-04",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-12-04T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-12-03",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-12-03T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-02",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": 29,
            "displayScore": 30,
            "scoreIsApproximate": true,
            "stage": "tapering",
            "maximum": 100,
            "riverCeiling": 30,
            "historicalRunStrength": "limited",
            "curveFraction": 0.9642857142857143,
            "curveDirection": "falling",
            "winterHoldingContext": false,
            "label": "Peak presence",
            "headline": "Steelhead seasonal presence is peak presence and declining.",
            "detail": "Calendar timing places Steelhead presence at its expected seasonal peak, declining as the fall migration advances. For the PM, expected run strength is Limited, and the opportunity concentrates in dependable holding water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_tapering",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-12-09T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-12-09T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-12-09T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          }
        }
      },
      {
        "id": "opportunity_limited_stage_ending",
        "label": "Limited / concentrated · Stage · Ending",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "runId": "qa_limited_opportunity",
          "localDate": "2026-12-23",
          "progressionSnapshotAt": "2026-12-23T12:00:00.000Z",
          "conditionRefreshAt": "2026-12-23T20:00:00.000Z",
          "progressionExpiresAt": "2026-12-23T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-12-23T22:00:00.000Z",
          "runStage": {
            "stage": "post_run",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-12-23",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Fall entry complete",
            "headline": "PM Steelhead fall entry is complete.",
            "detail": "Steelhead may remain in the river. This fall-entry model no longer scores their current presence or activity.",
            "tip": "Check back in early September when PM fall movement tracking resumes.",
            "whereToStart": "There is no active PM starting section in this fall-entry model.",
            "reasonCodes": [
              "stage_post_run",
              "stage_offseason"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Timing complete",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "peak_complete",
            "checkpointDate": "2026-11-21",
            "cutoffDate": "2026-11-20",
            "observationStartDate": "2026-09-01",
            "completedCheckpointCount": 5,
            "currentIndex": 92.5,
            "currentPercentile": 100,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 87.5,
            "usableDays": 81,
            "expectedDays": 81,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14",
              "2026-11-15",
              "2026-11-16",
              "2026-11-17",
              "2026-11-18",
              "2026-11-19",
              "2026-11-20"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00",
              "2026-11-15": "16:00",
              "2026-11-16": "16:00",
              "2026-11-17": "16:00",
              "2026-11-18": "16:00",
              "2026-11-19": "16:00",
              "2026-11-20": "16:00"
            },
            "headline": "PM fall-entry Migration Timing is complete.",
            "detail": "Earlier in fall, PM migration timing was earlier than usual. The fall-entry timing window is now complete.",
            "tip": "Do not use completed fall timing to infer current presence or activity. PM fall monitoring resumes in early September.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_complete",
              "conditions_ahead",
              "conditions_timing_complete"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "peak_start",
            "previousCheckpointDate": "2026-11-15",
            "previousTimingLabel": "Ahead"
          },
          "push": {
            "score": null,
            "label": "Fall entry complete",
            "headline": "PM Steelhead fall-entry Push is complete.",
            "detail": "Current water may affect Steelhead still in the river. This fall model no longer scores fresh-entry support.",
            "tip": "Do not use a completed fall Push to infer current presence. Check back in early September.",
            "reasonCodes": [
              "push_tracking_complete"
            ],
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "complete",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-12-22",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-12-22",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-21",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-20",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-12-20T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-19",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-18",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-12-18T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-12-17",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-12-17T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-16",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": null,
            "scoreIsApproximate": false,
            "stage": "post_run",
            "maximum": 100,
            "riverCeiling": 30,
            "historicalRunStrength": "limited",
            "curveFraction": 0,
            "curveDirection": "outside",
            "winterHoldingContext": false,
            "label": "Fall entry complete",
            "headline": "PM Steelhead fall entry is complete.",
            "detail": "Steelhead may remain in the river. This fall-entry model no longer estimates their current seasonal presence.",
            "tip": "Check back in early September when PM fall movement tracking resumes.",
            "reasonCodes": [
              "stage_post_run",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-12-23T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-12-23T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-12-23T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          }
        }
      },
      {
        "id": "opportunity_limited_presence_rising",
        "label": "Limited / concentrated · Presence · rising",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "runId": "qa_limited_opportunity",
          "localDate": "2026-11-14",
          "progressionSnapshotAt": "2026-11-14T12:00:00.000Z",
          "conditionRefreshAt": "2026-11-14T20:00:00.000Z",
          "progressionExpiresAt": "2026-11-14T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-11-14T22:00:00.000Z",
          "runStage": {
            "stage": "building",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": true,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-11-14",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Building",
            "headline": "Seasonal timing supports a broader PM Steelhead distribution.",
            "detail": "The Middle river is the dependable calendar choice. Earlier arrivals may also occupy the Upper river.",
            "tip": "Cover one Middle river section completely before comparing the Upper river.",
            "whereToStart": "Start in the Middle river (Scottville–Maple Leaf). Add the Upper river (Maple Leaf–M-37) when direct fish activity supports it.",
            "reasonCodes": [
              "stage_building"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-11-14",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-11-13",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-12",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-11",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-11-11T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-10",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-09",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-11-09T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-11-08",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-11-08T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-07",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": 29,
            "displayScore": 30,
            "scoreIsApproximate": true,
            "stage": "building",
            "maximum": 100,
            "riverCeiling": 30,
            "historicalRunStrength": "limited",
            "curveFraction": 0.9821428571428572,
            "curveDirection": "rising",
            "winterHoldingContext": false,
            "label": "Peak presence",
            "headline": "Steelhead seasonal presence is peak presence and building.",
            "detail": "Calendar timing places Steelhead presence at its expected seasonal peak, increasing as the fall migration builds. For the PM, expected run strength is Limited, and the opportunity concentrates in dependable holding water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_building",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-11-14T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-11-14T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-11-14T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          }
        }
      },
      {
        "id": "opportunity_limited_presence_peak",
        "label": "Limited / concentrated · Presence · peak",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "runId": "qa_limited_opportunity",
          "localDate": "2026-11-15",
          "progressionSnapshotAt": "2026-11-15T12:00:00.000Z",
          "conditionRefreshAt": "2026-11-15T20:00:00.000Z",
          "progressionExpiresAt": "2026-11-15T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-11-15T22:00:00.000Z",
          "runStage": {
            "stage": "peak",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-11-15",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Peak",
            "headline": "This is typically the PM’s strongest Steelhead window, within a Limited river-specific run.",
            "detail": "Seasonal timing favors the most dependable Steelhead holding water. Expected distribution is concentrated and uneven.",
            "tip": "Start in the Middle river. Compare the Upper river only after a complete first pass.",
            "whereToStart": "Start in the Middle river (Scottville–Maple Leaf). Compare the Upper river (Maple Leaf–M-37) when direct fish activity favors it.",
            "reasonCodes": [
              "stage_peak"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Ahead",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "peak_start",
            "checkpointDate": "2026-11-15",
            "cutoffDate": "2026-11-14",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-11-21",
            "completedCheckpointCount": 4,
            "currentIndex": 92.5,
            "currentPercentile": 100,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 87.5,
            "usableDays": 75,
            "expectedDays": 75,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00"
            },
            "headline": "The PM migration is developing earlier than usual.",
            "detail": "Season-to-date Scottville river-rise activity and M-37 cooling are stronger than the usual PM pattern.",
            "tip": "Start one PM section upstream from Migration Stage, capped at the Upper river (Maple Leaf–M-37).",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_start",
              "conditions_ahead"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "building_established",
            "previousCheckpointDate": "2026-10-15",
            "previousTimingLabel": "Ahead"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-11-15",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-11-14",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-13",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-12",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-11-12T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-11",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-10",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-11-10T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-11-09",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-11-09T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-08",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": 30,
            "displayScore": 30,
            "scoreIsApproximate": false,
            "stage": "peak",
            "maximum": 100,
            "riverCeiling": 30,
            "historicalRunStrength": "limited",
            "curveFraction": 1,
            "curveDirection": "near_peak",
            "winterHoldingContext": false,
            "label": "Peak presence",
            "headline": "Steelhead seasonal presence is peak presence and near its seasonal high.",
            "detail": "Calendar timing places Steelhead presence at its expected seasonal peak, near its expected seasonal high. For the PM, expected run strength is Limited, and the opportunity concentrates in dependable holding water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_peak",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-11-15T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-11-15T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-11-15T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "Seasonal presence is near peak, but today's water does not point to a fresh wave.",
            "detail": "Some fish may already be established in the river's most dependable holding water even when rain, river level, and temperature show little support for new movement. Start in established deep holes, fish each one from head to tail, and leave lower entry lanes secondary.",
            "reasonCodes": [
              "peak_presence_weak_push"
            ]
          }
        }
      },
      {
        "id": "opportunity_limited_presence_falling",
        "label": "Limited / concentrated · Presence · falling",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "runId": "qa_limited_opportunity",
          "localDate": "2026-12-06",
          "progressionSnapshotAt": "2026-12-06T12:00:00.000Z",
          "conditionRefreshAt": "2026-12-06T20:00:00.000Z",
          "progressionExpiresAt": "2026-12-06T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-12-06T22:00:00.000Z",
          "runStage": {
            "stage": "tapering",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-12-06",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Late fall",
            "headline": "PM Steelhead fall entry remains strong but is slowing.",
            "detail": "Seasonal timing still supports broad fall presence. Colder water makes established holding water more important than new entry.",
            "tip": "Start in the Middle river. Shift lower only when Push supports fresh movement.",
            "whereToStart": "Start in the Middle river (Scottville–Maple Leaf). Add the Lower river (Pere Marquette Lake–Scottville) only when Push supports fresh movement.",
            "reasonCodes": [
              "stage_tapering"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Timing complete",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "peak_complete",
            "checkpointDate": "2026-11-21",
            "cutoffDate": "2026-11-20",
            "observationStartDate": "2026-09-01",
            "completedCheckpointCount": 5,
            "currentIndex": 92.5,
            "currentPercentile": 100,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 87.5,
            "usableDays": 81,
            "expectedDays": 81,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14",
              "2026-11-15",
              "2026-11-16",
              "2026-11-17",
              "2026-11-18",
              "2026-11-19",
              "2026-11-20"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00",
              "2026-11-15": "16:00",
              "2026-11-16": "16:00",
              "2026-11-17": "16:00",
              "2026-11-18": "16:00",
              "2026-11-19": "16:00",
              "2026-11-20": "16:00"
            },
            "headline": "The early-season timing read is complete.",
            "detail": "Earlier in the season, the migration was moving earlier than usual. Now that the migration is well underway, current movement and river conditions matter more than an early-or-late call.",
            "tip": "Stop shifting river sections based on early-or-late timing. Begin in established holding water for the current Migration Stage; if Push is Possible or stronger, make lower travel lanes the next stop.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_complete",
              "conditions_ahead",
              "conditions_timing_complete"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "peak_start",
            "previousCheckpointDate": "2026-11-15",
            "previousTimingLabel": "Ahead"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-12-06",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-12-05",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-04",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-03",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-12-03T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-12-02",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-12-01",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-12-01T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-11-30",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-11-30T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-29",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": 30,
            "displayScore": 30,
            "scoreIsApproximate": false,
            "stage": "tapering",
            "maximum": 100,
            "riverCeiling": 30,
            "historicalRunStrength": "limited",
            "curveFraction": 0.9835714285714285,
            "curveDirection": "falling",
            "winterHoldingContext": false,
            "label": "Peak presence",
            "headline": "Steelhead seasonal presence is peak presence and declining.",
            "detail": "Calendar timing places Steelhead presence at its expected seasonal peak, declining as the fall migration advances. For the PM, expected run strength is Limited, and the opportunity concentrates in dependable holding water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_tapering",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-12-06T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-12-06T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-12-06T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          }
        }
      }
    ]
  },
  {
    "id": "combined",
    "label": "Combined reads",
    "scenarios": [
      {
        "id": "combined_strong_tough",
        "label": "Strong Push + Tough fishing",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "push": {
            "score": 75,
            "label": "Strong",
            "headline": "PM water strongly supports possible fresh movement.",
            "detail": "Scottville flow has made a clear rise. Measured water temperature is warmer than preferred and cooling. Scottville already reflects the rain response, so rain adds no extra credit.",
            "tip": "Use Lower river travel water as the fresh-movement check, then return to the section named by Migration Stage.",
            "reasonCodes": [
              "gauge_fresh",
              "push_normal_flow_context",
              "push_temperature_transitional",
              "push_rain_absorbed_by_gauge"
            ],
            "components": {
              "hydraulicBase": 70,
              "hydraulicAdjustment": 0,
              "temperatureModifier": 5,
              "rainModifier": 0,
              "hydraulicState": "normal",
              "temperatureState": "transitional_warm",
              "rainRole": "absorbed_by_gauge",
              "appliedCaps": []
            },
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "active_now",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-10-15",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-10-14",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-13",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-12",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-10-12T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-11",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-10-10",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-10-10T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-10-09",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-10-09T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-10-08",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ],
            "lastSupportiveConditions": {
              "localDate": "2026-10-15",
              "refreshSlot": "16:00",
              "conditionRefreshAt": "2026-10-15T20:00:00.000Z",
              "score": 75,
              "label": "Strong"
            }
          },
          "fishability": {
            "score": 45,
            "label": "Tough",
            "headline": "Very high Scottville flow leaves little controllable presentation water.",
            "detail": "Very high flow compresses controllable water into the slowest protected edges. Stable flow should keep presentation lanes consistent. This read applies to the Lower river (Pere Marquette Lake–Scottville), not the full PM.",
            "tip": "Favor protected margins and short controlled presentations. Choose another day if control is not dependable.",
            "reasonCodes": [
              "gauge_fresh",
              "very_high_flow_band"
            ],
            "components": {
              "bandBase": 40,
              "trendModifier": 5,
              "appliedCaps": []
            },
            "rulesVersion": "pm-scottville-fishability-v2",
            "copyVersion": "river-run-copy-v35"
          },
          "interpretationNote": {
            "headline": "Fresh movement looks possible, but the river may be difficult to fish.",
            "detail": "A strong weather-and-water event can encourage movement while also making the river high, fast, or unsettled. Leave the main channel alone; start at bank-side inside seams and protected current, and wait for the river to settle if the presentation will not stay controlled.",
            "reasonCodes": [
              "strong_push_low_fishability"
            ]
          }
        }
      },
      {
        "id": "combined_peak_weak",
        "label": "Peak + weak Push",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-11-15",
          "progressionSnapshotAt": "2026-11-15T12:00:00.000Z",
          "conditionRefreshAt": "2026-11-15T20:00:00.000Z",
          "progressionExpiresAt": "2026-11-15T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-11-15T22:00:00.000Z",
          "runStage": {
            "stage": "peak",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-11-15",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Peak",
            "headline": "This is typically the PM’s strongest Steelhead migration window.",
            "detail": "Seasonal timing supports the widest dependable Steelhead distribution. It does not confirm fish in every section.",
            "tip": "Start in the Middle river. Compare the Upper river only after a complete first pass.",
            "whereToStart": "Start in the Middle river (Scottville–Maple Leaf). Compare the Upper river (Maple Leaf–M-37) when direct fish activity favors it.",
            "reasonCodes": [
              "stage_peak"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Ahead",
            "timingLabel": "Ahead",
            "candidateLabel": "Ahead",
            "checkpointId": "peak_start",
            "checkpointDate": "2026-11-15",
            "cutoffDate": "2026-11-14",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-11-21",
            "completedCheckpointCount": 4,
            "currentIndex": 92.5,
            "currentPercentile": 100,
            "gaugeResponsePercentile": 100,
            "waterTemperaturePercentile": 87.5,
            "usableDays": 75,
            "expectedDays": 75,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00"
            },
            "headline": "The PM migration is developing earlier than usual.",
            "detail": "Season-to-date Scottville river-rise activity and M-37 cooling are stronger than the usual PM pattern.",
            "tip": "Start one PM section upstream from Migration Stage, capped at the Upper river (Maple Leaf–M-37).",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_start",
              "conditions_ahead"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "building_established",
            "previousCheckpointDate": "2026-10-15",
            "previousTimingLabel": "Ahead"
          },
          "push": {
            "score": 0,
            "label": "Weak",
            "headline": "PM water shows little support for fresh movement.",
            "detail": "Scottville flow is falling. Measured water temperature is too warm for strong movement support but warming quickly. Recent watershed weather shows little rain.",
            "tip": "Keep the section named by Migration Stage. Leave fresh-entry travel water secondary until measured temperature improves.",
            "reasonCodes": [
              "gauge_fresh",
              "push_normal_flow_context",
              "push_temperature_too_warm",
              "dry_72h",
              "push_no_gauge_response_cap",
              "temperature_too_warm_cap"
            ],
            "components": {
              "hydraulicBase": 20,
              "hydraulicAdjustment": 0,
              "temperatureModifier": -18,
              "rainModifier": -5,
              "hydraulicState": "normal",
              "temperatureState": "too_warm",
              "rainRole": "dry",
              "appliedCaps": []
            },
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-11-15",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-11-14",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-13",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-12",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-11-12T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-11",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-10",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-11-10T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-11-09",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-11-09T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-08",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ]
          },
          "fishInRiver": {
            "score": 80,
            "displayScore": 80,
            "scoreIsApproximate": false,
            "stage": "peak",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 1,
            "curveDirection": "near_peak",
            "winterHoldingContext": false,
            "label": "Peak presence",
            "headline": "Steelhead seasonal presence is peak presence and near its seasonal high.",
            "detail": "Calendar timing places Steelhead presence at its expected seasonal peak, near its expected seasonal high. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_peak",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-11-15T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-11-15T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-11-15T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "Seasonal presence is near peak, but today's water does not point to a fresh wave.",
            "detail": "Many fish may already be in the river even when rain, river level, and temperature show little support for new movement. Start in established deep holes, fish each one from head to tail, and leave lower entry lanes secondary.",
            "reasonCodes": [
              "peak_presence_weak_push"
            ]
          }
        }
      },
      {
        "id": "combined_good_low",
        "label": "Good shape + low presence",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-09-20",
          "progressionSnapshotAt": "2026-09-20T12:00:00.000Z",
          "conditionRefreshAt": "2026-09-20T20:00:00.000Z",
          "progressionExpiresAt": "2026-09-20T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-09-20T22:00:00.000Z",
          "runStage": {
            "stage": "beginning",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-09-20",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Beginning",
            "headline": "Seasonal timing supports the first Steelhead entering the Lower river.",
            "detail": "This is the opening river phase. Presence is expected to be scattered and uneven.",
            "tip": "Cover the Lower river first. Move upstream only after direct fish activity supports it.",
            "whereToStart": "Start in the Lower river (Pere Marquette Lake–Scottville).",
            "reasonCodes": [
              "stage_beginning"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Typical",
            "timingLabel": "Typical",
            "candidateLabel": "Typical",
            "checkpointId": "river_start",
            "checkpointDate": "2026-09-20",
            "cutoffDate": "2026-09-19",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-10-11",
            "completedCheckpointCount": 1,
            "currentIndex": 45.4,
            "currentPercentile": 40,
            "gaugeResponsePercentile": 40,
            "waterTemperaturePercentile": 49,
            "usableDays": 19,
            "expectedDays": 19,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00"
            },
            "headline": "The PM migration is progressing at its usual seasonal pace.",
            "detail": "Season-to-date Scottville river-rise activity and M-37 cooling are close to the usual PM pattern.",
            "tip": "Keep the PM section named by Migration Stage.",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_river_start",
              "conditions_typical"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "none_recorded",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-09-20",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": []
          },
          "fishability": {
            "score": 75,
            "label": "Good",
            "headline": "Scottville flow is in a comfortable presentation range.",
            "detail": "The flow band supports readable lanes, seams, and holding water. Stable flow should keep presentation lanes consistent. This read applies to the Lower river (Pere Marquette Lake–Scottville), not the full PM.",
            "tip": "Cover readable seams and holding water with a controlled presentation.",
            "reasonCodes": [
              "gauge_fresh",
              "normal_flow_band"
            ],
            "components": {
              "bandBase": 70,
              "trendModifier": 5,
              "appliedCaps": []
            },
            "rulesVersion": "pm-scottville-fishability-v2",
            "copyVersion": "river-run-copy-v35"
          },
          "fishInRiver": {
            "score": 8,
            "displayScore": 10,
            "scoreIsApproximate": true,
            "stage": "beginning",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 0.1,
            "curveDirection": "rising",
            "winterHoldingContext": false,
            "label": "Low presence",
            "headline": "Steelhead seasonal presence is low presence and building.",
            "detail": "Calendar timing places Steelhead presence at low presence, increasing as the fall migration builds. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_beginning",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-09-20T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-09-20T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "The river should fish well, but seasonal fish presence is still low.",
            "detail": "Good flow makes presentations easier, but it does not put fish in the river. Stay low, fish the first travel lane entering deep resting water, make one complete pass, and move instead of waiting for numbers that have not developed.",
            "reasonCodes": [
              "good_fishability_low_presence"
            ]
          }
        }
      },
      {
        "id": "combined_delayed_strong",
        "label": "Delayed + strong Push",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "localDate": "2026-11-15",
          "progressionSnapshotAt": "2026-11-15T12:00:00.000Z",
          "conditionRefreshAt": "2026-11-15T20:00:00.000Z",
          "progressionExpiresAt": "2026-11-15T23:59:59.000Z",
          "nextConditionRefreshAt": "2026-11-15T22:00:00.000Z",
          "runStage": {
            "stage": "peak",
            "copyStrategy": "pere_marquette",
            "stagingContext": false,
            "broadBuildingContext": false,
            "winterHoldingContext": false,
            "window": {
              "snapshotDate": "2026-11-15",
              "preRunStartDate": "2026-08-15",
              "stagingStartDate": "2026-09-01",
              "startDate": "2026-09-20",
              "beginningEndDate": "2026-10-10",
              "buildingEstablishedStartDate": "2026-10-15",
              "buildingBroadStartDate": "2026-11-01",
              "peakStartDate": "2026-11-15",
              "peakDate": "2026-11-15",
              "peakEndDate": "2026-12-04",
              "taperingEndDate": "2026-12-19",
              "endDate": "2026-12-22",
              "lateEndDate": "2026-12-23",
              "postRunLateCopyEndDate": "2026-12-24",
              "startToPeakDays": 56,
              "peakToEndDays": 37
            },
            "label": "Peak",
            "headline": "This is typically the PM’s strongest Steelhead migration window.",
            "detail": "Seasonal timing supports the widest dependable Steelhead distribution. It does not confirm fish in every section.",
            "tip": "Start in the Middle river. Compare the Upper river only after a complete first pass.",
            "whereToStart": "Start in the Middle river (Scottville–Maple Leaf). Compare the Upper river (Maple Leaf–M-37) when direct fish activity favors it.",
            "reasonCodes": [
              "stage_peak"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "conditionsSuggest": {
            "label": "Delayed",
            "timingLabel": "Delayed",
            "candidateLabel": "Delayed",
            "checkpointId": "peak_start",
            "checkpointDate": "2026-11-15",
            "cutoffDate": "2026-11-14",
            "observationStartDate": "2026-09-01",
            "nextCheckpointDate": "2026-11-21",
            "completedCheckpointCount": 4,
            "currentIndex": 16,
            "currentPercentile": 20,
            "gaugeResponsePercentile": 10,
            "waterTemperaturePercentile": 20,
            "usableDays": 75,
            "expectedDays": 75,
            "coveragePercent": 1,
            "historicalYears": 5,
            "baselineVersion": "pm-fall-steelhead-conditions-v2",
            "gaugeSiteId": "04122500",
            "temperatureSourceId": "pm_m37_temperature",
            "sourceDates": [
              "2026-09-01",
              "2026-09-02",
              "2026-09-03",
              "2026-09-04",
              "2026-09-05",
              "2026-09-06",
              "2026-09-07",
              "2026-09-08",
              "2026-09-09",
              "2026-09-10",
              "2026-09-11",
              "2026-09-12",
              "2026-09-13",
              "2026-09-14",
              "2026-09-15",
              "2026-09-16",
              "2026-09-17",
              "2026-09-18",
              "2026-09-19",
              "2026-09-20",
              "2026-09-21",
              "2026-09-22",
              "2026-09-23",
              "2026-09-24",
              "2026-09-25",
              "2026-09-26",
              "2026-09-27",
              "2026-09-28",
              "2026-09-29",
              "2026-09-30",
              "2026-10-01",
              "2026-10-02",
              "2026-10-03",
              "2026-10-04",
              "2026-10-05",
              "2026-10-06",
              "2026-10-07",
              "2026-10-08",
              "2026-10-09",
              "2026-10-10",
              "2026-10-11",
              "2026-10-12",
              "2026-10-13",
              "2026-10-14",
              "2026-10-15",
              "2026-10-16",
              "2026-10-17",
              "2026-10-18",
              "2026-10-19",
              "2026-10-20",
              "2026-10-21",
              "2026-10-22",
              "2026-10-23",
              "2026-10-24",
              "2026-10-25",
              "2026-10-26",
              "2026-10-27",
              "2026-10-28",
              "2026-10-29",
              "2026-10-30",
              "2026-10-31",
              "2026-11-01",
              "2026-11-02",
              "2026-11-03",
              "2026-11-04",
              "2026-11-05",
              "2026-11-06",
              "2026-11-07",
              "2026-11-08",
              "2026-11-09",
              "2026-11-10",
              "2026-11-11",
              "2026-11-12",
              "2026-11-13",
              "2026-11-14"
            ],
            "sourceRefreshSlots": {
              "2026-09-01": "16:00",
              "2026-09-02": "16:00",
              "2026-09-03": "16:00",
              "2026-09-04": "16:00",
              "2026-09-05": "16:00",
              "2026-09-06": "16:00",
              "2026-09-07": "16:00",
              "2026-09-08": "16:00",
              "2026-09-09": "16:00",
              "2026-09-10": "16:00",
              "2026-09-11": "16:00",
              "2026-09-12": "16:00",
              "2026-09-13": "16:00",
              "2026-09-14": "16:00",
              "2026-09-15": "16:00",
              "2026-09-16": "16:00",
              "2026-09-17": "16:00",
              "2026-09-18": "16:00",
              "2026-09-19": "16:00",
              "2026-09-20": "16:00",
              "2026-09-21": "16:00",
              "2026-09-22": "16:00",
              "2026-09-23": "16:00",
              "2026-09-24": "16:00",
              "2026-09-25": "16:00",
              "2026-09-26": "16:00",
              "2026-09-27": "16:00",
              "2026-09-28": "16:00",
              "2026-09-29": "16:00",
              "2026-09-30": "16:00",
              "2026-10-01": "16:00",
              "2026-10-02": "16:00",
              "2026-10-03": "16:00",
              "2026-10-04": "16:00",
              "2026-10-05": "16:00",
              "2026-10-06": "16:00",
              "2026-10-07": "16:00",
              "2026-10-08": "16:00",
              "2026-10-09": "16:00",
              "2026-10-10": "16:00",
              "2026-10-11": "16:00",
              "2026-10-12": "16:00",
              "2026-10-13": "16:00",
              "2026-10-14": "16:00",
              "2026-10-15": "16:00",
              "2026-10-16": "16:00",
              "2026-10-17": "16:00",
              "2026-10-18": "16:00",
              "2026-10-19": "16:00",
              "2026-10-20": "16:00",
              "2026-10-21": "16:00",
              "2026-10-22": "16:00",
              "2026-10-23": "16:00",
              "2026-10-24": "16:00",
              "2026-10-25": "16:00",
              "2026-10-26": "16:00",
              "2026-10-27": "16:00",
              "2026-10-28": "16:00",
              "2026-10-29": "16:00",
              "2026-10-30": "16:00",
              "2026-10-31": "16:00",
              "2026-11-01": "16:00",
              "2026-11-02": "16:00",
              "2026-11-03": "16:00",
              "2026-11-04": "16:00",
              "2026-11-05": "16:00",
              "2026-11-06": "16:00",
              "2026-11-07": "16:00",
              "2026-11-08": "16:00",
              "2026-11-09": "16:00",
              "2026-11-10": "16:00",
              "2026-11-11": "16:00",
              "2026-11-12": "16:00",
              "2026-11-13": "16:00",
              "2026-11-14": "16:00"
            },
            "headline": "The PM migration is developing later than usual.",
            "detail": "Season-to-date Scottville river-rise activity and M-37 cooling are weaker than the usual PM pattern.",
            "tip": "Start one PM section downstream from Migration Stage, capped at the Lower river (Pere Marquette Lake–Scottville).",
            "reasonCodes": [
              "gauge_fresh",
              "temperature_measured",
              "conditions_checkpoint_peak_start",
              "conditions_delayed"
            ],
            "copyVersion": "river-run-copy-v35",
            "previousCheckpointId": "building_established",
            "previousCheckpointDate": "2026-10-15",
            "previousTimingLabel": "Delayed"
          },
          "push": {
            "score": 75,
            "label": "Strong",
            "headline": "PM water strongly supports possible fresh movement.",
            "detail": "Scottville flow has made a clear rise. Measured water temperature is warmer than preferred and cooling. Scottville already reflects the rain response, so rain adds no extra credit.",
            "tip": "Use Lower river travel water as the fresh-movement check, then return to the section named by Migration Stage.",
            "reasonCodes": [
              "gauge_fresh",
              "push_normal_flow_context",
              "push_temperature_transitional",
              "push_rain_absorbed_by_gauge"
            ],
            "components": {
              "hydraulicBase": 70,
              "hydraulicAdjustment": 0,
              "temperatureModifier": 5,
              "rainModifier": 0,
              "hydraulicState": "normal",
              "temperatureState": "transitional_warm",
              "rainRole": "absorbed_by_gauge",
              "appliedCaps": []
            },
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          },
          "pushHistory": {
            "status": "active_now",
            "minimumSupportiveScore": 50,
            "trackingStartDate": "2026-09-20",
            "trackingEndDate": "2026-12-22",
            "throughDate": "2026-11-15",
            "recentDailyReadsStatus": "available",
            "recentDailyReads": [
              {
                "localDate": "2026-11-14",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-13",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-12",
                "status": "supportive_window",
                "refreshSlot": "08:00",
                "conditionRefreshAt": "2026-11-12T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-11",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              },
              {
                "localDate": "2026-11-10",
                "status": "supportive_window",
                "refreshSlot": "20:00",
                "conditionRefreshAt": "2026-11-10T23:59:00.000Z",
                "score": 81,
                "label": "Strong"
              },
              {
                "localDate": "2026-11-09",
                "status": "supportive_window",
                "refreshSlot": "16:00",
                "conditionRefreshAt": "2026-11-09T23:59:00.000Z",
                "score": 63,
                "label": "Possible"
              },
              {
                "localDate": "2026-11-08",
                "status": "no_supportive_window",
                "score": null,
                "label": "No supportive window"
              }
            ],
            "lastSupportiveConditions": {
              "localDate": "2026-11-15",
              "refreshSlot": "16:00",
              "conditionRefreshAt": "2026-11-15T20:00:00.000Z",
              "score": 75,
              "label": "Strong"
            }
          },
          "fishInRiver": {
            "score": 80,
            "displayScore": 80,
            "scoreIsApproximate": false,
            "stage": "peak",
            "maximum": 100,
            "riverCeiling": 80,
            "historicalRunStrength": "strong",
            "curveFraction": 1,
            "curveDirection": "near_peak",
            "winterHoldingContext": false,
            "label": "Peak presence",
            "headline": "Steelhead seasonal presence is peak presence and near its seasonal high.",
            "detail": "Calendar timing places Steelhead presence at its expected seasonal peak, near its expected seasonal high. For the PM, expected run strength is Strong, and the opportunity can extend across broadly accessible water. This is not a live fish count or today’s conditions.",
            "tip": "Use Migration Stage to choose a section. Use Push and Activity for current movement support and responsiveness.",
            "reasonCodes": [
              "stage_peak",
              "historical_presence_curve"
            ],
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": {
            "provider": "USGS",
            "siteId": "04122500",
            "primaryMetric": "flow_cfs",
            "observedAt": "2026-11-15T19:45:00.000Z",
            "value": 600,
            "band": "ideal",
            "trend": "stable",
            "absoluteChange24h": 0,
            "percentChange24h": 0
          },
          "waterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_maple_leaf",
            "siteId": "PMTU-MapleLeaf",
            "seriesId": "4939",
            "observedAt": "2026-11-15T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "conditionsWaterTemperature": {
            "provider": "MONITOR_MY_WATERSHED",
            "sourceId": "pmtu_m37",
            "siteId": "PMTU-M37",
            "seriesId": "3201",
            "observedAt": "2026-11-15T19:30:00.000Z",
            "waterTempF": 60,
            "trend": "steady",
            "sourceType": "same_gauge",
            "attribution": "Measured water temperature"
          },
          "interpretationNote": {
            "headline": "Today's reads are pointing to different parts of the picture.",
            "detail": "The usual calendar supports broader fish distribution, while slower river rises and cooling argue against assuming the normal middle- and upper-river water has filled in. Start one accessible section lower than the normal peak plan, work established holding water, and expand upstream only after direct fish activity supports it.\n\nA delayed migration can still receive a strong movement event. Start on lower-river travel lanes and the first deep resting holes above them; do not jump to upper water as though the whole season has caught up.",
            "reasonCodes": [
              "peak_delayed_conditions",
              "delayed_conditions_strong_push"
            ]
          }
        }
      }
    ]
  },
  {
    "id": "evidence",
    "label": "Evidence quality",
    "scenarios": [
      {
        "id": "evidence_fresh",
        "label": "Fresh",
        "note": "Canonical production copy",
        "snapshotOverride": {}
      },
      {
        "id": "evidence_limited",
        "label": "Limited · unavailable cards",
        "note": "Canonical production copy",
        "snapshotOverride": {
          "push": {
            "score": null,
            "label": "Unavailable",
            "headline": "A current Scottville flow reading is unavailable.",
            "detail": "Without Scottville flow and direction, rain cannot produce a dependable PM fresh-movement read.",
            "tip": "Keep the section named by Migration Stage. Do not treat recent rain as proof of movement.",
            "reasonCodes": [
              "gauge_missing"
            ],
            "rulesVersion": "pm-fall-steelhead-push-v1",
            "copyVersion": "river-run-copy-v35"
          },
          "fishability": {
            "score": null,
            "label": "Unavailable",
            "headline": "A current Scottville reading is unavailable.",
            "detail": "Without current Scottville flow and direction, Lower river presentation conditions cannot be determined.",
            "tip": "Do not extend an old or missing Scottville read across the PM. Use current authoritative local information.",
            "reasonCodes": [
              "gauge_missing"
            ],
            "rulesVersion": "pm-scottville-fishability-v2",
            "copyVersion": "river-run-copy-v35"
          },
          "gauge": null,
          "weather": null,
          "waterTemperature": null,
          "conditionsWaterTemperature": null,
          "freshness": {
            "gauge": "missing",
            "weather": "fresh",
            "waterTemperature": "missing",
            "conditionsWaterTemperature": "missing",
            "conditionsSuggestDaysUsable": 0
          },
          "dataQuality": {
            "label": "Limited",
            "reasonCodes": [
              "data_quality_limited"
            ]
          }
        }
      }
    ]
  }
] as const;

export const RIVER_RUN_STEELHEAD_REVIEW_GROUPS: RiverRunReviewGroup[] = GROUP_SEEDS.map(
  (group) => ({
    id: group.id,
    label: group.label,
    scenarios: group.scenarios.map((scenario) => ({
      id: scenario.id,
      label: scenario.label,
      note: scenario.note,
      snapshot: {
        ...BASE_SNAPSHOT,
        ...scenario.snapshotOverride,
      } as RiverRunSnapshotResponse,
    })),
  }),
);

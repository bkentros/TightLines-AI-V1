...
- [ ] **BLOCKER: Resolve fingerprint-based creator attribution, then update App Store Privacy.** The August 5, 2026 audit found Instally/custom network fingerprint install matching that should not ship unchanged on iOS. Follow `docs/APP_PRIVACY_UPDATE_20260805.md`, disable fingerprint matching, then publish the complete App Store privacy response before submission/release.
...
- [ ] Re-check App Store Privacy labels after PostHog and creator-code attribution. Current expected stance: product interaction/usage analytics, identifiers, purchases, and diagnostics are linked to the user; no tracking if data is not used for third-party advertising or cross-app/site tracking.

# Platte River Fall Steelhead River Run Profile

**Status:** `supported_hidden_review_correction`

## 0. Candidate capability audit

The prior decision treated a current spring egg-take and a general destination
listing as insufficient for fall timing, but it did not check the DNR technical
report archive. Technical Report 91-1 records Steelhead at the lower weir from
September 3 through November 30, calls the run strong throughout fall, and
places the peak in the week of October 17. Its 1980-90 table also demonstrates
recurring fall returns. Current DNR material separately confirms the Platte's
substantial Steelhead population.

**Capability decision:** `supported_hidden_review` **Contradiction search
completed by/date:** Codex / 2026-08-24 **Independent falsification review
by/date:** required before public enablement

**River ID:** `platte` **Species slug:** `steelhead` **Created/researched:**
2026-08-24 **Status:** `supported_hidden_review_correction`

## Truth decision

| Field                      | Decision                                                                                                                                     | Evidence IDs |
| -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | ------------ |
| Public catalog behavior    | Include in private review; do not publicly enable until corrected review gates pass                                                          | S-005        |
| Species support            | Verified: current DNR opportunity guidance lists Platte Steelhead and spring 2026 Platte weirs collected Steelhead                           | S-002, S-003 |
| Planned fall-entry support | Supported: Technical Report 91-1 directly documents recurring September–November fall returns and an Oct. 17 peak                            | S-005        |
| Lifecycle                  | Steelhead are iteroparous and were returned alive after egg take; never inherit Coho mortality/taper logic                                   | S-003        |
| Barrier response           | The emergency spring 2026 lower-weir deployment blocked/diverted Steelhead; normal fall passage and release reach were not established       | S-003, S-004 |
| Seasonal calendar          | Direct Sep. 3–Nov. 30 record, strong-throughout-fall description, and Oct. 17 peak                                                           | S-005        |
| Presence ceiling/curve     | 7/10 concentrated lower-corridor calibration; 1990 was the largest 1980–90 return and current stocking is comparable to accepted 7/10 rivers | S-002, S-005 |
| Activity                   | Hidden Limited weather-only candidate implemented and replayed; pending fixtures, owner review, and enablement                               | S-001        |
| Fishability                | Unavailable; lower corridor has no accepted local hydraulic source                                                                           | S-001        |
| Terminal/handoff           | Fall-entry model retains a holding tail and ends without saying fish died or left                                                            | S-003, S-005 |

## Corrected hidden-review truth

- Run window: monitor August 15; stage August 25; beginning September 3;
  strongest historic entry October 8 through November 2, anchored October 17;
  retained holding tail through January 15.
- Presence ceiling: `7/10`, concentrated in the lower corridor.
- Ending the fall-entry model must never say Steelhead died or left the river.
- Scope: never say Steelhead are absent or die after spawning. Hidden review may
  show Stage and Fish In River below the closure; Activity and Fishability stay
  unavailable until independently calibrated.

## Evidence ledger

| ID    | Authority/title                                                                                         | URL/path                                                                                                                                               | Published/updated               | Accessed   | Facts/limitations                                                                                                               |
| ----- | ------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------- |
| S-001 | Corrected Platte foundation                                                                             | ../river-foundation.md                                                                                                                                 | 2026-08-24                      | 2026-08-24 | Fall Steelhead supported for hidden review; reach/capability limits                                                             |
| S-002 | Michigan DNR — Better Fishing Waters                                                                    | https://www.michigan.gov/dnr/things-to-do/fishing/where/better-fishing-waters                                                                          | Current 2026 page               | 2026-08-24 | Lists Platte River for Steelhead; does not supply a fall calendar/ceiling                                                       |
| S-003 | Michigan DNR — Steelhead egg collection after lightning damage                                          | https://www.michigan.gov/dnr/about/newsroom/releases/2026/05/14/dnr-staff-successfully-complete-steelhead-egg-collection-after-lightning-strike-damage | 2026-05-14                      | 2026-08-24 | Spring Platte capture/egg take; lower weir blocked/diverted fish; adults iteroparous and returned alive; release reach unstated |
| S-004 | Michigan DNR — 2026 Michigan Fishing Regulations                                                        | https://www.michigan.gov/dnr/-/media/Project/Websites/dnr/Documents/LED/digests/2026-Michigan-Fishing-Regulations_web_accessible.pdf                   | Effective 2026-04-01–2027-03-31 | 2026-08-24 | Rainbow Trout limit and 300-ft weir closures; regulations do not prove fall timing                                              |
| S-005 | Michigan DNR — _Platte River Harvest Weir and Coho Salmon Egg-Take Report, 1990_, Technical Report 91-1 | https://www.michigandnr.com/publications/pdfs/DNRFishLibrary/TechnicalReports/TR91-1.pdf                                                               | 1991                            | 2026-08-24 | Steelhead present Sept. 3-Nov. 30; strong throughout fall; Oct. 17 peak; recurring 1980-90 returns                              |

**Run decision:** `supported_hidden_review`; the earlier unresolved/disabled
decision is withdrawn.

**Configuration version:** `platte-phase-c-draft.3` **Presence version:**
`platte-fall-steelhead-presence-v3-draft` **Activity:**
`platte-fall-steelhead-weather-activity-v1-draft`, using 0.70 effective light /
0.30 same-block precipitation and a 0.80 missing-primary-evidence scale. It has
no salmon mortality behavior. Final replay:
`docs/audits/river-run-platte-steelhead-weather-activity-replay.json`; decision
record:
`docs/audits/river-run-grand-platte-white-activity-calibration-2026-08-24.md`.
local calibration **Owner acceptance:** corrected 7/10 profile available for
renewed hidden review 2026-08-24.

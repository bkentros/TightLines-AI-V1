...
The archived `2026-09-09T18:00:00Z` cycle contains five cities, 121 forecast hours per city, and 605 total samples under engine `pier-cast-simple-model-v0.4.0`. Cron job `3` is active on `35 0,6,12,18 * * *`. Production privilege checks confirm that neither `anon` nor `authenticated` can select either archive table or execute the commit, read, or scheduler functions; `service_role` can execute the scheduler.
...
- The authenticated ingestion function is deployed to production.
- Dedicated Edge and Vault secrets are configured without reusing another module's key.
- The cron schedule is active and two authenticated production invocations passed.
- Three complete production model cycles (1,815 samples) and 13,460 unique observation records are archived; repeat ingestion is idempotent.
...
- The seasonal curves, thermal curves, and two-input formula are unchanged.

## Next work

1. Continue monitoring automatic scheduled runs, including source lag, archive freshness, fallback use, and GLOS resumption/cessation.
2. Accumulate the frozen protocol's required days, regimes, matches per lead, and operating seasons; do not promote aggregate flag `2`, `3`, or `4`.
3. Obtain qualified observation coverage for Manistee and Frankfort–Elberta and documented sensor depth/plume context for existing sources.
4. Re-run the city-specific representation audit on prospective evidence, reserving a holdout for any correction.
5. Validate rating usefulness against dated pier outcomes, then approve or reject one city/species scope at a time.

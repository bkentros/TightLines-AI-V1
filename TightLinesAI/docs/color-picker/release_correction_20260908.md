...
### Final crash-resilience gate

The client now validates generated and reopened report envelopes before they enter React rendering. Missing or duplicate light groups, anything other than two choices per light, duplicate pattern IDs, invalid swatches, incomplete copy, invalid metadata, and request-ID mismatches fail as recoverable report errors rather than reaching nested report components. Confirmed obsolete or malformed saved-report bookmarks are removed locally so the app does not repeatedly reopen unusable data. Legacy subtype IDs resolve to their current broad lure/fly artwork when possible.

The release guard exercises one valid envelope and seven malformed-envelope cases. TypeScript, all 30 engine/service/explanation tests, research QA, catalog/routing QA, 85 artwork hashes, 680 artwork-fit cases, iOS export, Android export, migration reconciliation, and the authenticated production smoke all passed after this hardening. The smoke account and its reports were deleted. No production mobile build was created.

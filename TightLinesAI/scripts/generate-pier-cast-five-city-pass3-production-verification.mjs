...
const configVersion = "piercast-v3-seventeen-city-five-city-pass3-v7";
...
const checks = {
  runForecastCount: run.forecast_count === 590,
  storedForecastCount: forecasts.length === 590,
  cityCount: cityIds.length === 17,
  pairCount: pairs.length === 118,
  ...
  onboardingPairCount: onboardingPairs.length === 24,
  onboardingForecastCount: onboarding.length === 120,
...
const artifact = {
  schemaVersion: "piercast-five-city-pass3-production-verification-v1",
  ...
  independentAcceptance: {
    ownerProductionQa:
      "PASS: 17 ranked cities from one coherent four-cohort issue.",
    publicProductionQa:
      "PASS: frozen 12-city projection and 12 complete five-day reports.",
    authenticatedNormalUserSmoke:
      "PASS: five onboarding cities absent from catalog and direct reports; owner review returns 403.",
  },
  result: "pass",
};
...
console.log(
  `PASS: production run ${run.run_id} contains 590 rows, 17 cities, 118 pairs, and all five private onboarding reports.`,
);

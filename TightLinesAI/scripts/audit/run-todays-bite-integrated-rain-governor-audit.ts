...
/**
 * Phase 9C integrated rain/wet disruption governor audit.
 *
...
const OUTPUT_JSONL =
  "scripts/audit/todays-bite-integrated-rain-governor-audit.jsonl";
const OUTPUT_MD = "scripts/audit/todays-bite-integrated-rain-governor-audit.md";
...
const CANDIDATES = [
  "production_control",
  "active_rain_score_ceiling_65",
  "active_rain_score_ceiling_62",
  "active_rain_contextual_ceiling",
  "wet_baseline_score_ceiling_70",
  "wet_baseline_score_ceiling_67",
  "wet_baseline_contextual_ceiling",
  "major_suppressor_ceiling",
  "rain_driver_damping",
  "combined_contextual_light",
  "combined_contextual_stronger",
] as const;
...
function applyCandidate(
  ...
  // Phase 9C candidates are score-level governors. Normalized labels/modes stay
...
function applyDriverDamping(scored: Scored, archetype: Archetype): Scored {
  const damp = archetype.id === "heavy_active_rain"
    ? 0.20
    : archetype.id === "wet_baseline_recent_rain"
    ? 0.10
    : 0;
...
function wetBaselineApplies(
  archetype: Archetype,
  norm: SharedNormalizedOutput,
): boolean {
  return archetype.id === "wet_baseline_recent_rain" &&
    (norm.normalized.precipitation_disruption?.score ?? 0) <= -0.45;
}
...
function applyScoreGovernor(args: {
  ...
  const activeRain = args.norm.normalized.precipitation_disruption?.label ===
    "active_disruption";
  const wet = wetBaselineApplies(args.archetype, args.norm);
  ...
  switch (args.candidate) {
    case "production_control":
      return scored;
    case "active_rain_score_ceiling_65":
      return activeRain ? capScored(scored, 65) : scored;
    case "active_rain_score_ceiling_62":
      return activeRain ? capScored(scored, 62) : scored;
    case "active_rain_contextual_ceiling":
      return activeRain
        ? capScored(scored, contextualActiveRainCap(args.context))
        : scored;
    case "wet_baseline_score_ceiling_70":
      return wet ? capScored(scored, 70) : scored;
    case "wet_baseline_score_ceiling_67":
      return wet ? capScored(scored, 67) : scored;
    case "wet_baseline_contextual_ceiling": {
      const cap = wet ? contextualWetCap(args.context, args.norm) : null;
      return cap == null ? scored : capScored(scored, cap);
    }
    case "major_suppressor_ceiling":
      return hasMajorSuppressor && scored.score > 70
        ? capScored(scored, 69)
        : scored;
    case "rain_driver_damping":
      return applyDriverDamping(scored, args.archetype);
    case "combined_contextual_light": {
      if (activeRain) {
    ...
    case "combined_contextual_stronger": {
      if (activeRain) {
        scored = capScored(scored, contextualActiveRainCap(args.context));
      }
      if (wet) scored = capScored(scored, 67);
      if (hasMajorSuppressor && scored.score > 70) {
        scored = capScored(scored, 69);
      }
      return applyDriverDamping(scored, args.archetype);
...
function safeCandidate(rows: Row[], candidate: CandidateId): boolean {
  ...
  return reductionNumber(
    ...
    reductionNumber(
        base.high_score_with_major_suppressor,
        cand.high_score_with_major_suppressor,
      ) >= 0.50 &&
    reductionNumber(
        base.report_copy_conflicts_with_score,
        cand.report_copy_conflicts_with_score,
      ) >= 0.30 &&
...
const markdown = `# Today's Bite Integrated Rain/Wet Governor Shadow Audit
...
| --- | ---: | --- | --- | --- | ---: | ---: | ---: | --- | --- |
${improvedRows(rows, "combined_contextual_stronger")}

## Top Regressions For Combined Contextual Stronger

| Region | Month | Context | Archetype | Clarity | Prod | Candidate | Delta | Prod flags | Candidate flags |
| --- | ---: | --- | --- | --- | ---: | ---: | ---: | --- | --- |
${worstRegressions(rows, "combined_contextual_stronger")}

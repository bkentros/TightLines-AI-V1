...
const run = RIVER_RUN_RUN_PROFILES.find((candidate) =>
  candidate.runId === "pere_marquette_fall_steelhead"
);
if (
  !run?.activity || !run.push || !run.waterTemperature ||
  !run.fishabilityBands
) throw new Error("PM Steelhead Activity audit rules are unavailable.");
...
const invariants = {
  ...
  warmCapBroken:
    rows.filter((row) =>
      row.waterTempF >= run.activity!.temperature.warmF + 0.05 &&
  ...
  barrierCapBroken:
    rows.filter((row) =>
      row.waterTempF >= run.activity!.temperature.barrierF &&
      row.blocks.some((block) => block.score >= 25)
    ).length,
  taperCapBroken: rows.filter((row) =>
    row.stage === "tapering" &&
    row.blocks.some((block) => block.score > run.activity!.caps.lateRun)
  ).length,
  endingCapBroken:
    rows.filter((row) =>
      ["ending", "post_run"].includes(row.stage) &&
      row.blocks.some((block) => block.score > run.activity!.caps.ending)
    ).length,
  prohibitedClaims:
    rows.filter((row) =>
...
const report = {
  ...
  lowerTail: {
    daysBelow20: rows.filter((row) => row.score < 20).length,
    blocksBelow20: allBlocks.filter(({ block }) => block.score < 20).length,
    daysBelow30: rows.filter((row) => row.score < 30).length,
    blocksBelow30: allBlocks.filter(({ block }) => block.score < 30).length,
  },
  ...
  temperatureBands: {
    ...
    from36To43F: scoreGroup(
      rows.filter((row) => row.waterTempF >= 36 && row.waterTempF < 44),
    ),
    from44To56F: scoreGroup(
      rows.filter((row) => row.waterTempF >= 44 && row.waterTempF <= 56),
    ),
    above56F: scoreGroup(rows.filter((row) => row.waterTempF > 56)),
  },
...
if (Deno.args.includes("--write")) {
  ...
  await Deno.writeTextFile(
    "docs/audits/river-run-pm-steelhead-activity-replay.json",
    `${JSON.stringify(report, null, 2)}\n`,
  );
  await Deno.writeTextFile(
    "docs/audits/river-run-pm-steelhead-activity-review-100.csv",
...
function scoreGroup(input: ReplayRow[]) {
  return {
    days: input.length,
    scores: input.length ? summary(input.map((row) => row.score)) : null,

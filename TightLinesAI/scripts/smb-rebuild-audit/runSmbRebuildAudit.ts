...
/**
 * SMB rebuild recommender audit (archived weather + structural + repetition).
...
import { SMALLMOUTH_AUDIT_ANCHORS } from "../recommender-v3-audit/smallmouthAuditMatrix.ts";

const ARCHIVE_PATH = new URL(
  "../../docs/recommender-v3-audit/generated/smallmouth-v3-matrix-archive-env.json",
  import.meta.url,
);

/** Mainstream SMB regions (matrix + CSV structural audit focus; mirrors LMB tiering). */
...
const ANCHOR_BY_KEY = new Map(
  SMALLMOUTH_AUDIT_ANCHORS.map((a) => [a.key, a]),
);

type MatrixAnchor = (typeof SMALLMOUTH_AUDIT_ANCHORS)[number];
...
function parseScenarioAnchorKey(scenarioId: string): string | null {
  const m = scenarioId.match(/^smb_matrix_(.+)_\d{2}$/);
  return m?.[1] ?? null;
}
...
function archivedWeatherAudit() {
  ...
  for (const sc of bundle.scenarios) {
    ...
    const res = runRecommenderRebuildSurface(req, { userSeed: "smb-rebuild-audit-static" });
...
function structuralSweep() {
  ...
  const rows = SMALLMOUTH_BASS_SEASONAL_ROWS_V4.filter((r) =>
...
function repetitionAudit() {
  const raw = Deno.readTextFileSync(ARCHIVE_PATH);
  const bundle = JSON.parse(raw) as ArchiveBundle;
  const june = bundle.scenarios.find((s) => s.scenario_id === "smb_matrix_great_lakes_clear_lake_06");
  if (!june?.env_data) {
    return { error: "missing smb_matrix_great_lakes_clear_lake_06 in archive" };
  }
  const anchor = ANCHOR_BY_KEY.get("great_lakes_clear_lake")!;
  ...
  return {
    scenario: "great_lakes_clear_lake_june_archived_env",
    days: 14,

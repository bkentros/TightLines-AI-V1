...
const run = RIVER_RUN_RUN_PROFILES.find((candidate) =>
  candidate.runId === "pere_marquette_fall_coho"
);
if (
  !run?.activity || !run.push || !run.waterTemperature ||
  !run.fishabilityBands
) throw new Error("PM Coho Activity audit rules are unavailable.");
...
if (Deno.args.includes("--write")) {
  ...
  await Deno.writeTextFile(
    "docs/audits/river-run-pm-coho-activity-replay.json",
    `${JSON.stringify(report, null, 2)}\n`,
  );
  await Deno.writeTextFile(
    "docs/audits/river-run-pm-coho-activity-review-100.csv",

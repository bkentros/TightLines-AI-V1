...
with tempfile.TemporaryDirectory(prefix="color-picker-pg-", dir="/tmp") as tmp:
    ...
    try:
        ...
        sql((root / "supabase/migrations/20260913160000_lifetime_color_and_pier_trials.sql").read_text())
        def trial_commit(payload, free=True):
            literal = json.dumps(payload).replace("'", "''")
            return sql(f"set role service_role; select public.commit_color_picker_report_with_trial('{user}','{literal}'::jsonb,{str(free).lower()})")
        def blocked(call, message="subscription_required"):
            try:
                call()
                raise AssertionError("Restricted operation succeeded")
            except subprocess.CalledProcessError as error:
                assert message in error.stderr, error.stderr
        # Malformed work must not burn the allowance.
        blocked(lambda: trial_commit(malformed), "invalid daily color report")
        assert sql("select count(*) from public.feature_report_trials") == "0"
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
            trials = list(executor.map(lambda n: trial_commit(envelope(n)), range(100,108)))
        assert len(set(trials)) == 1
        blocked(lambda: trial_commit(envelope(120, "second-clarity", "clear")))
        tomorrow = envelope(121)
        tomorrow["request"]["date"] = "2026-09-06"
        tomorrow["selection"]["report"]["generatedAt"] = "2026-09-06T12:00:00Z"
        blocked(lambda: trial_commit(tomorrow))
        trial_commit(tomorrow, False)  # Upgrading works; downgrading cannot reset.
        blocked(lambda: trial_commit(tomorrow))
        def pier(key, payload="{}", user_id=user):
            return sql(f"set role service_role; select public.claim_feature_report_trial('{user_id}','pier_cast','{key}','{payload}'::jsonb)")
        def attempt(key):
            try: return (key,pier(key))
            except subprocess.CalledProcessError as error:
                assert "subscription_required" in error.stderr
                return None
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
            cities = list(executor.map(attempt, ["ludington:2026-09-13","grandhaven:2026-09-13"]))
        winners = [result for result in cities if result]
        assert len(winners) == 1
        key = winners[0][0]
        assert json.loads(pier(key, '{"conditions":"updated"}'))["conditions"] == "updated"

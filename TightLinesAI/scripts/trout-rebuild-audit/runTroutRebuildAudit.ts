...
/**
 * Trout rebuild recommender audit (river-only product rule).
 * Matrix + structural use `freshwater_river` only — no lake/pond cells or requests.
 */
...
import { TROUT_AUDIT_ANCHORS } from "../recommender-v3-audit/troutAuditMatrix.ts";

const RIVER: EngineContext = "freshwater_river";

const ARCHIVE_PATH = new URL(
  "../../docs/recommender-v3-audit/generated/trout-v3-matrix-archive-env.json",
  import.meta.url,
);
...
for (const a of TROUT_AUDIT_ANCHORS) {
  if (a.context !== RIVER) {
    throw new Error(`trout audit anchor ${a.key} must use freshwater_river; got ${a.context}`);
  }
}

const ANCHOR_BY_KEY = new Map(
  TROUT_AUDIT_ANCHORS.map((a) => [a.key, a]),
);

type MatrixAnchor = (typeof TROUT_AUDIT_ANCHORS)[number];
...
function recommenderRequestFromArchiveScenario(
  ...
  if (engineReq.context !== RIVER) {
    throw new Error(`expected trout engine context ${RIVER}; got ${engineReq.context}`);
  }
  return engineReq;
}
...
function parseScenarioAnchorKey(scenarioId: string): string | null {
  const m = scenarioId.match(/^trout_matrix_(.+)_\d{2}$/);
  return m?.[1] ?? null;
}
...
function archivedWeatherAudit() {
  ...
  for (const sc of bundle.scenarios) {
    ...
    const res = runRecommenderRebuildSurface(req, { userSeed: "trout-rebuild-audit-static" });
...
/** River-only: one water_type dimension (no lake × river doubling). */
function structuralSweep() {
  const regimes: DailyRegime[] = ["suppressive", "neutral", "aggressive"];
  const rows = TROUT_SEASONAL_ROWS_V4.filter((r) =>
    ...
    for (const regime of regimes) {
      for (const surfaceBlocked of [false, true]) {
        ...
        if (lurePicks.length === 3) fullLure++;
          ...
          `${row.region_key} m${row.month} ${regime} surfBlk=${surfaceBlocked} →${lurePicks.length}`,
        );
        if (flyPicks.length === 3) fullFly++;
        else {
          flyShort.push(
            `${row.region_key} m${row.month} ${regime} surfBlk=${surfaceBlocked} →${flyPicks.length}`,
          );
        }
...
function repetitionAudit() {
  const raw = Deno.readTextFileSync(ARCHIVE_PATH);
  const bundle = JSON.parse(raw) as ArchiveBundle;
  const june = bundle.scenarios.find((s) => s.scenario_id === "trout_matrix_appalachian_tailwater_06");
  if (!june?.env_data) {
    return { error: "missing trout_matrix_appalachian_tailwater_06 in archive" };
  }
  const anchor = ANCHOR_BY_KEY.get("appalachian_tailwater")!;
  ...
  return {
    scenario: "appalachian_tailwater_june_archived_env",
    days: 14,
    ...
    JSON.stringify(
      {
        ...
        product_rule: { species: "trout", context_only: RIVER },
        region_tiers: {
          mainstream: [...MAINSTREAM_REGIONS],
          fringe_in_matrix_archive: [...FRINGE_REGIONS],
        },
        archived_weather: archived,
        structural_exact_slot_sweep_river_only: structural,

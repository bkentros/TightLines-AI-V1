...
const v10Path = resolve(root, "supabase/migrations/20260919230000_pier_cast_st_joseph_harrisville_private_pass3_v10.sql");
const outputPath = resolve(root, "supabase/migrations/20260921230000_pier_cast_pentwater_caseville_private_pass3_v11.sql");
...
if (pairs.length !== 254 || modeCount !== 451 || new Set(pairs.map((pair) => pair.pairKey)).size !== 254 || pairs.some((pair) => pair.speciesId === "bluegill")) {
  throw new Error("Formula v3 Pass 3 pair manifest is incomplete or exposes bluegill.");
}
const expectedPairs = pairs.map((pair) => `    ('${pair.cityId}','${pair.speciesId}')`).join(",\n");

const source = readFileSync(v10Path, "utf8");
...
const end = source.indexOf(`revoke all on function public.commit_pier_cast_st_joseph_harrisville_lmhofs_cycle`, start);
if (start < 0 || end < 0) throw new Error("Formula v3 migration template anchors were not found.");
let v3 = source.slice(start, end)
  .replaceAll("piercast-v3-twenty-seven-city-st-joseph-harrisville-pass3-v10", "piercast-v3-thirty-two-city-pentwater-caseville-pass3-v11")
  .replaceAll("pier-cast-opportunity-modes-v3-shadow-v1.8.0", "pier-cast-opportunity-modes-v3-shadow-v1.9.0")
  .replaceAll("jsonb_array_length(p_forecasts)<>1110", "jsonb_array_length(p_forecasts)<>1255")
  .replaceAll("<>27", "<>32")
  .replaceAll("<>222", "<>254")
  .replaceAll("'forecastCount',1110", "'forecastCount',1255")
  .replaceAll("invalid Formula v3 st-joseph-harrisville-pass3 shadow payload", "invalid Formula v3 pentwater-caseville-pass3 shadow payload")
  .replaceAll("existing Formula v3 st-joseph-harrisville-pass3 run is incomplete", "existing Formula v3 pentwater-caseville-pass3 run is incomplete")
  .replaceAll("Formula v3 st-joseph-harrisville-pass3 run manifest is incomplete", "Formula v3 pentwater-caseville-pass3 run manifest is incomplete")
  .replace("forecast_count in (180,280,350,470,590,845,865,1110)", "forecast_count in (180,280,350,470,590,845,865,1110,1255)");
v3 = v3.replace(
...
const newCases = [
  ["pentwater_mi", 218, 161, 43.78, -86.45], ["rogers_city_mi", 382, 426, 45.42, -83.80],
  ["tawas_city_mi", 267, 456, 44.27, -83.50], ["charlevoix_mi", 372, 278, 45.32, -85.28],
  ["caseville_mi", 235, 478, 43.95, -83.28],
];
const cityCases = [...existingCases, ...newCases];
...
const newCityCasesSql = newCases.map(([id, row, column, latitude, longitude]) =>
...
const prefix = `-- Add Pentwater, Rogers City, Tawas City, Charlevoix, and Caseville to
...
comment on function public.piercast_v3_expected_pairs() is
  'Exact private Formula v3 32-city, 254-pair manifest. Bluegill is intentionally absent; absence otherwise is not biological absence.';
...
if (process.argv.includes("--check")) {
  if (readFileSync(outputPath, "utf8") !== output) throw new Error("Pentwater–Caseville Pass 3 migration has drifted.");
  console.log("Pentwater–Caseville Pass 3 migration is current.");
} else {
  writeFileSync(outputPath, output);
  console.log(`Generated Pass 3 migration with ${pairs.length} pairs and ${modeCount} modes.`);
}

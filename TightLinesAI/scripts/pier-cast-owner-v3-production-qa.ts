...
if (!cohorts) {
  throw new Error("No complete matching owner-review cycle within 24 hours.");
}
...
if (outlook.cities.length !== 32 || rankedCities.length !== 32) {
  throw new Error(
    `Owner review returned ${outlook.cities.length} cities and ${rankedCities.length} ranked cities; expected 32 each.`,
  );
}
console.log(
  `PASS: private v3 model yields ${rankedCities.length} ranked cities from matching ${cohorts.issuedAt} cycle (${
    outlook.source.cycleAgeHours.toFixed(1)
  } hours old).`,
);

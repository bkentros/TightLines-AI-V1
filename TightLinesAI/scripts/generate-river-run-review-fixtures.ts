...
const generated = `// This file is generated by scripts/generate-river-run-review-fixtures.ts.
...
if (Deno.args.includes("--check")) {
  ...
  console.log(
    `River Run review fixtures match production copy (${scenarioCount(groups)} scenarios).`,
  );
} else {
  await Deno.writeTextFile(outputPath, generated);
  console.log(
    `Generated ${scenarioCount(groups)} River Run review scenarios from production scoring code.`,
  );
}
...
function runStageGroup(): Group {
  ...
  return {
    id: "run_stage",
    label: "Run Stage",
    scenarios: withBothVariants(states.flatMap(([id, label, localDate]) =>
...
function pushGroup(): Group {
  ...
  return {
    id: "push",
    label: "Push",
    scenarios: withBothVariants(cases.flatMap(([id, label, overrides]) =>
      variants().map((variant) => {
...
function snapshotScenario(input: {
  ...
  return {
    ...
    snapshot: {
      ...
      waterTemperature: input.missingSources
        ? null
        : {
      ...
      conditionsWaterTemperature: input.missingSources
        ? null
        : {
          provider: "MONITOR_MY_WATERSHED",
          sourceId: "pmtu_m37",

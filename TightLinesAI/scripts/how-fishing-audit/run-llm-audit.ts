...
function buildNarrationPrompt(
  ...
  if (cc) {
    engineVerdict.temperature_band = cc.temperature_band;
    engineVerdict.temperature_metabolic_context = cc.temperature_metabolic_context;
    engineVerdict.avoid_midday_for_heat = cc.avoid_midday_for_heat;
    engineVerdict.recommended_fishing_dayparts = cc.highlighted_dayparts_for_narration;
    engineVerdict.temperature_trend = cc.temperature_trend;
    if (cc.temperature_shock !== "none") engineVerdict.temperature_shock = cc.temperature_shock;
    if (cc.pressure_detail) engineVerdict.pressure_detail = cc.pressure_detail;
    if (cc.wind_detail) engineVerdict.wind_detail = cc.wind_detail;
    if (cc.tide_detail) engineVerdict.tide_detail = cc.tide_detail;
    engineVerdict.region = cc.region_key;
    engineVerdict.scored_variables_present = cc.available_variables;
    if (cc.missing_variables.length > 0) engineVerdict.data_gaps_variable_keys = cc.missing_variables;
  }

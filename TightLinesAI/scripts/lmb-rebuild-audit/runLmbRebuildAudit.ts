...
/**
 * LMB-only rebuild recommender audit (archived weather + structural + repetition).
...
} from "../../supabase/functions/_shared/recommenderEngine/testFixtures/syntheticHowsScoreEnvLake.ts";
...
import { LARGEMOUTH_AUDIT_ANCHORS } from "../recommender-v3-audit/largemouthAuditMatrix.ts";

const ARCHIVE_PATH = new URL(
  "../../docs/recommender-v3-audit/generated/largemouth-v3-matrix-archive-env.json",
  import.meta.url,
);

/** Product-priority LMB regions (matrix + CSV audit focus). */
...
const ANCHOR_BY_KEY = new Map(
  LARGEMOUTH_AUDIT_ANCHORS.map((a) => [a.key, a]),
);

type MatrixAnchor = (typeof LARGEMOUTH_AUDIT_ANCHORS)[number];

/** Same Edge path: raw `env_data` → `buildSharedEngineRequestFromEnvData` (via `buildRecommenderEngineRequest`). */
...
function parseScenarioAnchorKey(scenarioId: string): string | null {
  const m = scenarioId.match(/^lmb_matrix_(.+)_\d{2}$/);
  return m?.[1] ?? null;
}
...
function archivedWeatherAudit() {
  ...
  const mainstream: typeof bundle.scenarios = [];
  const fringe: typeof bundle.scenarios = [];
  ...
  for (const sc of bundle.scenarios) {
    ...
    const anchor = ANCHOR_BY_KEY.get(anchorKey);
    if (!anchor) continue;
    ...
    if (isMain) {
      tallies.mainstream_total++;
      mainstream.push(sc);
    } else {
      tallies.fringe_total++;
      fringe.push(sc);
    }
    ...
    const res = runRecommenderRebuildSurface(req, { userSeed: "lmb-rebuild-audit-static" });
...
function structuralSweep() {
  ...
  const rows = LARGEMOUTH_BASS_SEASONAL_ROWS_V4.filter((r) =>
  ...
  return {
    ...
    lure_3_pct: pct(fullLure, cells),
    fly_3_pct: pct(fullFly, cells),
    both_3_pct: pct(fullBoth, cells),
...
/** 14-day same-user simulation (Florida lake June archived env). */
function repetitionAudit() {
  const raw = Deno.readTextFileSync(ARCHIVE_PATH);
  const bundle = JSON.parse(raw) as ArchiveBundle;
  const june = bundle.scenarios.find((s) => s.scenario_id === "lmb_matrix_florida_lake_06");
  if (!june?.env_data) {
    return { error: "missing lmb_matrix_florida_lake_06 in archive" };
  }
  const anchor = ANCHOR_BY_KEY.get("florida_lake")!;
  ...
  return {
    scenario: "florida_lake_june_archived_env",
    days: 14,
    ...
    fly_bundles: flySets,
    top_fly_series: topFly,
  };
}

/** Optional: print synthetic How’s fixture scores (test-only env shapes). */
function printHowsFixtureScores(): void {
  const rows: { id: string; score: number; regime: ReturnType<typeof regimeFromHowsScore> }[] = [
    (() => {
      const r = syntheticLmbLakeRequest(syntheticHowsEnvLmbLakeSuppressive());
      const s = analyzeRecommenderConditions(r).scored.score;
      return { id: "lmb_lake_fl_apr_suppressive_v1", score: s, regime: regimeFromHowsScore(s) };
    })(),
    (() => {
      const r = syntheticLmbLakeRequest(syntheticHowsEnvLmbLakeNeutralMiddling());
      const s = analyzeRecommenderConditions(r).scored.score;
      return { id: "lmb_lake_fl_apr_neutral_middling_v1", score: s, regime: regimeFromHowsScore(s) };
    })(),
    (() => {
      const r = syntheticLmbLakeRequest(syntheticHowsEnvLmbLakeAggressive());
      const s = analyzeRecommenderConditions(r).scored.score;
      return { id: "lmb_lake_fl_apr_aggressive_v1", score: s, regime: regimeFromHowsScore(s) };
    })(),
  ];
  console.log(JSON.stringify({ synthetic_hows_fixtures: rows }, null, 2));
}

if (import.meta.main) {
  if (Deno.args.includes("--hows-fixtures")) {
    printHowsFixtureScores();
  } else {
